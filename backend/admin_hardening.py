    ...
    try:
        ...
        await send_email(
            ...
            html=(
                ...
                f"<li><strong>Time:</strong> {datetime.now(timezone.utc).strftime('%d/%m/%Y %H:%M UTC')}</li></ul>"
...
def status_summary() -> dict:
    """Read-only posture surfaced by /api/admin/hardening/status. Never
    returns the actual gate key or allowlist entries - only booleans and
    counts so the value can safely land in the System Health UI."""
    allow = _allowlist()
    return {
        "gate_enabled": bool(ADMIN_GATE_KEY),
        "allowlist_enabled": len(allow) > 0,
        "allowlist_entry_count": len(allow),
    }
