...
# ============================================================================
# SECTION 17, Statement Decoder: dedup-bypass window (DEC-1 v7.7 addendum)
# ============================================================================
# When a super admin needs to run determinism / regression tests against
# production, the statements-persistence dedup guard blocks the second
# and third decode of the same PDF. This section provides a role-gated,
# time-limited "bypass window" per user so QA can be run without weakening
# the guard for regular users.
#
# Contract:
#   * Only super_admin can grant, revoke, or read.
#   * Duration is clamped: 1 hour minimum, 168 hours (7 days) maximum.
#   * `dedup_bypass_until` is an ISO timestamp on the user document. When
#     it is in the future, _persist_decoded_statement_for_user skips the
#     extracted_fingerprint dedup check for that user only.
#   * Every grant, revocation, and use is recorded in audit_log.

MAX_DEDUP_BYPASS_HOURS = int(os.environ.get("MAX_DEDUP_BYPASS_HOURS", "168"))


class DedupBypassGrant(BaseModel):
    duration_hours: int = Field(24, ge=1, le=MAX_DEDUP_BYPASS_HOURS)
    reason: str = Field(..., min_length=4, max_length=280)


@phase_e.post("/users/{user_id}/dedup-bypass")
async def grant_dedup_bypass(
    user_id: str, body: DedupBypassGrant,
    admin: dict = Depends(require_super_admin),
):
    """Open a statement-decoder dedup-bypass window on a user account.
    Super-admin only. Auto-expires; another grant extends the window from
    the moment of the second grant (does not stack)."""
    target = await db.users.find_one({"id": user_id}, {"_id": 0, "id": 1, "email": 1})
    if not target:
        raise HTTPException(404, "User not found")
    hours = min(max(1, body.duration_hours), MAX_DEDUP_BYPASS_HOURS)
    until = (datetime.now(timezone.utc) + timedelta(hours=hours)).isoformat()
    await db.users.update_one(
        {"id": user_id},
        {"$set": {
            "dedup_bypass_until": until,
            "dedup_bypass_reason": body.reason,
            "dedup_bypass_granted_by": admin["id"],
            "dedup_bypass_granted_at": _now(),
        }},
    )
    await audit_log(
        admin["id"], "statement_dedup_bypass_granted",
        target_id=user_id,
        detail={"until": until, "hours": hours, "reason": body.reason,
                "target_email": target.get("email")},
    )
    return {"ok": True, "user_id": user_id, "dedup_bypass_until": until, "hours": hours}


@phase_e.delete("/users/{user_id}/dedup-bypass")
async def revoke_dedup_bypass(
    user_id: str, admin: dict = Depends(require_super_admin),
):
    """Close a dedup-bypass window immediately."""
    target = await db.users.find_one({"id": user_id}, {"_id": 0, "id": 1, "email": 1, "dedup_bypass_until": 1})
    if not target:
        raise HTTPException(404, "User not found")
    await db.users.update_one(
        {"id": user_id},
        {"$unset": {
            "dedup_bypass_until": "",
            "dedup_bypass_reason": "",
            "dedup_bypass_granted_by": "",
            "dedup_bypass_granted_at": "",
        }},
    )
    await audit_log(
        admin["id"], "statement_dedup_bypass_revoked",
        target_id=user_id,
        detail={"previous_until": target.get("dedup_bypass_until"),
                "target_email": target.get("email")},
    )
    return {"ok": True, "user_id": user_id}


@phase_e.get("/users/{user_id}/dedup-bypass")
async def read_dedup_bypass(
    user_id: str, _: dict = Depends(require_super_admin),
):
    """Show the current dedup-bypass state for a user."""
    u = await db.users.find_one(
        {"id": user_id},
        {"_id": 0, "id": 1, "email": 1,
         "dedup_bypass_until": 1, "dedup_bypass_reason": 1,
         "dedup_bypass_granted_by": 1, "dedup_bypass_granted_at": 1},
    )
    if not u:
        raise HTTPException(404, "User not found")
    until = u.get("dedup_bypass_until")
    active = False
    if until:
        try:
            active = datetime.fromisoformat(until.replace("Z", "+00:00")) > datetime.now(timezone.utc)
        except Exception:
            active = False
    return {
        "user_id": u.get("id"),
        ...
        "active": active,
        "dedup_bypass_until": until,
        "dedup_bypass_reason": u.get("dedup_bypass_reason"),
        "dedup_bypass_granted_by": u.get("dedup_bypass_granted_by"),
        "dedup_bypass_granted_at": u.get("dedup_bypass_granted_at"),
    }
