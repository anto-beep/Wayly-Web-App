...
def render_digest_html(digest: Dict[str, Any]) -> str:
    ...
    if total == 0:
        wellbeing_body = f"<p style='margin:0;color:#5C6878'>{participant} did not log a mood this week, that is fine. If you want to nudge them, the Participant view has big friendly buttons.</p>"
    else:
        ...
        wellbeing_body = (
            ...
            f"That is <strong>{total}</strong> check-in{'s' if total != 1 else ''} from {participant} this week."
...
<html><body style="margin:0;background:#EAF4FB;font-family:Helvetica,Arial,sans-serif;color:#0E2A47">
  <table align="center" style="width:600px;max-width:100%;background:#fff;border-radius:12px;border:1px solid #CFE0F0;overflow:hidden;margin:24px auto">
    ...
    <tr><td style="padding:16px 28px;background:#F0EBE0;color:#888;font-size:11px;line-height:1.5">
      You are getting this digest because you are on {participant}'s Family plan. <a href="https://wayly.com.au/settings/notifications" style="color:#0E2A47">Change what you get</a> · <a href="https://wayly.com.au/trust" style="color:#0E2A47">Privacy</a> · Crisis: Lifeline 13 11 14 · 1800ELDERHelp 1800 353 374.
    </td></tr>
  </table>
</body></html>"""
