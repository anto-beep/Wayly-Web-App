...
PLAN_PRICES_MONTHLY = {
    # Legacy name kept for compatibility; values are actually per-fortnight
    # under PRICING-UI-1 v11.
    "FREE": 0.0,
    "SOLO": 24.50,
    "FAMILY": 49.50,
    "ADVISER": 299.0,
...
ADDON_PRICE_MONTHLY = 24.50  # per-fortnight, name kept for import compat
SEAT_LIMITS = {"FREE": 1, "SOLO": 1, "FAMILY": 3, "ADVISER": 3, "ADVISER_PRO": 9999}
PARTICIPANT_BASE_INCLUDED = {"FREE": 1, "SOLO": 1, "FAMILY": 2, "ADVISER": 20, "ADVISER_PRO": 99999}
...
class ParticipantCreateV2(BaseModel):
    ...
    # Signup-flow hint. When creating the SECOND participant at signup from
    # the "Add one more person" toggle, the caller passes is_primary=False
    # so the primary slot is reserved for the account owner (participant
