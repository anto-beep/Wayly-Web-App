...
_redis = None
_redis_ready = False
_redis_disabled = False  # set once a connect attempt fails: fail-open for the process


async def _get_redis():
    global _redis, _redis_ready, _redis_disabled
    ...
    try:
        ...
        # Fail-fast timeouts so an unreachable Redis fails OPEN quickly instead
        # of hanging the request (login and other rate-limited endpoints).
        client = redis_async.from_url(
            url, encoding="utf-8", decode_responses=True,
            socket_connect_timeout=0.75, socket_timeout=0.75, retry_on_timeout=False,
        )
        await client.ping()
        _redis = client
        _redis_ready = True
        ...
        # Disable for the life of the process so we don't re-attempt the slow
        # connect on every request. Rate limiting fails open (documented).
        log.warning("rate-limit Redis unavailable (%s), disabling rate limiting for this process (fail-open).", e)
        _redis = None
        _redis_disabled = True
        return None
