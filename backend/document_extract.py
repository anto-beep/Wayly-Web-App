...
async def extract_document(filename: str, raw: bytes) -> Tuple[str, str, int, List[str]]:
    ...
    if ext == ".docx":
        # CPU-bound parse, run off the event loop so a large document
        # can't block liveness (/api/health) for other requests.
        text = await asyncio.to_thread(_extract_docx_text, raw)
        return text, "word_document", 1, warnings
    ...
    if ext == ".pdf":
        # pdfplumber/pypdf text extraction is synchronous and CPU-bound;
        # run it on a worker thread so it never starves the event loop.
        text, page_count = await asyncio.to_thread(_extract_pdf_selectable_text, raw)
        ...
        try:
            # pdf2image rasterisation (200 DPI, up to 8 pages) + OpenCV/PIL
            # work is the heaviest sync path; keep it off the event loop.
            page_records = await asyncio.to_thread(_pdf_to_image_pages_b64, raw, 8)
    ...
    if ext in (".jpg", ".jpeg", ".png", ".heic", ".heif", ".webp"):
        try:
            # PIL decode + EXIF transpose + OpenCV rotate + JPEG encode are
            # synchronous and CPU-bound; offload to a thread.
            b64, quality = await asyncio.to_thread(_image_to_base64_jpeg, raw, ext)
        except CorruptFileError:
            raise
        ...
        if quality.get("rating") == "poor":
            warnings.append(
                "Overall photo quality is poor. We will do our best, but if results look wrong, "
