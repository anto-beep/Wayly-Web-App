...
events already emitted from `observability.py`. When a threshold is crossed we:
  ...
  3. (Best-effort) send a notification email to `support@wayly.com.au` via the
...
RULE_THRESHOLDS = {
    ...
    "STATEMENT_STORAGE_DRIFT": {
        "event": "STATEMENT_STORAGE_DRIFT",
        "window_s": 1,
        "limit": 1,
        "severity": "HIGH",
        "description": "A non-deleted statement is missing or corrupt on disk, storage cross-check found drift.",
    },
    "STATEMENT_DERIVATION_DRIFT": {
        "event": "STATEMENT_DERIVATION_DRIFT",
        "window_s": 1,
        "limit": 1,
        "severity": "HIGH",
        "description": "Nightly reconciliation found a YTD total that drifted by > $0.50 vs. the prior snapshot.",
    },
}
...
async def record_storage_drift(db, *, drift_rows: list[dict]) -> None:
    """Phase 2 §Observability, pipe storage cross-check findings into the
    same `system_alerts` collection used by malware uploads so they
    surface in the Admin dashboard. One alert per drift row up to the
    list we receive; the cross-check itself caps the list (default 50)
    so an outage that touched 10k rows doesn't flood the inbox.

    Cool-down logic in `_fire_alert` already de-dupes the same
    (rule + subject) within `ALERT_COOLDOWN_S`, so re-runs of the
    sweeper won't re-page on a known-bad row that an admin is already
    triaging.
    """
    if not drift_rows:
        return
    ...
    for drift in drift_rows:
        sid = (drift.get("id") or "")[:48]
        await _fire_alert(
            db,
            rule_key="STATEMENT_STORAGE_DRIFT",
            subject_value=f"statement:{sid}",
            count=1,
            now=now,
            since=now,
            extra={
                "statement_id": sid,
                "household_id": drift.get("household_id"),
                "filename": (drift.get("filename") or "")[:120],
                "statement_state": drift.get("state"),
                "problem": drift.get("problem"),
                # Deep-link the admin straight to the drift row.
                "admin_link": f"/admin/statements/{sid}",
            },
        )


async def record_derivation_drift(db, *, drift_rows: list[dict]) -> None:
    """Phase 4: nightly reconciliation surfaces YTD-total drift into the
    same admin pipeline as storage drift + malware uploads. One alert
    per (household, participant, year) tuple. Cool-down de-dupes
    re-pages while an admin is investigating.
    """
    if not drift_rows:
        return
    now = datetime.now(timezone.utc)
    for drift in drift_rows:
        hh = (drift.get("household_id") or "")[:48]
        pid = (drift.get("participant_id") or "none")
        yr = drift.get("year")
        subject = f"recon:{hh}:{pid}:{yr}"
        await _fire_alert(
            db,
            rule_key="STATEMENT_DERIVATION_DRIFT",
            ...
            extra={
                ...
                "participant_id": pid,
                "year": yr,
                "drift_aud": drift.get("drift_aud"),
                "value_aud": drift.get("value_aud"),
                "prior_value_aud": drift.get("prior_value_aud"),
                "admin_link": f"/admin/households/{hh}/reconciliation",
            },
        )
...
async def _notify_admins(rule_key: str, subject: str, count: int, rule: dict) -> None:
    try:
        ...
        to_addr = os.environ.get("SECURITY_ALERT_EMAIL", "support@wayly.com.au")
