...
async def add_participant(payload: ParticipantCreate, request: Request):
    ...
    # A pre-existing non-primary participant (e.g. the "Add one more person"
    # stub from signup) shouldn't push the true primary into secondary. So
    # base `is_primary` on "is there already a primary?" rather than raw
    # count.
    has_primary = await _db.participants.count_documents({
        "household_id": hid,
        "is_archived": {"$ne": True},
        "is_primary": True,
    })
...
async def send_sms_test(payload: _SmsTestBody, request: Request):
    # UI-1 §10.2, SMS feature deprecated. The UI has been removed from
    # Settings; this endpoint now returns 410 Gone. The column-level data is
    # left in place pending the formal migration described in §10.2.
    raise HTTPException(
        status_code=410,
        detail="SMS notifications have been removed from Wayly. Use email and the in-app inbox instead.",
    )
...
_DEFAULT_MEANS_TEST = {
    ...
    # in _load_means_test_settings(). The figures below are the seeded values
    # from `seed_program_reference.py`, kept here only as a compile-time
    # fallback for the rare boot path where program_reference is unavailable.
    # Verified against the Department of Health "Schedule of Subsidies and
    # Supplements for Support at Home" (effective 1 November 2025), see
    # /app/docs/audit-9a-classification-figures.md.
    "lifetime_cap_new_entrant": 135318.69,
    "lifetime_cap_grandfathered": 84571.66,
    "subsidy_by_classification": {
        "1": 10731, "2": 16034, "3": 21966, "4": 29696,
        "5": 39697, "6": 48114, "7": 58148, "8": 78106,
    },
}
