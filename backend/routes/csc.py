...
def build_csc_router(
    *,
    ...
    async def _store_run(user: Optional[dict], payload: CSCPayload, body: Optional[CSCRunRequest] = None) -> None:
        ...
        try:
            ...
            # Persist the raw inputs so a saved check can be re-opened, edited
            # and re-run from the "Saved checks" list.
            if body is not None:
                set_fields["answers"] = body.answers.model_dump()
                set_fields["current_classification"] = body.current_classification
            await db.csc_runs.update_one(
                {"csc_run_id": payload.csc_run_id},
                {
                    "$set": set_fields,
                    # Preserve a name/saved flag if the user already saved this run.
                    "$setOnInsert": {"name": None, "saved": False},
                },
                upsert=True,
            )
    ...
    def _run_summary(doc: dict) -> dict:
        """Compact shape for the Saved checks list — enough to display and to
        re-open/re-run without a second fetch."""
        payload = doc.get("payload") or {}
        c = payload.get("classification") or {}
        return {
            "csc_run_id": doc.get("csc_run_id"),
            "name": doc.get("name"),
            "saved": bool(doc.get("saved")),
            "persona": doc.get("persona") or payload.get("persona") or "caregiver",
            "created_at": doc.get("created_at"),
            "current_classification": doc.get("current_classification"),
            ...
            "classification": {
                ...
                "annual_budget_low": c.get("annual_budget_low"),
                "annual_budget_high": c.get("annual_budget_high"),
            },
            "gap_detected": bool(payload.get("gap_detected")),
            "gap_direction": payload.get("gap_direction"),
        }
    ...
    async def csc_run(body: CSCRunRequest, request: Request, response: Response):
        ...
        await _store_run(user, payload, body)
        return payload.model_dump()

    # --- 1b. Saved checks management (list / rename+save / delete) -------

    @router.get("/public/csc/runs")
    async def csc_list_runs(request: Request):
        """List the current user's SAVED checks (newest first)."""
        user = await _current_user(request)
        if not user or not user.get("id"):
            raise HTTPException(status_code=401, detail="Sign in to view saved checks.")
        cur = db.csc_runs.find({"user_id": user["id"], "saved": True})
        docs = await cur.to_list(length=200)
        docs.sort(key=lambda d: d.get("created_at") or "", reverse=True)
        return {"runs": [_run_summary(d) for d in docs]}

    @router.patch("/public/csc/runs/{run_id}")
    async def csc_patch_run(run_id: str, body: CSCRunPatch, request: Request):
        """Name and/or save a completed check, or rename an existing one."""
        user = await _current_user(request)
        if not user or not user.get("id"):
            raise HTTPException(status_code=401, detail="Sign in to save checks.")
        doc = await db.csc_runs.find_one({"csc_run_id": run_id, "user_id": user["id"]})
        if not doc:
            raise HTTPException(status_code=404, detail="Check not found")
        upd: dict = {}
        if body.name is not None:
            upd["name"] = body.name.strip() or None
        if body.saved is not None:
            upd["saved"] = body.saved
        if upd:
            upd["updated_at"] = datetime.now(timezone.utc).isoformat()
            await db.csc_runs.update_one({"csc_run_id": run_id, "user_id": user["id"]}, {"$set": upd})
            doc.update(upd)
        return {"run": _run_summary(doc)}

    @router.delete("/public/csc/runs/{run_id}")
    async def csc_delete_run(run_id: str, request: Request):
        user = await _current_user(request)
        if not user or not user.get("id"):
            raise HTTPException(status_code=401, detail="Sign in to delete checks.")
        res = await db.csc_runs.delete_one({"csc_run_id": run_id, "user_id": user["id"]})
    ...
    async def csc_bands():
        ...
        for n in range(1, 9):
            annual = round(float(get_value(f"classification_annual.{n}")), 2)
            # Official quarterly budget = annual / 4 and INCLUDES the 10% care
            # management allocation (Department of Health schedule), so this is
            # what the "Quarterly budget" fields across the app prefill with.
