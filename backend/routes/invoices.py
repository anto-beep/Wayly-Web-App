...
from lib.upload_guard import classify_upload
...
def build_invoices_router(
    *,
    ...
    async def upload_invoice(
        ...
        override_guard: bool = Form(False),
        ...
        # Mirror the Statement Decoder behaviour: reject re-uploads of the
        # exact same bytes for this user so they don't create silent
        # duplicate invoices. Excludes soft-deleted invoices so a user who
        # deleted one can re-upload it.
        file_sha = hashlib.sha256(raw).hexdigest()
        existing_dupe = await db.invoices.find_one(
            {
                "user_id": user_id,
                "file_sha256": file_sha,
                "state": {"$ne": "deleted"},
            },
            {"_id": 0, "id": 1, "filename": 1, "invoice_date": 1,
             "provider_name": 1, "created_at": 1},
        )
        if existing_dupe:
            raise HTTPException(
                status_code=409,
                detail={
                    ...
                    "existing_invoice_id": existing_dupe["id"],
                    "existing_filename": existing_dupe.get("filename"),
                    "existing_provider_name": existing_dupe.get("provider_name"),
                    "existing_invoice_date": existing_dupe.get("invoice_date"),
                    "existing_created_at": existing_dupe.get("created_at"),
                    "message": "You have already checked this exact invoice.",
                },
            )
        ...
        # UPLOAD-GUARD-1 (v1): server-authoritative document validation.
        # The statement wrong-tool case is left to the existing statement
        # redirect below so its UX is unchanged; every other block/confirm
        # is surfaced via the shared `upload_guard` verdict.
        guard = classify_upload("invoice-checker", filename, raw, file.content_type or "", text or "")
        _guard_detected = guard.get("detected_type")
        _is_statement_wrongtool = guard["reason"] == "wrong_tool" and _guard_detected == "statement-decoder"
        if guard["decision"] == "block" and not _is_statement_wrongtool:
            return {
                "invoice_id": None,
                "upload_guard": guard,
                "document_shape": None,
        ...
        if guard["decision"] == "confirm" and not override_guard:
            return {
                "invoice_id": None,
                "upload_guard": guard,
                "document_shape": None,
                "checks_status": "needs_confirmation",
                "created_at": _now_iso(),
            }
        ...
        if resolve_active_participant is not None:
            try:
                ...
                if p:
                    ...
                    _active_participant = p
            except Exception:
                pass

        # Anti account-sharing: compare the name on the invoice against the
        # participant being viewed (and the rest of the household).
        ...
        try:
            ...
            others: list = []
            if hid:
                async for op in db.participants.find(
            ...
            if _active_participant:
                active_ctx = {
                    "id": _active_participant.get("id"),
                    "name": _active_participant.get("name") or _active_participant.get("first_name"),
                }
            elif others:
                # No active header; default to the household's primary.
                primary = next((o for o in others if o.get("is_primary")), others[0])
                active_ctx = {"id": primary.get("id"), "name": primary.get("name") or primary.get("first_name")}
            identity_warning = check_document_identity(
                header.get("participant_name"),
                active_ctx,
                others,
                tool_label="invoice",
            )
        except Exception:
            logger.exception("identity guard (invoice) failed")
        if identity_warning:
            doc["identity_warning"] = identity_warning

        # Duplicate detection (non-blocking warning): flag if this looks like an
        # invoice the user has already checked — exact same file, or same
        # provider + invoice date. The client surfaces a dismissible notice.
        dup = None
        try:
            prov = header.get("provider_name")
            idate = header.get("invoice_date")
            or_clauses: list = [{"file_sha256": file_sha}]
            if prov and idate:
                or_clauses.append({"provider_name": prov, "invoice_date": idate})
            existing = await db.invoices.find_one(
                {"user_id": user_id, "state": "active", "$or": or_clauses},
                {"_id": 0, "id": 1, "provider_name": 1, "invoice_date": 1, "created_at": 1, "file_sha256": 1},
                sort=[("created_at", -1)],
            )
            if existing:
                exact = existing.get("file_sha256") == file_sha
                dup = {
                    "invoice_id": existing.get("id"),
                    "provider_name": existing.get("provider_name"),
                    "invoice_date": existing.get("invoice_date"),
                    "checked_at": existing.get("created_at"),
                    "exact_file": exact,
                    "message": ("You've already checked this exact invoice file."
                                if exact else
                                "You've already checked an invoice from this provider for this date."),
                }
        except Exception:
            dup = None
    ...
    async def get_invoice(
        ...
        # Backfill invoice_total for invoices reconciled before the field
        # existed, so the "Amount billed" banner is correct on web + mobile.
        recon = doc.get("reconciliation") or {}
        if recon and not recon.get("invoice_total"):
            total = round(sum(float((ln or {}).get("gross_cost") or 0) for ln in (recon.get("lines") or [])), 2)
            if total:
                recon["invoice_total"] = total
                await db.invoices.update_one(
                    {"id": invoice_id, "user_id": user_id},
                    {"$set": {"reconciliation.invoice_total": total}},
                )
        return doc
