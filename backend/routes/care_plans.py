...
async def _compose_letter_intake(db, participant_id, archetype, issues, provider_name):
    ...
    if participant_id:
        p = await db.participants.find_one(
            {"id": participant_id}, {"_id": 0, "display_name": 1, "first_name": 1, "name": 1}
        )
        if p:
            pname = (p.get("display_name") or p.get("first_name") or p.get("name") or "").strip()
    if not pname:
    ...
    summary = ("These points came out of a review of the support plan, and I would like them addressed:\n" + "\n".join(lines)) if lines else ""
    named = pname and pname != "the participant"
    ...
    field_map = {
        ...
        "dispute": ("disputed_charge_summary", f"Query about charges{who}", {}),
        "complaint": ("complaint_summary", f"Concerns about care{who}", {}),
...
def build_care_plans_router() -> APIRouter:
    ...
    async def letter_from_finding(
        ...
        intake = await _compose_letter_intake(
            ...
            [{"title": finding.get("title"), "detail": finding.get("detail"), "suggested_question": finding.get("suggested_question")}],
            body.provider_name,
        )
        entry = {
            "id": entry_id,
    ...
    async def letter_from_findings(
        ...
        entry_id = str(uuid4())
        now = utcnow_iso()
        intake = await _compose_letter_intake(db, body.participant_id, archetype, issues, body.provider_name)
        entry = {
            "id": entry_id,
            "user_id": user_id,
            "participant_id": body.participant_id,
