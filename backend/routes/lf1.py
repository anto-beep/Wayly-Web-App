...
class PrefillBody(BaseModel):
    """LF-1 v2, seed a starter intake + draft when a situation is first opened."""
...
def build_lf1_router(db, get_current_user_id, get_current_user_id_optional, require_paid_plan=None):
    ...
    async def prefill_letter(
        ...
        """LF-1 v2: seed the intake + draft a starting letter when a user
        opens a freshly picked situation. Keeps the user from starting on a
        blank form. Idempotent, never overwrites a draft the user already has,
        and never clobbers intake fields the user has already filled in."""
        from services import lf1_generate as _gen
        ...
        # Idempotent: a draft already exists, hand it back untouched.
        if (entry.get("content_draft") or "").strip():
            return {"entry": entry, "generated": None, "already_drafted": True}

        situation_id = entry.get("situation_id")
        archetype = entry.get("archetype") or ""

        # Resolve the participant's first name: client-supplied wins, then the
        # stored participant_id lookup, then the warm fallback in the builder.
        participant_first_name = (body.participant_name or "").strip() or None
        if not participant_first_name and entry.get("participant_id"):
            acct = (
                await db.account_members.find_one(
                    {"user_id": user_id, "status": "ACTIVE"}, {"_id": 0, "account_id": 1}
                )
                or await db.accounts.find_one({"owner_user_id": user_id}, {"_id": 0, "id": 1})
                or {}
            )
            account_id = acct.get("account_id") or acct.get("id")
            if account_id:
                p = await db.participants.find_one(
                    {"id": entry.get("participant_id"), "account_id": account_id},
                    {"_id": 0, "first_name": 1, "preferred_name": 1, "name": 1},
                )
                if p:
                    participant_first_name = (
        ...
        starter = lf1.build_starter_intake(situation_id, participant_first_name)
        # User-entered (non-empty) intake values always win over the seeds.
        merged = dict(starter)
        for k, v in (entry.get("intake") or {}).items():
            if v not in (None, "", []):
                merged[k] = v
        entry["intake"] = merged

        # Persist the seeded intake up front so the fields are prefilled and
        # saved even if draft generation is slow or unavailable.
        await db.lf1_correspondence.update_one(
            {"user_id": user_id, "id": entry_id},
            {"$set": {"intake": merged, "updated_at": _iso()}},
        )

        # Safeguarding (phone-first) and response drafts (need the inbound
        # message) only get their fields seeded, not an auto-generated draft.
        if not lf1.situation_auto_draftable(situation_id, archetype):
            row = await db.lf1_correspondence.find_one(
                {"user_id": user_id, "id": entry_id}, {"_id": 0}
            )
    ...
    @r.post("/lf1/correspondence/{entry_id}/email")
    async def email_export(
        entry_id: str,
        user_id: str = Depends(get_current_user_id),
    ):
        """Email the server-rendered letter PDF to the signed-in user."""
        from services import lf1_generate as _gen
        from services import lf1_pdf as _pdf
        ...
        _fname = f"wayly-letter-{(entry.get('archetype') or 'letter')}.pdf"
        html = (
            "<!doctype html><html><body style=\"font-family:Helvetica,Arial,sans-serif;"
            "background:#FBF8F3;padding:24px;color:#1C2B2D\">"
            "<table align=\"center\" style=\"width:600px;max-width:100%;background:#fff;"
            "border-radius:12px;border:1px solid #E7E0D5;overflow:hidden\">"
            ...
            "opacity:.85;margin-top:4px\">Letters &amp; Follow-ups</div></td></tr>"
            "<tr><td style=\"padding:24px 28px\">"
            f"<h2 style=\"margin:0 0 12px;font-family:Georgia,serif;color:#0E4D52\">{subject_text}</h2>"
            "<p style=\"font-size:14px;line-height:1.6\">Your drafted letter is attached as a PDF. "
            "Review it in full before sending.</p></td></tr>"
            "<tr><td style=\"padding:16px 28px;background:#F4EFE7;color:#888;font-size:11px\">"
            "Sent because you requested it from Wayly. wayly.com.au</td></tr>"
            "</table></body></html>"
        )
        result = await _send({
            "from": _sender(),
            "to": [to],
            "subject": f"Your Wayly letter , {subject_text}",
            "html": html,
            "attachments": [{
                "filename": _fname,
