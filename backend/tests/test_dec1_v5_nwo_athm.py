...
def test_rule_15_still_fires_on_real_extraction_miss():
    """Sanity check: a real extraction miss must still be caught. Reported
    $1,000; services extracted $600; CM $50 → the band tops out at $650 vs
    $1,000, a 35% shortfall. The DEC-1 G2 gate supersedes the softer RULE_15
    here (higher-severity block), so accept either signal."""
    ext = _base_extracted(
    ...
    rules = _rules(audit)
    assert (
        "RULE_15_GROSS_TOTAL_PARSE_WARNING" in rules
        or "RULE_G2_GROSS_RECONCILE" in rules
    ), rules
...
def test_rule_15_evidence_includes_care_mgmt_line():
    """The gross-reconciliation anomaly's evidence must show the care
    management amount so users can see how the reconciliation was computed.
    The DEC-1 G2 gate supersedes RULE_15 on a large gap, so accept whichever
    gross-reconcile anomaly the pipeline emits."""
    ext = _base_extracted(
    ...
    recon = next(
        (a for a in audit.get("anomalies", [])
         if a.get("rule") in ("RULE_15_GROSS_TOTAL_PARSE_WARNING", "RULE_G2_GROSS_RECONCILE")),
        None,
    )
    assert recon is not None, "a gross-reconcile anomaly should fire on a $2000 vs $600 gap"
    evidence_str = " ".join(recon.get("evidence") or []).lower()
    assert "care management" in evidence_str
