...
def test_part_pension_no_user_rates_returns_range(auth_token):
    ...
    assert streams["Independence"]["rate_pct_high"] == 50.0
...
def test_part_pension_user_rate_outside_band_returns_400(auth_token):
    r = _post(auth_token, {
        ...
        "expected_mix_independence_pct": 45,
        "expected_mix_everyday_pct": 25,
        "independence_rate_pct": 60.0,  # outside the 5%-50% band
    })
    _check_rate_limit(r)
    assert r.status_code == 400, r.text
    detail = r.json().get("detail") or ""
    assert "Independence" in detail
    assert "5" in detail and "50" in detail


# (5) cshc accepted; matches part band (DoH PDF treats them as the same cohort).
def test_cshc_matches_part_band(auth_token):
    ...
    # Per DoH "participant contributions" PDF (effective 1 November 2025),
    # part Age Pension and CSHC share the same band (Independence 5%-50%,
    # Everyday Living 17.5%-80%).
    assert cshc["annual_contribution_low"] == pytest.approx(part["annual_contribution_low"], abs=0.01)
    assert cshc["annual_contribution_high"] == pytest.approx(part["annual_contribution_high"], abs=0.01)
