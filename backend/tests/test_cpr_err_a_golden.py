...
def test_verification_panel_all_pass():
    ...
    # 5 flagship checks are always present; short-term pathway checks (6-8)
    # are appended only when the plan states those amounts. This fixture states
    # an AT-HM allocation ($1,200) so the AT-HM budget check is also present.
    flagship = {
        "care_management_fee", "at_hm_ring_fence", "personal_care_placement",
        "rollover_cap", "quarterly_budget_vs_classification",
    }
    check_names = {c["check"] for c in checks}
    assert flagship.issubset(check_names), f"missing flagship checks: {flagship - check_names}"
    assert "at_hm_budget" in check_names, "AT-HM budget check should run when an allocation is stated"
