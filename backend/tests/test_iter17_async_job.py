...
BASE_URL = os.environ.get("REACT_APP_BACKEND_URL", "https://wayly-engagement-hub.preview.emergentagent.com").rstrip("/")
...
def test_rule_9_part_age_pension_mismatch_independence():
    """Part-age Independence charged 52.6% is outside the 5%-50% band.
    DEC-FINDINGS-1 decision 3: surfaces as ONE consolidated INFO note, not a
    dollar-level MEDIUM mismatch (the Invoice Checker is authoritative)."""
    import program_reference as pr
    _saved_cache, _saved_ready = getattr(pr, "_CACHE", {}), getattr(pr, "_CACHE_READY", False)
    pr._CACHE = {
    ...
    try:
        ...
        pr._CACHE, pr._CACHE_READY = _saved_cache, _saved_ready
    rules = [a.get("rule") for a in out["anomalies"]]
    assert "RULE_9_CONTRIBUTION_INFO" in rules, out["anomalies"]
    assert "RULE_9_CONTRIBUTION_MISMATCH" not in rules
    assert not [a for a in out["anomalies"] if (a.get("severity") or "").lower() == "medium"]
