...
def test_T2_rollover_class8_q4_percent_wins():
    """Class 8 (annual $80,137.12), Apr-Jun quarter (91 days):
    10% × ($80,137.12/365×0.9) × 91 ≈ $1,798.15."""
    cap = budget.rollover_cap(8, days_in_quarter=91)
    assert 1795 <= cap <= 1802, f"Expected ~$1,798.15, got {cap}"


def test_T3_rollover_class6_q4_percent_wins():
    """Class 6 (annual $49,365.27), Apr-Jun quarter (91 days):
    10% × ($49,365.27/365×0.9) × 91 ≈ $1,107.68."""
    cap = budget.rollover_cap(6, days_in_quarter=91)
    assert 1104 <= cap <= 1112, f"Expected ~$1,107.68, got {cap}"
