...
# ---------------------------------------------------------------------------
# (2) Independence at 60% on a part-pension statement → exactly ONE flag
#     (60% is outside the 5%-50% band)
# ---------------------------------------------------------------------------
def test_part_pension_independence_above_band_flags_once():
    ...
    assert "5" in combined and "50" in combined, (
        f"Detail must cite the 5% to 50% band, got: {combined}"
    )
...
def test_part_pension_inconsistent_rate_flagged():
    ...
    assert mismatches == [], (
        "Both 12% and 19% are inside the 5%-50% band — mismatch must not fire. "
        f"Got: {mismatches}"
    )
