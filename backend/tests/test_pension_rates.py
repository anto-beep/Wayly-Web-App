...
Everyday Living 50%. The real Support at Home structure is:

  Clinical / AT-HM / Care Mgmt: 0% for everyone.
  Independence:
    full Age Pension                    -> exactly  5%
    part Age Pension                    -> band      5% – 25%
    Commonwealth Seniors Health Card    -> band      5% – 50%
    self-funded (no CSHC)               -> exactly 50%
  Everyday Living:
    full Age Pension                    -> exactly 17.5%
    part Age Pension                    -> band   17.5% – 25%
    Commonwealth Seniors Health Card    -> band   17.5% – 80%
    self-funded (no CSHC)               -> exactly 80%

Rule 9 in the Statement Decoder now validates against these bands. The tests
here exercise the deterministic ``_add_parse_warnings`` helper with synthetic
``extracted`` payloads so we do NOT touch the LLM.
"""
...
# ---------------------------------------------------------------------------
# (2) Independence at 30% on a part-pension statement → exactly ONE flag
# ---------------------------------------------------------------------------
def test_part_pension_independence_above_band_flags_once():
    ...
    extracted = _extracted(
        ...
        _line("Independence", 100.0, 12.0, date="2026-03-05", code="IND-01"),
        _line("Independence", 100.0, 12.0, date="2026-03-12", code="IND-01"),
        _line("Independence", 100.0, 30.0, date="2026-03-19", code="IND-02",
    ...
    assert len(mismatches) == 1, f"Expected exactly one mismatch, got {mismatches}"
    detail = (mismatches[0].get("detail") or "")
    headline = (mismatches[0].get("headline") or "")
    combined = f"{headline} {detail}".lower()
    assert "5" in combined and "25" in combined, (
        f"Detail must cite the 5% to 25% band, got: {combined}"
    )
    assert "part age pension" in combined


# ---------------------------------------------------------------------------
# (3) Two Independence lines with different implied rates → INCONSISTENT_RATE
# ---------------------------------------------------------------------------
def test_part_pension_inconsistent_rate_flagged():
    ...
    inconsistent = [
        a for a in audit["anomalies"]
        if (a.get("rule") or "").upper() == "RULE_9_INCONSISTENT_RATE"
    ]
    assert len(inconsistent) == 1, (
        f"Expected exactly one RULE_9_INCONSISTENT_RATE flag, got {audit['anomalies']}"
    )
    # The mismatch flag for 12%/19% lines should NOT fire — both are inside the band.
    mismatches = [
        a for a in audit["anomalies"]
        if (a.get("rule") or "").upper() == "RULE_9_CONTRIBUTION_MISMATCH"
    ]
    assert mismatches == [], (
        "Both 12% and 19% are inside the 5%-25% band — mismatch must not fire. "
        f"Got: {mismatches}"
    )
...
def test_full_pension_exact_rate_flags_when_wrong():
    ...
    detail = (mismatches[0].get("detail") or "").lower()
...
def test_part_or_cshc_unconfirmed_uses_widest_band():
    ...
    # 22% Independence — inside CSHC range (5-50) and inside part range (5-25)
    # → silent.
    ...
    # 65% Independence — outside even the widest 5-50 band → must flag.
    ...
    assert "RULE_9_CONTRIBUTION_MISMATCH" in loud_rule_9, loud
...
def test_pension_rates_table_shape():
    ...
    assert _PENSION_RATES["part_age_pension"]["Independence"] == (0.05, 0.25)
    assert _PENSION_RATES["part_age_pension"]["EverydayLiving"] == (0.175, 0.25)
