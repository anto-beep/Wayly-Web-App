...
def test_level_6_uses_post_cm_quarterly_pct():
    """Aged Care Rules 2025 section 193-5: rollover = 10% × base individual
    daily amount × days in the current quarter, floored at $1,000. For L6 the
    percentage path beats the floor. Date-robust (derives days from the live
    quarter window)."""
    from budget import rollover_cap, classification_annual, get_quarter_window
    qs, qe, _ = get_quarter_window()
    days = (qe - qs).days + 1
    daily_bi = classification_annual(6) / 365.0 * 0.9
    expected = round(daily_bi * days * 0.10, 2)
    assert expected > 1000.00, (
        f"Test pre-condition: L6 percent path must beat the floor; got {expected}"
    )
...
def test_level_7_uses_post_cm_quarterly_pct():
    ...
    qs, qe, _ = get_quarter_window()
    days = (qe - qs).days + 1
    daily_bi = classification_annual(7) / 365.0 * 0.9
    expected = round(daily_bi * days * 0.10, 2)
    assert expected > 1000.00, (
        f"Test pre-condition: L7 percent path must beat the floor; got {expected}"
    )
    assert rollover_cap(7) == pytest.approx(expected, abs=0.01)


def test_level_8_returns_post_cm_value():
    """Level 8 annual $80,137.12 (effective 1 July 2026). Rollover = 10% ×
    base individual daily amount × days in the current quarter."""
    from budget import rollover_cap, classification_annual, get_quarter_window
    assert classification_annual(8) == pytest.approx(80137.12, abs=0.01)
    qs, qe, _ = get_quarter_window()
    days = (qe - qs).days + 1
    daily_bi = classification_annual(8) / 365.0 * 0.9
    expected = round(daily_bi * days * 0.10, 2)
    assert rollover_cap(8) == pytest.approx(expected, abs=0.01)
