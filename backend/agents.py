...
- Cancelled services must be included with is_cancellation: true and gross: 0.00. If the statement charges a fee for the cancellation (e.g. a short-notice cancellation fee), record that fee amount in charged_amount so it is not lost (gross still stays 0.00 because no service was delivered).
- AT-HM items must be coded stream: "ATHM" not "EverydayLiving" even if the statement places them in Everyday Living.
- source_stream: record the stream or category name the statement ITSELF printed this line under, verbatim (e.g. "Everyday Living", "Independence"). Set the "stream" field to the correct Support at Home stream. If you move a line to a different stream than the statement showed (for example an AT-HM item the statement put in Everyday Living, or a service you reclassify), source_stream MUST reflect the ORIGINAL stream the statement showed and stream your corrected value. If the statement's stream is already correct, set source_stream equal to stream.
...
Return STRICT JSON only:
{
  ...
  "package_management_deducted": 0.00,
  "gst_total": 0.00,
...
- reported_total_gross is the statement's printed BOTTOM-LINE TOTAL row in the "STATEMENT SUMMARY" / "TOTAL" section (the single number that sums Clinical + Independence + Everyday Living + Care Management + AT-HM current-period + Previous-period adjustments).- FORBIDDEN SOURCES , do NOT use any figure from:
...
- package_management_deducted is THIS PERIOD'S package management / administration / package administration fee, a fee SEPARATE from care management. It is found in a dedicated line or row typically labelled "Package management fee", "Package administration", "Administration fee", or shown as its own category in the category-breakdown table (e.g. "Package Management $178.00"). Record the current-period dollar amount only; 0.00 if the statement has no separate package/administration management fee. This fee is part of the statement's bottom-line total but is NOT a service line item, so it is captured here (like care management) rather than in line_items.
- gst_total is the TOTAL GST charged to the participant on this statement, if the statement prints a GST line or a GST row in the category-breakdown table (e.g. "GST $18.90"). Record the dollar amount only; 0.00 if no GST is charged. GST is part of the statement's bottom-line total but is NOT a service line item, so it is captured here rather than in line_items.
...
- SAME-DATE DUPLICATES: if two rows share the SAME date, SAME service, and SAME amount (for example two identical "Domestic assistance, general" visits both dated 28/08), you MUST extract BOTH as separate line items. NEVER merge them into one row, and NEVER combine them into a single row with quantity 2. Duplicate detection happens downstream, not during extraction , dropping the second copy makes the statement fail its own total.
...
async def audit_statement(
    ...
    *,
    ...
    try:
        ...
        # Normalise anomaly_count from the canonically-ordered list.
        anoms = result.get("anomalies", []) or []
        from lib.anomaly_order import sort_anomalies, compute_counts
        anoms = sort_anomalies(anoms)
        result["anomalies"] = anoms
        result["anomaly_count"] = compute_counts(anoms)
        return result
...
# Valid pension cohorts (membership only; the numeric rates live in INDEX-1).
_PENSION_COHORTS = {
    "full_age_pension", "part_age_pension", "cshc",
    "self_funded", "part_or_cshc_unconfirmed",
}


def _emit_consolidated_contribution_note(anomalies, extracted, pension_status):
    """DEC-FINDINGS-1 decision 3: the Decoder demotes all contribution
    observations to ONE informational finding. It notes that the charged
    contribution differs from the stream's typical rate, lists the affected
    streams and dates, and points to the Contribution Estimator. It asserts
    NO dollar variance and contains NO refund wording. Rates come from INDEX-1
    (decision 2). The Invoice Checker remains the authoritative dollar-level
    contribution checker.
    """
    from lib import services_base as _sb
    ps = (pension_status or "unknown").strip().lower()
    if ps not in _PENSION_COHORTS:
        anomalies.append({
            "severity": "low",
            ...
            "dollar_impact": 0.0,
            "evidence": [f"pension_status: {ps or 'unknown'}"],
            "suggested_action": (
                "Ask your provider to confirm the participant's recorded pension status, then "
                "check your contributions in the Contribution Estimator."
            ),
        })
        return

    line_items = [li for li in (extracted.get("line_items") or []) if isinstance(li, dict)]
    as_of = extracted.get("period_start") or None
    try:
        total_contrib = float(extracted.get("reported_total_participant_contribution") or 0.0)
    except Exception:
        total_contrib = 0.0
    services_base = _sb.care_services_subtotal(line_items)
    agg_rate = (total_contrib / services_base) if services_base > 0 else 0.0
    ...
    for li in line_items:
        if li.get("is_cancellation") or not _sb.is_care_service_line(li):
            continue
        stream = (li.get("stream") or "").strip()
        eff_stream = _sb.effective_contribution_stream(
            stream, li.get("service_description") or "", as_of,
        )
        band = _sb.contribution_expected_band(eff_stream, ps, as_of)
        if band is None:
            continue
        lo, hi = band
        if hi <= 0:
            continue  # stream carries no contribution (Clinical / AT-HM)
        ...
        try:
            ...
            gross = 0.0
        if contrib is None:
            charged_rate = agg_rate
        else:
            try:
                charged_rate = (float(contrib) / gross) if gross > 0 else 0.0
            except Exception:
                charged_rate = 0.0
        # Note only when the charged rate falls OUTSIDE the expected band.
        if charged_rate < (lo - 0.005) or charged_rate > (hi + 0.005):
            label = _labels.get(eff_stream, eff_stream)
            affected.setdefault(label, []).append(str(li.get("date") or ""))

    if not affected:
        return
    parts = []
    for label in sorted(affected):
        dates = [d for d in affected[label] if d]
        parts.append(f"{label} ({', '.join(dates)})" if dates else label)
    streams_txt = "; ".join(parts)
    cohort_label = _PENSION_STATUS_LABELS.get(ps, ps.replace("_", " "))
    anomalies.append({
        "severity": "info",
        "rule": "RULE_9_CONTRIBUTION_INFO",
        "headline": "The contribution charged differs from the typical rate for these services.",
        "detail": (
            f"On this statement the participant contribution charged differs from the typical "
            f"Support at Home rate for a {cohort_label} participant on: {streams_txt}. "
            f"Contribution rates are means-tested by Services Australia and set per participant, "
            f"so a difference here is not necessarily an error. Check the authoritative figure in "
            f"the Contribution Estimator."
        ),
        "dollar_impact": 0.0,
        "impact_aud": None,
        "evidence": [
            f"pension_status: {ps}",
            f"affected streams: {', '.join(sorted(affected))}",
            "contribution rates sourced from INDEX-1 (program_reference)",
        ],
        "suggested_action": (
            "Open the Contribution Estimator in Wayly for an authoritative contribution figure "
            "for this participant and period."
        ),
    })
...
def _add_parse_warnings(audit_result: Dict[str, Any], extracted: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not (existing_rules & rule_9_keys):
        ...
        from lib import services_base as _sb9
        if _sb9.remediation_b_enabled():
            # DEC-FINDINGS-1 decision 3: one consolidated INFO note only.
            _emit_consolidated_contribution_note(anomalies, extracted, pension_status)
        elif pension_status not in _PENSION_RATES:
            anomalies.append({
                "severity": "low",
                "rule": "RULE_9_PENSION_STATUS_UNKNOWN",
                "headline": "We couldn't confirm this participant's pension status from the statement.",
    ...
    if "RULE_25_SOURCE_ARITHMETIC_GAP" not in existing_rules:
        ...
        if _sb.remediation_a_enabled():
            # DEC-FINDINGS-1 decision 1: reconcile the declared services total
            # against the sum of care-service line items. AT-HM is not part of
            # the care-services subtotal, but some statements print a "services
            # total" that INCLUDES AT-HM (e.g. an AT-HM-only statement). To
            # avoid both the phantom AT-HM gap (SD-ERR-C) and a false gap on
            # AT-HM-only statements, reconcile against whichever interpretation
            # (full line-item sum, or care-services-only sum) is closest to the
            # declared figure. A genuine arithmetic defect fails BOTH.
            _items = extracted.get("line_items") or []
            _persisted = extracted.get("computed_line_item_sum")
            try:
                _full_sum = float(_persisted) if _persisted is not None else _sb.line_items_gross_total(_items)
            except (TypeError, ValueError):
                _full_sum = _sb.line_items_gross_total(_items)
            _care_sum = round(_full_sum - _sb.athm_subtotal(_items), 2)
            if declared_f is not None:
                sum_f = _care_sum if abs(declared_f - _care_sum) <= abs(declared_f - _full_sum) else _full_sum
            else:
                sum_f = _care_sum
        else:
            try:
                # Legacy path: persisted computed_line_item_sum includes AT-HM.
    ...
    if "RULE_1_CARE_MGMT_CAP" not in existing_rules and _cadence == "quarterly":
        ...
        # DEC-FINDINGS-1 decision 1: resolve the cap base through the ONE
        # shared services-base module so the cap check and the reconciliation
        # check can never disagree on what "services" means.
        from lib import services_base as _sb_cap
        if _sb_cap.remediation_a_enabled():
            services_gross_from_items = _sb_cap.care_services_subtotal(_line_items)
    ...
    #      DEC-FINDINGS-1 decision 8: removed. It added noise on otherwise
    #      clean statements without pointing to a recoverable amount or a
    #      participant action that matters. Gated off under remediation D.
    from lib import services_base as _sb_d29
    if ("RULE_29_MISSING_ACT_DISCLOSURE" not in existing_rules and _source_text
            and not _sb_d29.remediation_d_enabled()):
    ...
    # DEC-1 reconciliation gates (P0): per-line arithmetic, budget-vs-INDEX-1,
    # QTD-over-budget, gross reconciliation. Runs after dedup and BEFORE the
    # impact cap so gate impacts are capped too. Any HIGH gate finding blocks
    # the plain-English summary from publishing (sets audit_result['publishable']).
    try:
        from lib.dec1_gates import run_reconciliation_gates
        run_reconciliation_gates(final_deduped, extracted, audit_result)
    except Exception as _gate_err:  # never let a gate crash the decode
        logger.warning("DEC-1 gates failed (non-fatal): %s", _gate_err)
    ...
    # DOC-PARITY-1 decision 5-6: sort into canonical band order once, so both
    # surfaces and the downloadable artefact consume the same ordered list.
    from lib.anomaly_order import sort_anomalies, compute_counts
    final_deduped = sort_anomalies(final_deduped)
    audit_result["anomalies"] = final_deduped
    audit_result["anomaly_count"] = compute_counts(final_deduped)
    return audit_result
