...
def build_facts(
    *,
    ...
    at_hm_total = _search_money(r"at-?hm\s+(?:allocation|budget|funding|cap)[^\n]*?\$\s*([\d,]+(?:\.\d+)?)", text)
    rollover_cap = _search_money(r"rollover cap[^\n]*?\$\s*([\d,]+(?:\.\d+)?)", text)

    # Short-term pathway budgets + AT-HM tier (verification checks 6-8).
    restorative_budget = _search_money(r"restorativ[^\n]*?\$\s*([\d,]+(?:\.\d+)?)", text)
    end_of_life_budget = _search_money(r"(?:end[\s\-]?of[\s\-]?life|palliativ)[^\n]*?\$\s*([\d,]+(?:\.\d+)?)", text)
    at_hm_tier = None
    m_tier = (
        re.search(r"at-?hm[^\n]{0,40}?\b(low|medium|high)\b", text, re.IGNORECASE)
        or re.search(r"\b(low|medium|high)\b[^\n]{0,20}?tier[^\n]{0,20}?at-?hm", text, re.IGNORECASE)
    )
    if m_tier:
        at_hm_tier = m_tier.group(1).lower()

    qb = ex.get("quarterly_budget") if ex.get("quarterly_budget") is not None else quarterly_budget
    ...
    return {
        ...
        "at_hm_tier": at_hm_tier,
        "restorative_budget": restorative_budget,
        "end_of_life_budget": end_of_life_budget,
...
def _index1(key: str) -> Optional[Decimal]:
    """Fetch a single verified dollar figure from the INDEX-1 register.
    Never sourced from the model, so it is safe to compare against."""
    try:
        from monetary_constants import get_value
        v = get_value(key)
        return _q2(Decimal(str(v))) if v is not None else None
    except Exception:      # noqa: BLE001
        return None
...
def run_verification_panel(facts: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if quarterly is None or cls is None:
        ...
        checks.append(_pass(
            "quarterly_budget_vs_classification", "Quarterly budget vs classification",
            f"The plan's quarterly budget of ${_q2(quarterly):,.2f} matches the official Support at Home "
            f"Classification {cls} quarterly budget (${index_q:,.2f}).",
        ))
    else:
        checks.append(_flag(
            "quarterly_budget_vs_classification", "Quarterly budget vs classification",
            f"The plan states a quarterly budget of ${_q2(quarterly):,.2f}, but the official Support at Home "
            f"Classification {cls} quarterly budget is ${index_q:,.2f}. Ask your provider why they differ.",
        ))

    # 6. AT-HM allocation within the tier cap (only when an amount is stated).
    #    The high tier CAN be exceeded, but only with clinical evidence, so an
    #    over-cap amount is a "confirm this was approved" flag, never a hard fail.
    if at_hm is not None:
        tier = facts.get("at_hm_tier")
        tier_caps = {
            "low": _index1("athm.tier.low.amount_aud"),
            "medium": _index1("athm.tier.medium.amount_aud"),
            "high": _index1("athm.tier.high.amount_aud"),
        }
        high_cap = tier_caps["high"]
        cap = tier_caps.get(tier) if tier else high_cap
        tier_txt = f"{tier} tier " if tier else ""
        if cap is None:
            checks.append(_cannot_run(
                "at_hm_budget", "AT-HM budget vs tier cap",
                "the AT-HM tier reference value",
            ))
        elif _q2(at_hm) <= cap + Decimal("1.00"):
            checks.append(_pass(
                "at_hm_budget", "AT-HM budget vs tier cap",
                f"AT-HM allocation of ${_q2(at_hm):,.2f} is within the {tier_txt}AT-HM cap "
                f"of ${cap:,.2f}.",
            ))
        elif not tier or tier == "high":
            checks.append(_flag(
                "at_hm_budget", "AT-HM budget vs tier cap",
                f"AT-HM allocation of ${_q2(at_hm):,.2f} is above the AT-HM high-tier cap of "
                f"${high_cap:,.2f}. The high tier can be exceeded, but only with clinical evidence "
                f"such as a prescription (Aged Care Rules 2025 section 211). Ask your provider to "
                f"confirm the exceedance was approved.",
            ))
        else:
            checks.append(_flag(
                "at_hm_budget", "AT-HM budget vs tier cap",
                f"AT-HM allocation of ${_q2(at_hm):,.2f} is above the {tier} tier cap of "
                f"${cap:,.2f}. Ask your provider which tier applies and whether the amount is correct.",
            ))

    # 7. Restorative Care Pathway budget within the episode / total caps.
    restorative = facts.get("restorative_budget")
    if restorative is not None:
        ep = _index1("pathway.restorative_care.episode_aud")      # $6,000
        max_total = _index1("pathway.restorative_care.max_total_aud")  # $12,000
        if ep is None or max_total is None:
            checks.append(_cannot_run(
                "restorative_budget", "Restorative Care budget",
                "the Restorative Care Pathway reference budget",
            ))
        elif _q2(restorative) <= ep + Decimal("50.00"):
            checks.append(_pass(
                "restorative_budget", "Restorative Care budget",
                f"Restorative Care budget of ${_q2(restorative):,.2f} is within the ${ep:,.2f} "
                f"episode budget.",
            ))
        elif _q2(restorative) <= max_total + Decimal("50.00"):
            checks.append(_flag(
                "restorative_budget", "Restorative Care budget",
                f"Restorative Care budget of ${_q2(restorative):,.2f} is above the ${ep:,.2f} "
                f"single-episode budget. Up to ${max_total:,.2f} is only allowed across two approved "
                f"episodes. Confirm a second episode or extension was approved.",
            ))
        else:
            checks.append(_flag(
                "restorative_budget", "Restorative Care budget",
                f"Restorative Care budget of ${_q2(restorative):,.2f} exceeds the ${max_total:,.2f} "
                f"maximum across all Restorative Care episodes.",
            ))

    # 8. End-of-Life Pathway budget within the cap.
    eol = facts.get("end_of_life_budget")
    if eol is not None:
        eol_cap = _index1("pathway.end_of_life.episode_aud")      # $25,000
        if eol_cap is None:
            checks.append(_cannot_run(
                "end_of_life_budget", "End-of-Life budget",
                "the End-of-Life Pathway reference budget",
            ))
        elif _q2(eol) <= eol_cap + Decimal("50.00"):
            checks.append(_pass(
                "end_of_life_budget", "End-of-Life budget",
                f"End-of-Life budget of ${_q2(eol):,.2f} is within the ${eol_cap:,.2f} "
                f"End-of-Life Pathway budget.",
            ))
        else:
            checks.append(_flag(
                "end_of_life_budget", "End-of-Life budget",
                f"End-of-Life budget of ${_q2(eol):,.2f} exceeds the ${eol_cap:,.2f} "
                f"End-of-Life Pathway budget.",
            ))

    flagged = sum(1 for c in checks if c["status"] == "flag")
    cannot_run = sum(1 for c in checks if c["status"] == "cannot_run")
    return {
        "checks": checks,
        "flagged_count": flagged,
        "cannot_run_count": cannot_run,
        "classification_quarterly_budget": float(index_q) if index_q is not None else None,
        "all_passing": flagged == 0 and cannot_run == 0,
    }
