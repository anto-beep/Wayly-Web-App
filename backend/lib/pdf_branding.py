...
def make_doc(buf, *, title: str, author: str = "Wayly",
    ...
    # DOC-PARITY-1: deterministic PDF metadata (fixed CreationDate/ModDate/ID)
    # so repeated renders of the same content are byte/hash-identical.
    doc.invariant = True
    return doc
...
def footer_block(styles: dict, generated: Optional[str] = None) -> list:
    today = generated or datetime.now().strftime("%-d %b %Y")
    return [
        Spacer(1, 12),
        Paragraph(
