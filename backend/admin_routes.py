...
from lib.query_helpers import admin_users_with_subscription  # Section 2 hardening
...
async def list_users(
    ...
    # Section 2: single-aggregation pushdown (was 1 + page_size round trips).
    users, total = await admin_users_with_subscription(
...
@admin.put("/users/{user_id}/email-verified")
async def admin_toggle_email_verified(user_id: str, body: dict, admin: dict = Depends(get_current_admin)):
    """Admin "test mode" override, flip email_verified for any user without
    waiting for a fresh email link. Useful when QA reports a bogus signup or
    a real user gets locked out and needs an emergency unlock."""
    from datetime import datetime, timezone
    target = await db.users.find_one({"id": user_id}, {"_id": 0, "id": 1, "email": 1})
    if not target:
        raise HTTPException(404, "User not found")
    verified = bool(body.get("email_verified", False))
    update: dict = {"email_verified": verified}
    if verified:
        update["email_verified_at"] = datetime.now(timezone.utc).isoformat()
        update["admin_verified_by"] = admin["id"]
    else:
        update["email_verified_at"] = None
    await db.users.update_one({"id": user_id}, {"$set": update})
    # Audit trail, capture who flipped the flag for compliance.
    try:
        await db.audit_events.insert_one({
            "id": __import__("uuid").uuid4().hex,
            "actor_id": admin["id"],
            "actor_email": admin.get("email"),
            "action": "admin_toggle_email_verified",
            "target": user_id,
            "target_email": target.get("email"),
            "detail": f"email_verified set to {verified}",
...
@admin.post("/seo/indexnow/changed")
async def indexnow_submit_changed(admin: dict = Depends(get_current_admin)):
    """SEO-2: submit only URLs whose content changed since the last run.

    Diffs the current sitemap dates against the stored manifest (Mongo interim).
    First run stores the manifest without pinging; a datePublished change is
    rejected with 409.
    """
    ...
    await audit_log(
        actor_id=admin.get("id"),
        action="indexnow_submit_changed",
        target_id=f"{result.get('changed_count', len(result.get('changed', [])))} URLs",
        result="success" if result.get("ok") else "error",
        detail={"first_run": result.get("first_run"), "error": result.get("error") or result.get("indexnow_error")},
    )
...
# ---------------------------------------------------------------------------
# Cache observability, Section 3 hardening
# ---------------------------------------------------------------------------

@admin.get("/cache/stats")
async def admin_cache_stats(admin: dict = Depends(get_current_admin)):
    """Snapshot of in-process cache hit/miss/set/error counters per
    namespace. Cheap to call, purely in-memory. Use this from the admin
    console to confirm the Redis layer is actually reducing Mongo load.
    """
    from lib import cache as _cache  # noqa: WPS433 lazy import
    stats = _cache.get_stats()
    totals = {"hit": 0, "miss": 0, "set": 0, "err": 0}
    for ns_stats in stats.values():
        for k, v in ns_stats.items():
            totals[k] = totals.get(k, 0) + v
    hit_rate = (totals["hit"] / max(1, totals["hit"] + totals["miss"])) * 100
    return {
        "redis_url_set": bool(__import__("os").environ.get("REDIS_URL")),
...
async def admin_cache_invalidate(
    ...
    """Manually flush a slice of the cache. Use after admin edits a
    reference row or after a data fix. Super-admin only because cascade
    invalidation can spike Mongo briefly."""
    from lib import cache as _cache  # noqa: WPS433
    if household_id:
        n = await _cache.invalidate_household(household_id)
    elif participant_id:
        n = await _cache.invalidate_participant(participant_id)
    elif user_id:
        n = await _cache.invalidate_user(user_id)
    elif namespace == "ref":
        n = await _cache.invalidate_ref()
    elif namespace:
        n = await _cache.invalidate_pattern(f"wayly:{namespace}:*")
    else:
        raise HTTPException(400, "Provide one of: household_id, participant_id, "
                                  "user_id, namespace")


# ---------------------------------------------------------------------------
# Background-jobs observability, Section 4 hardening
# ---------------------------------------------------------------------------

@admin.get("/jobs/stats")
async def admin_jobs_stats(admin: dict = Depends(get_current_admin)):
    """Per-handler counters (started/done/error/retried) + live queue depth.
    Pair with /admin/jobs/dead-letter to see what's failed irrecoverably."""
    from lib import jobs as _jobs  # noqa: WPS433
    stats = _jobs.get_stats()
    totals = {"started": 0, "done": 0, "error": 0, "retried": 0}
    for h in stats.values():
        for k, v in h.items():
            totals[k] = totals.get(k, 0) + v
    return {
        "queue_depth": await _jobs.queue_depth(),
        "totals": totals,
        "handlers": stats,
    }


@admin.get("/jobs/dead-letter")
async def admin_jobs_dead_letter(
    limit: int = 20,
    admin: dict = Depends(get_current_admin),
):
    """Recent jobs that exhausted their retry budget. Inspect, then either
    fix the upstream cause or manually replay via the registered handler."""
    from lib import jobs as _jobs  # noqa: WPS433
    return {"items": await _jobs.dead_letter_recent(limit=limit)}


# ---------------------------------------------------------------------------
# LLM wrapper observability, Section 5 hardening
# ---------------------------------------------------------------------------

@admin.get("/llm/stats")
async def admin_llm_stats(admin: dict = Depends(get_current_admin)):
    """Per-model req/ok/err/cache_hit/breaker_open counters."""
    from lib import llm_wrapper as _llm  # noqa: WPS433
    stats = _llm.get_stats()
    totals = {"req": 0, "ok": 0, "err": 0, "cache_hit": 0, "breaker_open": 0}
    for m in stats.values():
        for k, v in m.items():
            totals[k] = totals.get(k, 0) + v
    cache_hit_rate = (totals["cache_hit"] / max(1, totals["req"])) * 100
    err_rate = (totals["err"] / max(1, totals["req"])) * 100
    return {
        "totals": totals,
...
@admin.post("/llm/breaker/reset")
async def admin_llm_breaker_reset(
    model: str,
    admin: dict = Depends(require_super_admin),
):
    """Manually close an open circuit-breaker (e.g. after provider outage
    is confirmed resolved). Super-admin only."""
    from lib import llm_wrapper as _llm  # noqa: WPS433
    ok = _llm.reset_breaker(model)
    return {"model": model, "reset": ok, "state": _llm.breaker_state(model)}
