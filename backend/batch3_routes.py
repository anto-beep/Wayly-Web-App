...
def _tc_name(s):
    """Capitalise the first letter of each name part without touching the rest."""
    if not s:
        return s
    return re.sub(r"(^|[\s'\-])([a-z])", lambda m: m.group(1) + m.group(2).upper(), str(s).strip())
...
async def add_v2_participant(payload: ParticipantCreateV2, request: Request):
    ...
    p = ParticipantV2(
        account_id=acct["id"],
        first_name=_tc_name(payload.first_name.strip()),
        last_name=_tc_name((payload.last_name or "").strip()),
        date_of_birth=payload.date_of_birth,
        classification=payload.classification,
        provider_name=(payload.provider_name or "").strip() or None,
        provider_email=(payload.provider_email or "").strip() or None,
