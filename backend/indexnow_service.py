...
def all_sitemap_urls() -> list[str]:
    """Return every URL in the Wayly sitemap (matches /api/public/seo/sitemap.xml)."""
    # Import lazily to avoid circular import (seo_routes imports back into us once we add admin trigger)
    from seo_routes import STATIC_PAGES

    return [f"https://{HOST}{path}" for path, _priority, _changefreq in STATIC_PAGES]
