...
async def apply_reseed_for_authoritative_keys(seed_rows: list[dict]) -> None:
    ...
    for (key, eff_from), row in seed_by_key_eff.items():
        ...
        if existing and all(
            existing.get(k) == row.get(k)
            for k in ("value", "source_url", "notes", "effective_to")
        ):
            continue
...
async def preload_cache() -> None:
    ...
    async for doc in cur:
        ...
        if not key or eff_from is None or "value" not in doc:
            skipped_missing_id += 1
            continue
        doc_id = doc.get("id")
        if not doc_id:
            # Resilience: a legacy row without a stable `id` should not poison
            # the whole cache. Skip it (matches the dec-2026 hardening note).
            log.warning("program_reference row missing 'id' (key=%s); skipping", key)
            skipped_missing_id += 1
            continue
    ...
    _CACHE = fresh
    _CACHE_READY = True
    log.info("program_reference cache loaded: %d keys, %d rows total (skipped %d malformed)",
             len(_CACHE), sum(len(v) for v in _CACHE.values()), skipped_missing_id)
