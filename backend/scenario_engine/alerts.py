...
PUSH_TITLE = "There is an update on a participant"
...
def _month_iso_to_display(month_iso: str) -> str:
    """UI-2 Rule 2.6, render "2026-05" as "May 2026" in every user-facing
    notification body. Returns the raw string on invalid input so we never
    lose data in an alert."""
    try:
        y, m = month_iso.split("-", 1)
        idx = int(m) - 1
        if 0 <= idx < 12:
...
async def _clock_statement_overdue(db, p: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    return [{
        ...
        "title": "Last month's statement has not arrived",
        "body": f"We have not recorded a statement for {_month_iso_to_display(month_iso)}. "
                "Providers must issue it by the last day of the following month. "
                "Ask the provider for a copy if it has not been emailed yet.",
...
async def _clock_budget_exhaustion_projected(db, p: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    burn rate is meaningful, and (b) ≥10% safety margin so we do not fire
...
async def _clock_means_not_disclosed(db, p: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    return [{
        ...
        "title": "Means assessment is not on file",
        "body": "Without an assessment, the system uses the highest contribution rate. "
                ...
                "We cannot recommend a figure, Services Australia is the right place to start.",
...
async def _clock_policy_gates(db, p: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    gates = [
        ...
        ("policy_date.eol_second_round_start", "policy_eol_round2_2027",
         ...
         "A second round of EoL funding becomes available. If the first 12 weeks are not enough, "
