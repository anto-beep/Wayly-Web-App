...
class CE2Output:
    ...
    # Per-category dollar breakdown of the classification budget (spec parity
    # with the My Aged Care fee estimator's "budget summary by service category").
...
def _finalise(registry, input_data: CE2Input, resolution: EntryPathResolution, rate_lookup: Dict[str, float]) -> CE2Output:
    ...
    # Per-category dollar breakdown (budget / government pays / you pay), the
    # "budget summary by service category" the My Aged Care estimator shows.
    clinical_share_annual = base_annual * (input_data.service_mix.clinical / 100.0)
    ind_contribution_annual = ind_share_annual * (rate_lookup["independence_rate_pct"] / 100.0)

    def _cat(key, label, rate_pct, budget, you):
        return {
            "key": key, "label": label, "rate_pct": _round(rate_pct, 2),
            "budget_annual": _round(budget, 2), "budget_quarterly": _round(budget / QUARTERS_PER_YEAR, 2),
            "you_annual": _round(you, 2), "you_quarterly": _round(you / QUARTERS_PER_YEAR, 2),
            "govt_annual": _round(budget - you, 2), "govt_quarterly": _round((budget - you) / QUARTERS_PER_YEAR, 2),
        }

    category_breakdown = [
        _cat("clinical", "Clinical care", 0.0, clinical_share_annual, 0.0),
        _cat("independence", "Independence", rate_lookup["independence_rate_pct"], ind_share_annual, ind_contribution_annual),
        _cat("everyday", "Everyday living", rate_lookup["everyday_rate_pct"], ev_share_annual, ev_contribution_annual),
    ]
