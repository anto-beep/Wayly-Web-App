...
_NAME_LABEL_WORDS = {"name", "participant", "client", "for", "mr", "mrs", "ms", "dr", "the", "surname", "given"}


def _first_name_from(text: str) -> Optional[str]:
    """Extract the participant's first name from an unredacted plan.

    Handles 'Participant Name: Sam Burke', 'Name: Sam Burke', 'Client: Sam',
    'Prepared for: Sam' and 'For: Sam'. Never returns a label word (e.g. the
    literal 'Name'), which a naive 'Participant:\\s*(Word)' regex would wrongly
    capture from a 'Participant Name:' row."""
    patterns = [
        r"Participant(?:\s+Name)?\s*:?\s*([A-Z][a-z]+)",
        r"Client(?:\s+Name)?\s*:?\s*([A-Z][a-z]+)",
        r"(?:Prepared\s+for|Care\s+plan\s+for|Plan\s+for|For)\s*:?\s*([A-Z][a-z]+)",
        r"(?m)^\s*Name\s*:?\s*([A-Z][a-z]+)",
    ]
    for pat in patterns:
        for m in re.finditer(pat, text):
            cand = (m.group(1) or "").strip()
            if cand and cand.lower() not in _NAME_LABEL_WORDS:
                return cand
    return None
...
def _extract_goals(text: str) -> List[str]:
    """Best-effort pull of goal statements from a plan's GOALS section.
    Deterministic. Captures the block under a GOALS-style heading up to the
    next all-caps heading, then returns cleaned individual goal lines."""
    if not text:
        return []
    m = re.search(
        r"(?im)^\s*(?:MY\s+GOALS|PARTICIPANT\s+GOALS|GOALS\s+AND\s+ASPIRATIONS|GOALS\s+&\s+ASPIRATIONS|GOALS|ASPIRATIONS|WHAT\s+MATTERS\s+TO\s+ME)\s*[:\n]([\s\S]*?)(?=^\s*[A-Z][A-Z \t&/]{3,}\s*$|\Z)",
        text,
    )
    if not m:
        return []
    ...
    for l in raw_lines:
        ...
        cleaned = re.sub(r"^\s*(?:[-*•·▪◦]|\d+[.)]|[a-z][.)])\s*", "", l).strip()
        low = cleaned.lower()
        if len(cleaned) < 8:
            continue
        if any(h in low for h in ("services delivered", "narrative", "signed", "authorised", "reviewed", "prepared")):
            break
        # skip lines that are clearly not goals (dates, money, headings)
