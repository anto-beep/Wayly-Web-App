"""Iteration 122 focused Playwright verification for participant profile switcher bug.

Preview: https://wayly-engagement-hub.preview.emergentagent.com
Credentials: cathy@example.com / testpass123

Focus:
- Exact reported bug: participant switcher selection must match profile data and URL.
- Regression: direct participant URLs still win over persisted localStorage.
- Regression: Profile sidebar item highlights on canonical participant profile and /app/me redirect.
- Focused cascade smoke: participant-scoped pages/API requests use the newly active participant.

This file is intended to be executed by the MCP browser automation runner, which
injects an async Playwright `page` object into the script context.
"""

import json

BASE = "https://wayly-engagement-hub.preview.emergentagent.com"
...
async def nav_profile_class():
    await page.wait_for_selector('a[data-testid="nav-profile"]', timeout=30000)
    return await page.locator('a[data-testid="nav-profile"]').get_attribute("class")
...
async def switch_to(pid, expected_name):
    ...
    record(active_id == pid and expected_name.lower() in text.lower(), f"switcher chip reflects {expected_name}", f"active_id={active_id}, pathname={pathname}, trigger='{text}'")
    return {"active_id": active_id, "trigger": text, "pathname": pathname}


try:
    await page.set_viewport_size({"width": 1920, "height": 1080})
    print("Test plan: login as Cathy; load Dorothy to persist active; direct-nav to Robert and verify URL/chip/H1/Profile highlight; switcher-select Dorothy and verify pathname/localStorage/chip/H1; verify direct URL still wins; verify /app/me/Profile nav; then run focused ATHM/CSC/Ask Wayly/LF-2/BC-2 participant cascade checks.")
    ...
    # Direct/bookmarked Robert URL must win over persisted Dorothy.
    ...
    pathname = await page.evaluate("() => window.location.pathname")
    profile_class = await nav_profile_class()
    ...
    record("bg-primary-k" in (profile_class or "") and "text-white" in (profile_class or ""), "Profile nav item highlighted on canonical participant URL", f"class={profile_class}")

    # Primary Iter 122 retest: while on Robert profile, selecting Dorothy updates URL and body.
    ...
    switched_active = await page.evaluate("() => localStorage.getItem('wayly_active_participant_id')")
    switched_pathname = await page.evaluate("() => window.location.pathname")
    switched_profile_class = await nav_profile_class()
    record(switched_pathname == f"/app/participants/{DOROTHY_ID}" and switched_active == DOROTHY_ID and h1_matches_name(switched_h1, dorothy_names) and "Dorothy" in switched_trigger, "switcher on profile navigates and rerenders Dorothy profile", f"url={page.url}, pathname={switched_pathname}, active={switched_active}, trigger='{switched_trigger}', h1='{switched_h1}'")
    record("bg-primary-k" in (switched_profile_class or "") and "text-white" in (switched_profile_class or ""), "Profile nav remains highlighted after switcher participant change", f"class={switched_profile_class}")

    # Direct URL wins still works after Dorothy was persisted.
    await page.goto(f"{BASE}/app/participants/{ROBERT_ID}", wait_until="domcontentloaded")
    await page.wait_for_selector('[data-testid="participant-switcher-trigger"]', timeout=30000)
    await page.wait_for_timeout(2500)
    direct_again_h1 = await h1_text()
    direct_again_trigger = await trigger_text()
    direct_again_active = await page.evaluate("() => localStorage.getItem('wayly_active_participant_id')")
    direct_again_pathname = await page.evaluate("() => window.location.pathname")
    record(direct_again_pathname == f"/app/participants/{ROBERT_ID}" and direct_again_active == ROBERT_ID and "Robert" in direct_again_trigger and h1_matches_name(direct_again_h1, robert_names), "direct Robert URL still wins with Dorothy persisted", f"url={page.url}, active={direct_again_active}, trigger='{direct_again_trigger}', h1='{direct_again_h1}'")

    # /app/me should redirect to active participant and keep Profile nav highlighted.
    ...
    me_h1 = await h1_text()
    me_class = await nav_profile_class()
    record(ROBERT_ID in page.url and h1_matches_name(me_h1, robert_names), "/app/me redirects to active Robert", f"url={page.url}, h1='{me_h1}'")
    record("bg-primary-k" in (me_class or "") and "text-white" in (me_class or ""), "Profile nav highlighted after /app/me active redirect", f"class={me_class}")

    # Focused cascade regressions from iter 117.
    before = len(requests)
