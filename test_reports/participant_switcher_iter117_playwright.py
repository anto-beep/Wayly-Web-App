...
try:
    ...
    print("Test plan: login as Cathy, identify Dorothy/Robert ids, switch in header, then verify cascade via visible UI state and network request participant_id/path/header on ATHM, CSC, Ask Wayly, LF2, dashboard/statements/care-plans, and reassessment letter.")
    ...
    await switch_to(dorothy, "Dorothy")
    # Re-open final step after remount caused by switch.
    for _ in range(10):
        if await page.locator('[data-testid="lf2-chain-generate-hardship_full"]').count() > 0:
            break
        await page.locator('[data-testid="hardship-next-btn"]').click(force=True)
    ...
    # Reassessment Letter direct route regression.
    await switch_to(robert, "Robert")
    ...
    if rl_count == 0:
        record(False, "Reassessment Letter direct /ai-tools/reassessment-letter exposes participant_name form", f"current_url={current_url}, rl-participant count={rl_count}")
    else:
        robert_prefill = await page.locator('[data-testid="rl-participant"]').input_value()
        await switch_to(dorothy, "Dorothy")
        await page.wait_for_timeout(1000)
        dorothy_prefill = await page.locator('[data-testid="rl-participant"]').input_value()
        record(robert_name in robert_prefill and dorothy_name in dorothy_prefill,
               "Reassessment Letter participant_name prefill follows active participant switch",
