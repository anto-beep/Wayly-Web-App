        ...
        <ScrollView ref={scrollRef} contentContainerStyle={{ padding: spacing.lg, paddingBottom: spacing.xxl, gap: spacing.md }} keyboardShouldPersistTaps="handled">
          ...
          {result ? (
            <View testID="pc-result" onLayout={onResultLayout} style={{ gap: spacing.md }}>
              {showGuard ? (
                <View testID="pc-quality-guard" style={{ borderWidth: 2, borderColor: colors.terracotta, borderRadius: radius.lg, backgroundColor: colors.surface, overflow: "hidden" }}>
                  <View style={{ flexDirection: "row", height: 6 }}>
                    <View style={{ flex: 1, backgroundColor: colors.terracotta }} />
                  ...
                  <View style={{ flexDirection: "row", gap: spacing.sm, padding: spacing.md }}>
                    ...
                    <View style={{ flex: 1 }}>
                      <T style={{ fontFamily: fonts.bodySemi, fontSize: 11, letterSpacing: 0.8, color: colors.terracotta, textTransform: "uppercase" }}>Quick sanity check</T>
                      <T style={{ marginTop: 4, lineHeight: 21, fontSize: 15, fontFamily: fonts.bodyMedium, color: colors.text }}>{guard.prompt}</T>
                      <View style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.sm, marginTop: spacing.md }}>
                        {guard.after_hours_toggle_available ? (
                          <Button label="Yes, this was after-hours" testID="pc-guard-after-hours" onPress={() => { setAfterHours(true); setContinueAnyway(false); submit(); }} style={{ paddingHorizontal: spacing.md, minHeight: 44, backgroundColor: colors.sage }} />
                        ) : null}
                        {guard.allow_continue ? (
                          <Button label="Continue anyway" testID="pc-guard-continue" onPress={() => setContinueAnyway(true)} style={{ paddingHorizontal: spacing.md, minHeight: 44, backgroundColor: colors.primary }} />
                        ) : null}
                      </View>
                    </View>
                  </View>
                ...
                <>
                  <Card testID="pc-how-this-compares">
                    ...
                    <CompareBars result={result} colors={colors} />
                  ...
                  <View style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }}>
                    <StatCard label="You are charged" value={money(result.charged)} sub={UNIT_WORD[result.unit] || `per ${result.unit}`} colors={colors} testID="pc-stat-charged" tone="teal" />
                    <ShareCard share={result.your_share} unit={result.unit} colors={colors} />
                    {result.lower != null && result.upper != null ? (
                      <StatCard label="Indicative range" value={`${money(result.lower)} to ${money(result.upper)}`} sub={`DoH ${snapshotId || ""}`} colors={colors} testID="pc-range" small tone="sage" />
                    ) : (
                      <StatCard label="Indicative range" value="Not published" sub="for this service" colors={colors} testID="pc-range-unavailable" small tone="sage" />
                    )}
                  </View>
...
// Side-by-side comparison bars: what you pay vs the typical median and usual
// range. Mirrors the web pc-compare-bars graphic.
function CompareBars({ result, colors }: any) {
  const chg = Number(result.charged) || 0;
  const med = Number(result.median) || 0;
  const lo = Number(result.lower) || 0;
  const hi = Number(result.upper) || 0;
  if (!chg && !med) return null;
  const domain = Math.max(chg, med, hi) * 1.1 || 1;
  const pctOf = (v: number) => Math.min(100, Math.max(3, (v / domain) * 100));
  const overRange = hi > 0 && chg > hi;
  const chgColor = overRange ? colors.terracotta : "#425F47";
  const u = result.unit ? `/${result.unit}` : "";
  const diffPct = med > 0 ? Math.round(((chg - med) / med) * 100) : null;

  const Bar = ({ label, value, color, testID }: any) => (
    <View style={{ marginTop: spacing.sm }} testID={testID}>
      <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 3 }}>
        <T variant="small" style={{ color: colors.muted, fontSize: 11 }}>{label}</T>
        <T style={{ fontFamily: fonts.mono, fontSize: 12, color: colors.text }}>{money(value)}{u}</T>
      </View>
      <View style={{ height: 24, borderRadius: 8, backgroundColor: colors.surface2, overflow: "hidden" }}>
        <View style={{ width: `${pctOf(value)}%`, height: "100%", backgroundColor: color, borderRadius: 8 }} />
      </View>
    </View>
  );

  return (
    <View testID="pc-compare-bars" style={{ marginTop: spacing.md, borderWidth: 1, borderColor: colors.border, borderRadius: radius.md, backgroundColor: colors.surface2, padding: spacing.md }}>
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 6 }}>
        <T variant="small" style={{ color: colors.muted, letterSpacing: 0.4, fontSize: 11 }}>WHAT YOUR PROVIDER CHARGES VS TYPICAL</T>
        <View style={{ backgroundColor: overRange ? colors.terracotta : "#425F47", borderRadius: radius.pill, paddingHorizontal: 10, paddingVertical: 3 }}>
          <T style={{ color: "#fff", fontFamily: fonts.bodySemi, fontSize: 11 }}>{overRange ? "Above the range" : "Within the range"}</T>
        </View>
      </View>
      {med > 0 && diffPct !== null ? (
        <T variant="small" style={{ marginTop: spacing.sm, lineHeight: 20, color: colors.text }} testID="pc-compare-plain">
          You pay <T style={{ fontFamily: fonts.bodySemi }}>{money(chg)}{u}</T>. That is{" "}
          <T style={{ fontFamily: fonts.bodySemi, color: chgColor }}>{Math.abs(diffPct)}% {diffPct >= 0 ? "more than" : "less than"}</T>{" "}
          the typical rate of {money(med)}{u}.
        </T>
      ) : null}
      {chg > 0 ? <Bar label="You pay" value={chg} color={chgColor} testID="pc-charge-pin" /> : null}
      {med > 0 ? <Bar label="Typical median" value={med} color="#0E4D52" /> : null}
      {hi > lo ? (
        <View style={{ marginTop: spacing.sm }}>
          <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 3 }}>
            <T variant="small" style={{ color: colors.muted, fontSize: 11 }}>Usual range</T>
            <T style={{ fontFamily: fonts.mono, fontSize: 12, color: colors.text }}>{money(lo)}–{money(hi)}{u}</T>
          </View>
          <View style={{ height: 24, borderRadius: 8, backgroundColor: colors.surface2, borderWidth: 1, borderColor: colors.border }}>
            <View style={{ position: "absolute", top: 0, bottom: 0, left: `${pctOf(lo)}%`, width: `${Math.max(6, pctOf(hi) - pctOf(lo))}%`, backgroundColor: "rgba(66,95,71,0.28)", borderWidth: 1, borderColor: "rgba(66,95,71,0.5)", borderRadius: 8 }} />
          </View>
        </View>
      ) : null}
    </View>
...
function SnapshotSelector({ snapshots, selected, onChange, colors }: any) {  if (!snapshots || snapshots.length === 0) return null;
...
function SaveResultButton({ result, provider, snapshotId, ceState, colors }: any) {
  ...
  return (
    <View>
      <Button label="Save this result" testID="pc-save-check" variant="primary" icon={Save} onPress={() => post(null)} loading={busy} style={{ backgroundColor: "#0E4D52" }} />
      {error ? <T variant="small" style={{ color: colors.terracotta, marginTop: 6 }}>{error}</T> : null}
    </View>
  );
}

function StatCard({ label, value, sub, colors, testID, small, tone }: any) {
  const toneMap: any = {
    teal: { accent: colors.primary, bg: colors.primarySoft },
    sage: { accent: colors.sage, bg: colors.sageSoft },
    clay: { accent: colors.gold, bg: colors.goldSoft },
  };
  const t = toneMap[tone] || { accent: colors.border, bg: colors.surface };
  return (
    <View testID={testID} style={{ minWidth: 150, flexGrow: 1, borderWidth: 1, borderColor: colors.border, borderLeftWidth: 4, borderLeftColor: t.accent, borderRadius: radius.md, backgroundColor: t.bg, padding: spacing.md }}>
...
function ShareCard({ share, unit, colors }: any) {
  ...
  return (
    <View testID="pc-your-share" style={{ minWidth: 150, flexGrow: 1, borderWidth: 1, borderColor: colors.border, borderLeftWidth: 4, borderLeftColor: colors.gold, borderRadius: radius.md, backgroundColor: colors.goldSoft, padding: spacing.md }}>
  ...
  return (
    <View style={{ gap: spacing.sm }}>
      <View style={{ flexDirection: "row", gap: spacing.sm, flexWrap: "wrap" }}>
        <Button label="Download PDF" variant="primary" icon={Download} testID="pc-pdf-export" loading={busyPdf} onPress={downloadPdf} style={{ flexGrow: 1, minWidth: "45%", backgroundColor: "#A5512B" }} />
        <Button label="Email the provider" variant="primary" icon={Mail} testID="pc-open-email" onPress={() => setEmailOpen(true)} style={{ flexGrow: 1, minWidth: "45%", backgroundColor: "#425F47" }} />
      </View>
