...
import { Sparkles, AlertTriangle, RefreshCcw, ArrowRight, Phone, Download, Mail, ChevronDown, ChevronUp, CheckCircle2, FolderOpen, BookmarkPlus, Trash2, Info } from "lucide-react-native";
...
export default function ClassificationSelfCheck() {
  ...
  const reopenCheck = (check: any) => {
    ...
    setResult(null); setResumed(false); setError("");
        ...
        <ScrollView ref={scrollRef} contentContainerStyle={{ padding: spacing.lg, paddingBottom: spacing.xxl, gap: spacing.md }} keyboardShouldPersistTaps="handled">
          ...
          {!result ? (
            <>
              ...
              {!allDone ? (
                <View testID="csc-cta-help" style={{ flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, backgroundColor: colors.goldSoft, borderWidth: 1, borderColor: colors.gold, borderRadius: radius.md, paddingHorizontal: spacing.md, paddingVertical: 12 }}>
                  <Info size={16} color={colors.gold} />
                  <T variant="small" style={{ color: colors.text, fontFamily: fonts.bodyMedium, textAlign: "center" }}>
                    {answered === 0 ? "Answer all 16 questions to see your result." : `Almost there — ${total - answered} question${total - answered === 1 ? "" : "s"} still to answer.`}
                  </T>
                </View>
              ) : null}
            ...
            <View testID="csc-result" onLayout={onResultLayout} style={{ gap: spacing.md }}>
              {/* Profile header — teal, white text, big band + clear budgets */}
              <View testID="csc-result-header" style={{ backgroundColor: colors.primary, borderRadius: radius.lg, padding: spacing.lg }}>
                ...
                <View style={{ flexDirection: "row", gap: spacing.sm, marginTop: spacing.md }}>
                  <View style={{ flex: 1, backgroundColor: "rgba(255,255,255,0.12)", borderRadius: radius.md, padding: spacing.md }}>
                    <T style={{ fontFamily: fonts.bodySemi, fontSize: 10, letterSpacing: 1, color: "rgba(255,255,255,0.6)" }}>PER YEAR</T>
                    <T style={{ fontFamily: fonts.headingSemi, fontSize: 18, color: "#fff", marginTop: 2 }}>{`${moneyWhole(c.annual_budget_low)} to ${moneyWhole(c.annual_budget_high)}`}</T>
                  </View>
                  <View style={{ flex: 1, backgroundColor: "rgba(255,255,255,0.12)", borderRadius: radius.md, padding: spacing.md }}>
                    <T style={{ fontFamily: fonts.bodySemi, fontSize: 10, letterSpacing: 1, color: "rgba(255,255,255,0.6)" }}>PER QUARTER</T>
                    <T style={{ fontFamily: fonts.headingSemi, fontSize: 18, color: "#fff", marginTop: 2 }}>{`${moneyWhole(c.quarterly_budget_low)} to ${moneyWhole(c.quarterly_budget_high)}`}</T>
                  </View>
                </View>
              </View>
