        ...
        <ScrollView ref={scrollRef} contentContainerStyle={{ padding: spacing.lg, paddingBottom: spacing.xxl, gap: spacing.md }} keyboardShouldPersistTaps="handled">
          ...
          {!result ? (
            ...
            <View testID="ce-result" onLayout={onResultLayout} style={{ gap: spacing.md }}>
              ...
              <ResultActions mode="payload" tool="contribution" payload={result} personName={form.person_name || undefined} fileBaseName="wayly-contribution-estimate" testIDPrefix="ce-export" />
            </View>
          )}
...
function ResultScreen({ result, form, colors, onEdit }: any) {
  ...
  return (
    <>
      ...
      {/* Budget summary by service category (dollar breakdown) */}
      {!result.range_mode && !result.is_fee_exempt && (result.category_breakdown?.length > 0) ? (
        <BudgetBreakdown result={result} colors={colors} />
      ) : null}
...
const CE_CAT: Record<string, { bar: string; soft: string; label: string; note: string }> = {
...
function BudgetBreakdown({ result, colors }: any) {
  const [period, setPeriod] = useState<"quarterly" | "monthly">("quarterly");
  const rows = result.category_breakdown || [];
  const pick = (r: any, f: string) => (period === "monthly" ? (r[`${f}_annual`] || 0) / 12 : r[`${f}_quarterly`] || 0);
  const totalBudget = rows.reduce((s: number, r: any) => s + pick(r, "budget"), 0);
  const totalYou = rows.reduce((s: number, r: any) => s + pick(r, "you"), 0);
  const totalGovt = rows.reduce((s: number, r: any) => s + pick(r, "govt"), 0);
  const per = period === "monthly" ? "a month" : "a quarter";
  return (
    <Card testID="ce-budget-breakdown">
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", gap: spacing.sm }}>
        <View style={{ flex: 1 }}>
          <T variant="small" style={{ color: colors.muted, letterSpacing: 0.5 }}>WHERE YOUR BUDGET GOES</T>
          <T variant="small" style={{ color: colors.muted, marginTop: 2, lineHeight: 18 }}>How the budget splits, and who pays for each part.</T>
        </View>
        <View style={{ flexDirection: "row", backgroundColor: colors.bg, borderRadius: radius.pill, padding: 2, borderWidth: 1, borderColor: colors.border }} testID="ce-budget-period">
          {(["quarterly", "monthly"] as const).map((p) => (
            <Pressable key={p} testID={`ce-budget-period-${p}`} onPress={() => setPeriod(p)} style={{ paddingHorizontal: 12, paddingVertical: 6, borderRadius: radius.pill, backgroundColor: period === p ? colors.primary : "transparent" }}>
              <T style={{ fontSize: 12, fontFamily: fonts.bodySemi, color: period === p ? "#FFFFFF" : colors.primary, textTransform: "capitalize" }}>{p}</T>
            </Pressable>
          ))}
        </View>
      </View>
      ...
      <View style={{ gap: spacing.sm, marginTop: spacing.md }}>
        {rows.map((r: any) => {
          const tone = CE_CAT[r.key] || CE_CAT.independence;
          const budget = pick(r, "budget");
          const you = pick(r, "you");
          const govt = pick(r, "govt");
          const youPct = budget > 0 ? Math.min(100, (you / budget) * 100) : 0;
          return (
            <View key={r.key} style={{ backgroundColor: tone.soft, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, padding: spacing.md }} testID={`ce-budget-row-${r.key}`}>
              <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start" }}>
                <View style={{ flex: 1 }}>
                  <T style={{ fontFamily: fonts.bodySemi, fontSize: 15, color: colors.primary }}>{tone.label}</T>
                  <T variant="small" style={{ color: colors.muted }}>{tone.note}</T>
                </View>
                <View style={{ alignItems: "flex-end" }}>
                  <T style={{ fontFamily: fonts.heading, fontSize: 17, color: colors.primary }}>{money(budget)}</T>
                  <T style={{ fontSize: 10, color: colors.muted }}>in your budget {per}</T>
                </View>
              </View>
              <View style={{ height: 9, borderRadius: 5, overflow: "hidden", flexDirection: "row", marginTop: spacing.sm, backgroundColor: "rgba(66,95,71,0.20)" }}>
                <View style={{ width: `${youPct}%`, backgroundColor: tone.bar }} />
              </View>
              <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 6 }}>
                <T style={{ fontSize: 12, color: colors.primary }}><T style={{ fontFamily: fonts.bodySemi, fontSize: 12, color: colors.primary }}>{money(you)}</T> you {r.rate_pct > 0 ? `(${r.rate_pct}%)` : "(free)"}</T>
                <T style={{ fontSize: 12, color: colors.muted }}><T style={{ fontFamily: fonts.bodySemi, fontSize: 12, color: colors.muted }}>{money(govt)}</T> government</T>
              </View>
            </View>
          );
        })}
      ...
      <View style={{ backgroundColor: colors.primary, borderRadius: radius.md, padding: spacing.md, flexDirection: "row", justifyContent: "space-between", marginTop: spacing.md }} testID="ce-budget-totals">
        <View>
          <T style={{ fontSize: 10, letterSpacing: 0.5, color: "rgba(255,255,255,0.7)" }}>TOTAL</T>
          <T style={{ fontFamily: fonts.heading, fontSize: 16, color: "#FFFFFF" }} testID="ce-budget-total">{money(totalBudget)}</T>
        </View>
        <View>
          <T style={{ fontSize: 10, letterSpacing: 0.5, color: "rgba(255,255,255,0.7)" }}>YOU PAY</T>
          <T style={{ fontFamily: fonts.heading, fontSize: 16, color: "#FFFFFF" }} testID="ce-budget-you">{money(totalYou)}</T>
        </View>
        <View>
          <T style={{ fontSize: 10, letterSpacing: 0.5, color: "rgba(255,255,255,0.7)" }}>GOVERNMENT</T>
          <T style={{ fontFamily: fonts.heading, fontSize: 16, color: "#FFFFFF" }} testID="ce-budget-govt">{money(totalGovt)}</T>
        </View>
      </View>
      <T variant="small" style={{ color: colors.muted, marginTop: spacing.sm, lineHeight: 18 }}>
        Based on the classification budget split across a sample service mix. Providers set actual prices, so real amounts may differ. Clinical care is always fully funded.
      </T>
    </Card>
  );
}
