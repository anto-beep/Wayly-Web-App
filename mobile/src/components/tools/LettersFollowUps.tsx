...
import { FileText, MessageSquare, AlertTriangle, ShieldAlert, Clock, ChevronRight, Info, Phone, ArrowRight, Mail, PenLine } from "lucide-react-native";
...
// The 12 situations grouped into six plain-English families, each with a
...
const BLANK_TYPES: { label: string; situationId: number; icon: any }[] = [
...
function accentColor(colors: any, key: string): string {
  return ({ teal: colors.primary, gold: colors.gold, terracotta: colors.terracotta, sage: colors.sage } as Record<string, string>)[key] || colors.primary;
}
      ...
      {loading ? <Loading label="Loading…" /> : (
        <ScrollView contentContainerStyle={{ padding: spacing.lg, paddingBottom: spacing.xxl, gap: spacing.md }}>
          ...
          <Pressable testID="lf1-open-log" onPress={() => router.push("/letters")} style={{ flexDirection: "row", alignItems: "center", gap: 5 }}>
            ...
            <T variant="small" style={{ color: colors.primary, fontFamily: fonts.bodySemi }}>Your mailbox</T>
          </Pressable>
          ...
          {/* Start a blank letter — not limited to the situations below. */}
          <View testID="lf1-blank-letter-panel" style={[styles.blankPanel, { borderColor: colors.primary, backgroundColor: colors.primarySoft }]}>
            <View style={{ flexDirection: "row", gap: spacing.sm, alignItems: "flex-start" }}>
              <View style={{ width: 36, height: 36, borderRadius: radius.pill, backgroundColor: colors.primary, alignItems: "center", justifyContent: "center" }}>
                <PenLine size={17} color="#fff" />
              </View>
              <View style={{ flex: 1 }}>
                <T style={{ fontFamily: fonts.headingSemi, fontSize: 19 }}>Start a blank letter</T>
                <T variant="small" style={{ color: colors.muted, marginTop: 2, lineHeight: 19 }}>Know what you need? Pick a letter type and write it your way — you choose the recipient and the details.</T>
              </View>
            </View>
            <View style={styles.blankGrid}>
              {BLANK_TYPES.map((t) => {
                const sit = situations.find((s) => s.id === t.situationId);
                const Icon = t.icon;
                return (
                  <Pressable key={t.label} testID={`lf1-blank-type-${t.situationId}`} disabled={!sit || !!busyId} onPress={() => sit && begin(sit)}
                    style={[styles.blankBtn, { borderColor: colors.border, backgroundColor: colors.surface, opacity: !sit || busyId ? 0.6 : 1 }]}>
                    <Icon size={15} color={colors.primary} />
                    <T variant="small" style={{ flex: 1, color: colors.text }}>{t.label}</T>
          ...
          <T variant="small" style={{ color: colors.muted, letterSpacing: 0.5, marginTop: spacing.sm }}>OR PICK THE SITUATION THAT BEST MATCHES YOURS</T>
          <View testID="lf1-situation-grid" style={{ gap: spacing.lg }}>
            {GROUPS.map((group) => {
              const groupSits = situations.filter((s) => group.archetypes.includes(s.archetype));
              if (!groupSits.length) return null;
              const accent = accentColor(colors, group.accent);
              return (
                <View key={group.key} testID={`lf1-group-${group.key}`} style={{ gap: spacing.sm }}>
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                    <View style={{ width: 10, height: 10, borderRadius: 5, backgroundColor: accent }} />
                    <T style={{ fontFamily: fonts.headingSemi, fontSize: 18 }}>{group.title}</T>
                  </View>
                  <T variant="small" style={{ color: colors.muted, marginLeft: 18, marginTop: -2, lineHeight: 19 }}>{group.desc}</T>
                  {groupSits.map((s) => {
                    const Icon = CARD_ICON[s.archetype] || FileText;
                    const elder = s.id === 11;
                    const bz = busyId === s.id;
                    return (
                      <Pressable key={s.id} testID={`lf1-situation-${s.id}`} disabled={bz} onPress={() => begin(s)}
                        style={[styles.sitCard, { borderColor: elder ? colors.gold : colors.border, backgroundColor: elder ? colors.goldSoft : colors.surface }, bz && { opacity: 0.6 }]}>
                        <View style={{ width: 34, height: 34, borderRadius: radius.md, backgroundColor: accent + "22", alignItems: "center", justifyContent: "center" }}>
                          <Icon size={18} color={elder ? colors.gold : accent} />
                        </View>
                        <View style={{ flex: 1 }}>
...
const styles = StyleSheet.create({
  ...
  blankPanel: { borderWidth: 1, borderRadius: radius.lg, padding: spacing.md, gap: spacing.md },
  blankGrid: { gap: spacing.sm },
  blankBtn: { flexDirection: "row", alignItems: "center", gap: spacing.sm, borderWidth: 1, borderRadius: radius.md, paddingHorizontal: spacing.md, paddingVertical: 12 },
