...
export default function CarePlanReviewer() {
  ...
  // Auto-save a reviewed plan into the Care Plans register (parity with web),
  // so the user doesn't have to remember to tap Save. Runs once per review.
  const autoSaveRef = useRef(false);
  useEffect(() => {
    if (!result || guard || savedPlanId || saving || autoSaveRef.current) return;
