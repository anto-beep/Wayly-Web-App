...
function IdentityNotice({ warn }: { warn: any }) {
  const { colors } = useTheme();
  return (
    <View testID="inv1-identity-warning" style={{ flexDirection: "row", gap: 10, borderWidth: 2, borderColor: colors.terracotta, backgroundColor: colors.errorSoft, borderRadius: radius.md, padding: spacing.md }}>
      <AlertTriangle size={18} color={colors.terracotta} style={{ marginTop: 1 }} />
      <View style={{ flex: 1 }}>
        <T style={{ fontFamily: fonts.bodySemi, fontSize: 13, color: colors.terracotta }}>{warn.title}</T>
        <T style={{ fontSize: 12.5, color: colors.text, marginTop: 3, lineHeight: 18 }}>{warn.message}</T>
      </View>
    </View>
  );
}
...
function DuplicateNotice({ dup }: { dup: any }) {
  ...
  return (
    <View testID="inv1-duplicate-warning" style={{ flexDirection: "row", gap: 10, backgroundColor: colors.alertSoft, borderRadius: radius.md, padding: spacing.md }}>
      <AlertTriangle size={18} color={colors.alert} style={{ marginTop: 1 }} />
      <View style={{ flex: 1 }}>
        <T style={{ fontFamily: fonts.bodySemi, fontSize: 13, color: colors.text }}>{dup.message}</T>
        <T style={{ fontSize: 12, color: colors.muted, marginTop: 2, lineHeight: 17 }}>
          We&apos;ve still checked this one — but you may have already looked at{dup.provider_name ? ` ${dup.provider_name}` : " this provider"}{dup.invoice_date ? ` dated ${dup.invoice_date}` : ""}.
        </T>
        {dup.invoice_id ? (
          <Pressable testID="inv1-duplicate-view" onPress={() => router.push(`/invoices/${dup.invoice_id}` as any)} hitSlop={6}>
            <T style={{ fontFamily: fonts.bodySemi, fontSize: 12, color: colors.primary, marginTop: 4, textDecorationLine: "underline" }}>View the earlier check</T>
          </Pressable>
        ) : null}
      </View>
    </View>
...
export function WaylySummaryCard({ summary }: { summary?: string }) {
  ...
  return (
    <View testID="inv1-summary" style={{ borderRadius: radius.lg, borderWidth: 1, borderColor: "rgba(255,255,255,0.15)", borderLeftWidth: 4, borderLeftColor: "#F0B267", backgroundColor: "#0E4D52", padding: spacing.lg }}>
      <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 8 }}>
        <View style={{ width: 32, height: 32, borderRadius: radius.pill, backgroundColor: "rgba(255,255,255,0.15)", alignItems: "center", justifyContent: "center" }}>
          <Sparkles size={16} color="#fff" />
        </View>
        <T style={{ fontFamily: fonts.headingSemi, fontSize: 16, color: "#fff" }}>Wayly Summary</T>
      </View>
      <T style={{ fontFamily: fonts.body, fontSize: 15, lineHeight: 24, color: "rgba(255,255,255,0.92)" }}>{sanitizeAI(summary)}</T>
    </View>
  );
}
...
export default function InvoiceResultView({ result, onDraftLetter, onDraftAll }: { result: any; onDraftLetter?: (i: number) => void; onDraftAll?: () => void }) {
  ...
  return (
    <View testID="inv1-result" style={{ gap: spacing.md }}>
      {result?.identity_warning ? <IdentityNotice warn={result.identity_warning} /> : null}
