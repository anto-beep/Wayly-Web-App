...
import { KeyboardAvoidingView, Modal, Platform, Pressable, ScrollView, StyleSheet, TextInput, View, ActivityIndicator, Linking } from "react-native";
...
// ---------------------------------------------------------------------------
// Client-side required-field gate — parity with web. The letter must NOT
// generate until required intake fields for the situation are filled.
// ---------------------------------------------------------------------------
const REQUIRED_INTAKE_BY_ARCHETYPE: Record<string, [string, string][]> = {
...
function missingRequiredIntake(archetype: string, intake: any): string[] {
  const req = REQUIRED_INTAKE_BY_ARCHETYPE[archetype] || [];
  const it = intake || {};
  return req.filter(([k]) => { const v = it[k]; return v === undefined || v === null || String(v).trim() === ""; }).map(([, label]) => label);
}
// Match the capitalisation of the Letters & Follow-Ups situation cards.
function capitaliseSituation(label?: string | null): string {
  if (!label) return "";
...
function CrossToolPanel({ entryId, onImport }: { entryId: string; onImport: (data: any) => void }) {
  ...
  const addedLabel = (tool: string, fields: any): string => {
    switch (tool) {
  ...
  return (
    <View testID="lf1-cross-tool-panel">
      ...
      {added.length > 0 ? (
        <View testID="lf1-cross-tool-added" style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.sm, marginTop: spacing.sm }}>
          {added.map((a) => (
            <View key={a.key} testID={`lf1-added-${a.key}`} style={[styles.chip, { borderColor: colors.sage, backgroundColor: colors.sage + "22" }]}>
              <ClipboardCheck size={13} color={colors.sage} />
              <T variant="small" style={{ color: colors.sage, fontFamily: fonts.bodySemi }}>Added: {a.label}</T>
            </View>
          ))}
        </View>
      ) : null}
...
function RecipientDirectoryCard({ entryId, currentSpecific, onApplied }: { entryId: string; currentSpecific: any; onApplied: (specific: any) => void }) {
  ...
  // Auto-address on open: pre-fill the saved provider once when no recipient is set.
  const autoAppliedRef = useRef(false);
  useEffect(() => {
    if (autoAppliedRef.current) return;
    const pe = (active as any)?.provider_email;
    if (!currentName && !currentEmail && pe) {
      autoAppliedRef.current = true;
      apply({ id: "__provider__", name: (active as any)?.provider_name || "Your provider", email: pe });
    }
  }, [currentName, currentEmail, active]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <Card testID="lf1-recipient-directory">
      ...
      {(active as any)?.provider_email && !(currentName === ((active as any)?.provider_name || "") && currentEmail === (active as any).provider_email) ? (
        <Pressable testID="lf1-recipient-use-provider" onPress={() => apply({ id: "__provider__", name: (active as any)?.provider_name || "Your provider", email: (active as any).provider_email })} disabled={busyId === "__provider__"}
          style={{ flexDirection: "row", alignItems: "center", gap: 8, borderWidth: 2, borderColor: colors.primary, borderRadius: radius.md, paddingHorizontal: spacing.sm, paddingVertical: 10, marginBottom: spacing.sm, opacity: busyId === "__provider__" ? 0.6 : 1 }}>
          <Users size={16} color={colors.primary} />
          <View style={{ flex: 1 }}>
            <T variant="small" style={{ fontFamily: fonts.bodySemi, color: colors.text }}>Address to {(active as any)?.provider_name || "your saved provider"}</T>
            <T variant="small" style={{ color: colors.muted, fontSize: 12 }}>{(active as any).provider_email}</T>
          </View>
          <T variant="small" style={{ color: colors.primary, fontFamily: fonts.bodySemi, fontSize: 12 }}>One tap</T>
        </Pressable>
      ) : null}
      {currentName ? (
...
export default function CorrespondenceDetail() {
  ...
  const requiredMissing = missingRequiredIntake(archetype, intake);
  const scrollRef = useRef<ScrollView>(null);

  // Issues carried over from another tool (Statement Decoder, Invoice Checker,
  // Support Plan Reviewer, etc.) via entry.source_import. Mirrors web.
  ...
  const generate = async () => {
    if (requiredMissing.length) { setError("Please complete the required fields before drafting."); return; }
  ...
  // Edit the generated letter body inline; kept when the user taps Save as draft.
  const editBody = (v: string) => { setDraft((d: any) => (d ? { ...d, body: v } : d)); setDirty(true); };

  // Open the drafted letter in the device email app, pre-addressed to the
  // saved provider email when we have one.
  const openInEmailApp = async () => {
    if (!draft?.body) return;
    const to = (entry?.recipient_specific?.email || "").trim();
    const subject = draft?.subject || "";
    const body = draft?.body || "";
    const url = `mailto:${encodeURIComponent(to)}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
    try { await Linking.openURL(url); } catch { setError("Could not open your email app."); }
  };
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <ScrollView ref={scrollRef} contentContainerStyle={{ padding: spacing.lg, paddingBottom: spacing.xxl, gap: spacing.md }} keyboardShouldPersistTaps="handled">
          ...
          <Card testID="lf-detail-summary" style={{ backgroundColor: colors.sageSoft, borderColor: colors.sageSoft }}>
            <T style={{ fontFamily: fonts.bodySemi, fontSize: 15, color: colors.text }}>{capitaliseSituation(entry.situation_label)}</T>
          ...
          {requiredMissing.length ? (
            <View testID="lf1-required-gate">
              <View style={[styles.softNote, { backgroundColor: colors.goldSoft, borderColor: colors.gold }]} testID="lf1-required-missing">
                <View style={{ flex: 1 }}>
                  <T variant="small" style={{ fontFamily: fonts.bodySemi, color: colors.text }}>Complete these required fields before drafting:</T>
                  {requiredMissing.map((l) => <T key={l} variant="small" style={{ color: colors.muted, fontSize: 12 }}>• {l}</T>)}
                  <Pressable testID="lf1-required-missing-jump" onPress={() => scrollRef.current?.scrollTo({ y: 0, animated: true })} style={{ marginTop: 6 }}>
                    <T variant="small" style={{ color: colors.primary, fontFamily: fonts.bodySemi, textDecorationLine: "underline" }}>Take me to the fields</T>
                  </Pressable>
                </View>
              </View>
            </View>
          ) : null}
          <Button label={draft ? "Regenerate letter" : (isGuided ? "Build safeguarding record" : isResponseDraft ? "Draft my reply" : "Generate letter")} testID="lf1-generate-button" icon={isGuided ? ShieldCheck : Sparkles} onPress={generate} loading={busy} disabled={requiredMissing.length > 0} />
              ...
              <Card testID="lf1-draft">
                <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: spacing.sm }}>
                  ...
                  <View style={{ flexDirection: "row", gap: spacing.sm, flexWrap: "wrap" }}>
                    ...
                    {outMode === "email" ? (
                      <Button label={editingBody ? "Done" : "Edit"} variant={editingBody ? "primary" : "outline"} icon={PenLine} testID="lf1-edit-letter" onPress={() => setEditingBody((e) => !e)} style={{ minHeight: 38, paddingHorizontal: spacing.md }} />
                    ) : null}
                    <Button label="Open in email" variant="outline" icon={Send} testID="lf1-open-in-email" onPress={openInEmailApp} style={{ minHeight: 38, paddingHorizontal: spacing.md }} />
                ...
                {outMode === "email" && editingBody ? (
                  <TextInput
                    testID="lf1-generated-body-edit"
                    value={draft.body || ""}
                    onChangeText={editBody}
                    multiline
                    style={{ marginTop: spacing.sm, minHeight: 240, borderWidth: 1, borderColor: colors.primary, borderRadius: radius.md, padding: spacing.md, textAlignVertical: "top", fontFamily: fonts.body, fontSize: 14, lineHeight: 22, color: colors.text, backgroundColor: colors.bg }}
                  />
                ) : (
                ...
                {outMode === "email" && editingBody ? <T variant="small" style={{ color: colors.muted, marginTop: 6, fontStyle: "italic", fontSize: 12 }} testID="lf1-edit-hint">Your edits are kept when you tap Save as draft, and used for the PDF and email.</T> : null}
