...
export default function PlanBillingScreen() {
  ...
  const [busy, setBusy] = useState<null | "portal" | "cancel" | "reactivate" | "downgrade" | "keep">(null);
  ...
  const noPlan = !sub?.plan || planKey === "free";
  ...
  const statusLabel = noPlan ? "NO ACTIVE PLAN" : isCancelling ? "CANCELS SOON" : isTrial ? "FREE TRIAL" : (sub?.status || "active").toUpperCase();
  const statusTone: "success" | "alert" | "brand" = noPlan ? "alert" : isCancelling ? "alert" : isTrial ? "brand" : "success";
  ...
  const keepCurrentPlan = async () => {
    setBusy("keep"); setActionError("");
    try { await apiFetch("/payments/cancel-scheduled-change", { method: "POST", body: {} }); load(); }
    catch (e) { setActionError(e instanceof ApiError ? e.message : "Could not cancel the scheduled change. Please try again."); }
    finally { setBusy(null); }
  };

  const hasPending = !!sub?.pending_plan;

  // Participant-driven billing breakdown (parity with web + the Participants
  // screen): base plan + add-ons for participants beyond what the plan includes.
  const { participants, reload: reloadParticipants } = useParticipants();
  useFocusEffect(useCallback(() => { reloadParticipants(); }, [reloadParticipants]));
  const activeCount = participants.filter((p) => !(p as any).removed_at && (p as any).status !== "removed").length || participants.length;
  const included = planKey === "family" ? 2 : 1;
  const baseFortnight = planKey === "family" ? 49.5 : planKey === "solo" ? 24.5 : 0;
  const addonCount = Math.max(0, activeCount - included);
      ...
      {loading ? (
        ...
        >
          <Card testID="billing-current-plan">
            ...
            <T style={{ fontFamily: fonts.heading, fontSize: 30, color: colors.text, marginTop: 4 }}>{noPlan ? "No active plan" : meta.name}</T>
            {noPlan ? (
              <T variant="small" style={{ marginTop: 6 }}>You don&apos;t have a plan yet. Start a 7-day free trial to unlock Wayly. Your card is held securely by Stripe and you can cancel anytime.</T>
            ) : (
              <T style={{ fontFamily: fonts.bodySemi, fontSize: 16, color: colors.primary, marginTop: 2 }}>{meta.price}</T>
            )}
            {!noPlan && planKey !== "free" ? <T variant="small" style={{ marginTop: 2 }}>Billed every 14 days · Includes GST</T> : null}
            ...
            {hasPending ? (
              <View testID="billing-pending-change" style={{ marginTop: spacing.md, padding: spacing.md, borderRadius: radius.md, backgroundColor: colors.goldSoft, flexDirection: "row", gap: 8, alignItems: "flex-start" }}>
                <Clock size={16} color={colors.gold} style={{ marginTop: 2 }} />
                <T variant="small" style={{ flex: 1, color: colors.text }}>
                  Your plan changes to <T style={{ fontFamily: fonts.bodySemi }}>{(PLAN_META[(sub?.pending_plan || "").toLowerCase()] || {}).name || sub?.pending_plan}</T> on {fmt(sub?.pending_effective)}. You keep {meta.name} until then.
                </T>
              </View>
            ) : null}

            {noPlan ? (
              <Button label="Start your free trial" testID="billing-start-trial-btn" onPress={() => router.push("/signup")} style={{ marginTop: spacing.md }} />
            ) : (
          ...
          {planKey !== "free" ? (
            <Card testID="billing-breakdown">
              ...
              <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: spacing.md }}>
                ...
                <T style={{ fontFamily: fonts.mono, fontSize: 13, color: colors.text }}>{money(baseFortnight)}</T>
              </View>
              <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: spacing.sm }}>
                <T variant="small">Participants</T>
                <T testID="billing-participant-count" style={{ fontFamily: fonts.mono, fontSize: 13, color: colors.text }}>{activeCount} ({included} included)</T>
              </View>
              {addonCount > 0 ? (
                <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: spacing.sm }}>
                  <T variant="small" testID="billing-addon-label">Extra participants ({addonCount} × $24.50)</T>
                  <T testID="billing-addon-amount" style={{ fontFamily: fonts.mono, fontSize: 13, color: colors.text }}>{money(addonCount * 24.5)}</T>
                </View>
              ) : null}
              <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: spacing.sm, borderTopWidth: 1, borderTopColor: colors.border, paddingTop: spacing.sm }}>
                ...
                <T testID="billing-fortnightly-total" style={{ fontFamily: fonts.bodySemi, fontSize: 14, color: colors.text }}>{money(fortnightTotal)}</T>
              </View>
              <T variant="small" style={{ marginTop: 6, color: colors.muted }}>Billed every 14 days · Includes GST</T>
          ...
          {!noPlan ? (
            <>
              ...
              {hasPending ? (
                <Button label={`Keep ${meta.name} plan`} testID="billing-keep-plan" variant="outline" onPress={keepCurrentPlan} loading={busy === "keep"} />
              ) : planKey === "family" ? (
                <Button label="Downgrade to Solo" testID="billing-downgrade-solo" variant="outline" onPress={downgradeToSolo} loading={busy === "downgrade"} />
              ) : null}
            </>
          ) : null}
        </ScrollView>
