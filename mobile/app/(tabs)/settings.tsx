...
export default function SettingsScreen() {
  ...
  const noPlan = !plan || plan === "free";
  const planLabel = noPlan ? "No active plan" : plan.replace(/^\w/, (c) => c.toUpperCase());
  const rawStatus = (user?.subscription_status || "").toLowerCase();
  const isTrialing = rawStatus === "trialing" || rawStatus === "trial";
  const status = noPlan ? "PAYMENT REQUIRED" : isTrialing ? "FREE TRIAL" : (user?.subscription_status || "active").toUpperCase();
    ...
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      <ScrollView ref={scrollRef} contentContainerStyle={{ padding: spacing.lg, paddingBottom: spacing.xxl, gap: spacing.md }}>
        ...
        <Card testID="settings-plan" style={{ backgroundColor: colors.primary, borderColor: colors.primary }}>
          ...
          {noPlan ? (
            <T style={{ color: "rgba(255,255,255,0.75)", fontFamily: fonts.body, fontSize: 13, marginTop: 2 }}>
              Add a card in Plan &amp; Billing to start your 7-day free trial.
            </T>
          ) : user?.trial_ends_at && isTrialing ? (
            <T style={{ color: "rgba(255,255,255,0.75)", fontFamily: fonts.body, fontSize: 13, marginTop: 2 }}>
        ...
        <Card testID="settings-privacy">
          ...
          <Pressable testID="settings-nav-about" onPress={() => router.push("/about")}
            style={[styles.linkRow, { borderBottomWidth: 1, borderBottomColor: colors.border }]}>
            <Info size={18} color={colors.primary} />
            <T style={{ fontFamily: fonts.bodyMedium, fontSize: 15, flex: 1, marginLeft: 8 }}>About Wayly</T>
            <ChevronRight size={18} color={colors.muted} />
          </Pressable>
          {[
