...
import { Sparkles, AlertTriangle, FileText, Clock, MessageCircle } from "lucide-react-native";
...
function StatementDecoderTool() {
  ...
  const draftLetterFromAnomaly = async (a: any) => {
    const data: any = await apiFetch("/care-plans/letter-from-finding", {
      method: "POST",
      body: {
        finding: { title: a.headline || a.title, detail: a.detail, suggested_question: a.suggested_action, addressee_primary: "provider" },
        addressee: "provider",
          ...
          {result?.upload_guard ? (
            ...
            <View testID="decoder-result">
              {result?.identity_warning ? (
                <View testID="decoder-identity-warning" style={{ flexDirection: "row", gap: 10, borderWidth: 2, borderColor: colors.terracotta, backgroundColor: colors.errorSoft, borderRadius: radius.md, padding: spacing.md, marginBottom: spacing.md }}>
                  <AlertTriangle size={18} color={colors.terracotta} style={{ marginTop: 1 }} />
                  <View style={{ flex: 1 }}>
                    <T style={{ fontFamily: fonts.bodySemi, fontSize: 13, color: colors.terracotta }}>{result.identity_warning.title}</T>
                    <T style={{ fontSize: 12.5, color: colors.text, marginTop: 3, lineHeight: 18 }}>{result.identity_warning.message}</T>
                  </View>
                </View>
              ) : null}
              {result?.duplicate_warning ? (
                <View testID="decoder-duplicate-warning" style={{ flexDirection: "row", gap: 10, backgroundColor: colors.goldSoft, borderRadius: radius.md, padding: spacing.md, marginBottom: spacing.md }}>
                  <AlertTriangle size={18} color={colors.gold} style={{ marginTop: 1 }} />
                  <View style={{ flex: 1 }}>
                    <T style={{ fontFamily: fonts.bodySemi, fontSize: 13, color: colors.text }}>{result.duplicate_warning.message}</T>
                    {result.duplicate_warning.existing_statement_id ? (
                      <Pressable testID="decoder-duplicate-view" onPress={() => router.push(`/statement/${result.duplicate_warning.existing_statement_id}` as any)} hitSlop={6}>
                        <T style={{ fontFamily: fonts.bodySemi, fontSize: 12, color: colors.primary, marginTop: 4, textDecorationLine: "underline" }}>Open the statement you already have</T>
                      </Pressable>
                    ) : null}
                  </View>
                </View>
              ...
              {result?.persisted_statement_id ? (
                <Pressable
                  ...
                  onPress={() => router.push({ pathname: "/(tabs)/ask", params: { statement_id: result.persisted_statement_id } } as any)}
                  style={{ flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, marginTop: spacing.lg, borderWidth: 1, borderColor: colors.primary, borderRadius: radius.pill, paddingVertical: 13 }}
                >
                  <MessageCircle size={17} color={colors.primary} />
                  <T style={{ fontFamily: fonts.bodySemi, fontSize: 14, color: colors.primary }}>Ask Wayly about this statement</T>
                </Pressable>
              ) : null}
            </View>
          ) : null}
