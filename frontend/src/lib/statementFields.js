...
/** Billing-period range, DD/MM/YYYY (DOC-PARITY-1 v2 decision 8), e.g.
 *  "01/07/2026 to 30/09/2026". Falls back to `period_label` when the
 *  start/end aren't extractable. */
export function periodCompact(stmt) {
    ...
    if (s && e) return `${ddmmyyyy(s)} to ${ddmmyyyy(e)}`;
    if (e) return ddmmyyyy(e);
