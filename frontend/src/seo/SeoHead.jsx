...
export default function SeoHead({
    ...
    return (
        <Helmet prioritizeSeoTags>
            ...
            {/* Bing site verification, emitted via helmet so it survives the
                react-snap prerender into every public page. Token is public.
                Google is verified via DNS; Yandex intentionally omitted. */}
            <meta name="msvalidate.01" content="6E0EE2D604B0A2FE3D507B04C335CBAD" />
...
export function articleLd({ headline, description, url, image, datePublished, dateModified, author, reviewedBy, citation }) {
    ...
    if (author) {
        ld.author = {
            "@type": "Person",
            name: author.name, ...(author.url ? { url: author.url } : {}), ...(author.sameAs ? { sameAs: author.sameAs } : {}), ...(author.jobTitle ? { jobTitle: author.jobTitle } : {}),
        };
    }
    if (reviewedBy) {
        ld.reviewedBy = {
            "@type": "Person",
            name: reviewedBy.name, ...(reviewedBy.jobTitle ? { jobTitle: reviewedBy.jobTitle } : {}), ...(reviewedBy.sameAs ? { sameAs: reviewedBy.sameAs } : {}),
        };
    }
    if (citation && citation.length) {
        ld.citation = citation.map((c) => ({
            "@type": "CreativeWork",
            name: c.title || c.name,
            url: c.url, ...(c.publisher ? { publisher: { "@type": "Organization", name: c.publisher } } : {}),
        }));
    }
    return ld;
}
...
export function breadcrumbLd(items) {
    return {
        ...
        itemListElement: items.map((it, i) => ({
            "@type": "ListItem",
            position: i + 1,
            name: it.name, ...(it.url ? { item: it.url.startsWith("http") ? it.url : `${SITE.domain}${it.url}` } : {}),
        })),
    };
}
