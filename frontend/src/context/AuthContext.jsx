...
export function AuthProvider({ children }) {
    ...
    // completeGoogleAuth now takes a Google ID token (credential) from Google
    // Identity Services / expo-auth-session and posts it to /auth/google.
    ...
    const logout = async () => {
        ...
        // Reset appearance to light on logout so the next session / new
        // session always starts in light mode (never inherits dark).
        try {
            ["wayly:app:appearance", "wayly:marketing:appearance", "kindred_theme"].forEach((k) => localStorage.removeItem(k));
            const raw = localStorage.getItem("wayly_a11y_v1");
            if (raw) {
                const p = JSON.parse(raw);
                p.dark = false;
                localStorage.setItem("wayly_a11y_v1", JSON.stringify(p));
            }
