...
function loadPrefs() {
    ...
    // Reconcile with the Settings app-appearance key so a user who toggled
    // dark mode in Settings (or any other surface) keeps it when this widget
    // boots on the next page load.
    try {
        const appPref = localStorage.getItem("wayly:app:appearance");
        if (appPref === "dark") prefs.dark = true;
        else if (appPref === "light") prefs.dark = false;
    } catch (_e) { /* noop */ }
    return prefs;
}
...
function applyPrefs(p) {
    ...
    // whatever the user picked here, and AppearanceScope keeps theme-dark
    // applied on every authenticated route mount.
    try { localStorage.setItem("wayly:app:appearance", p.dark ? "dark" : "light"); } catch (_e) { /* noop */ }
}
function savePrefs(p) {
    try { localStorage.setItem(STORE, JSON.stringify(p)); } catch (_e) { /* noop */ }
}
...
function ToggleRow({ icon, label, description, value, onChange, testid }) {
    return (
        ...
        >
            ...
            <span
                className={`switch-track flex-none mt-1 inline-flex h-5 w-9 rounded-full items-center transition-colors ${
