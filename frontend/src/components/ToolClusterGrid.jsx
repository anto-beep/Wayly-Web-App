...
function ToolCard({ tool, tone }) {
    const Icon = tool.IconComponent;
    const panel = tone === "clay" ? "panel-clay" : tone === "teal" ? "panel-teal" : tone === "terracotta" ? "panel-terracotta" : "panel-sage";
    return (
        <Link
                    ...
                    return (
                        <div key={cluster.id} data-testid={`cluster-${cluster.id}`}>
                            <div className="flex-1 min-w-0">
                                <h3 className="font-heading text-3xl font-bold tracking-tight" style={{ color: "#A5512B" }}>{cluster.title}</h3>
                                <p className="mt-2 text-base text-muted-k leading-relaxed max-w-2xl">{cluster.blurb}</p>
                            </div>
                            <div className="mt-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                {tools.map((t, ti) => (
                                    <ToolCard key={t.slug} tool={t} tone={["teal", "clay", "sage", "terracotta"][ti % 4]} />
                                ))}
                            </div>
                        </div>
                    );
