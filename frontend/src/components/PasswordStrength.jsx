...
export default function PasswordStrength({ password, email, name }) {
    ...
    const barColors = ["#E07A5F", "#D99E42", "#2BC4D6", "#3DB8A8"];
            ...
            <div className="flex gap-1">
                {[0, 1, 2, 3].map((i) => (
                    <div
                        ...
                        style={{ background: i < result.score - 1 ? activeColor : "#CFE0F0" }}
                    />
                ))}
            </div>
            <div className="mt-1.5 flex items-center justify-between text-[11px]">
                <span className="font-medium" style={{ color: activeColor }}>{result.label}</span>
                {result.containsIdentity && <span className="text-[#E07A5F]">Don't include your name/email</span>}
            </div>
            <ul className="mt-2 grid grid-cols-2 gap-y-1 gap-x-3 text-[11px]">
                {result.rules.map((r) => (
                    <li key={r.id} className={`flex items-center gap-1.5 ${r.ok ? "text-[#3DB8A8]" : "text-[#6B7280]"}`}>
