...
export default function StatementNotes({ statementId, initialNote = "", onSaved }) {
    ...
    return (
        <div className="panel-clay rounded-xl p-5" data-testid="statement-notes-card">
            <label className="flex items-center justify-between gap-2 text-xs uppercase tracking-[0.16em] font-semibold text-clay-k mb-2.5" htmlFor={`stmt-note-${statementId}`}>
                <span className="inline-flex items-center gap-1.5">
                    <StickyNote className="h-3.5 w-3.5" aria-hidden="true" /> Private note
                </span>
                <span className="inline-flex items-center gap-1.5 tabular-nums text-muted-k font-normal" aria-live="polite">
                    {saving ? (
            ...
            <textarea
                ...
                className="w-full min-h-[70px] resize-y text-sm bg-surface border border-kindred rounded-lg p-3 focus:outline-none focus:border-primary-k focus:ring-2 focus:ring-primary-k/20 text-primary-k placeholder:text-muted-k"
                maxLength={MAX_LEN}
