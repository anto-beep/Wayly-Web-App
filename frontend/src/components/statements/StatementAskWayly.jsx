...
export default function StatementAskWayly({ statementId, providerName, periodLabel }) {
    ...
    return (
        <section
            className="panel-teal rounded-xl overflow-hidden"
            data-testid="statement-ask-wayly-card"
            aria-labelledby={`ask-wayly-title-${statementId}`}
        >
            <header className="px-4 py-3 border-b border-kindred bg-primary-k/[0.06] flex items-start justify-between gap-3">
            ...
            <div className="p-4 space-y-3">
                ...
                {messages.length === 0 && (
                    <div className="flex flex-wrap gap-2" data-testid="statement-ask-wayly-suggestions">
                        {SUGGESTED_QUESTIONS.map((q, i) => (
                            <button
                                ...
                                onClick={() => send(q)}
                                disabled={sending}
                                className="text-xs font-medium bg-surface border border-gold text-clay-k rounded-full px-3.5 py-2 hover:bg-gold hover:text-white disabled:opacity-40 transition-colors shadow-sm"
                ...
                {/* Input */}
                <div className="rounded-xl bg-surface border border-primary-k/25 p-2.5 shadow-sm">
                    ...
                    <textarea
                        ...
                        placeholder={messages.length === 0
                            ? "Type your question here, e.g. what am I paying out of pocket?"
                            : "Ask a follow-up…"}
                        className="flex-1 min-h-[42px] max-h-[160px] resize-none text-sm bg-transparent border-0 px-2 py-2 focus:outline-none text-primary-k placeholder:text-muted-k"
