...
export default function NotificationsBell({ tone = "dark" }) {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            setItems(fresh.filter((n) => !n.read));
    ...
    const markAllRead = async () => {
        try {
            ...
            // Clear the list entirely — read notifications drop off the bell.
            setItems([]);
            setUnread(0);
        } catch {/* ignore */}
    };
    ...
    const markOneRead = async (n) => {
        // Viewing a notification takes it off the list. Optimistically remove
        // it and decrement the badge; roll back if the server call fails.
        ...
        try {
            ...
            // Roll back: restore the item and the badge.
    ...
    return (
        <div className="relative" ref={ref}>
            ...
            {open && (
                <div className="fixed left-3 right-3 top-16 w-auto max-w-none sm:absolute sm:left-auto sm:right-0 sm:top-auto sm:mt-2 sm:w-[22rem] sm:max-w-[95vw] bg-white rounded-xl shadow-[0_24px_64px_rgba(0,0,0,0.18)] border border-[#CFE0F0] overflow-hidden z-50" data-testid="notifications-dropdown">
