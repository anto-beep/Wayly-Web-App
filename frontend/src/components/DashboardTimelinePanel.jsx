    ...
    if (item.type === "alert") {
        ...
        return (
            <li className="flex items-start gap-2.5 text-sm" data-testid="dashboard-timeline-row-alert">
                ...
                <div className="flex-1 min-w-0">
                    <div className="text-primary-k font-medium truncate">{humanizeMonths(a.title) || "Alert"}</div>
                    <div className="text-xs text-muted-k truncate">{humanizeMonths(a.body)}</div>
                </div>
                <span className="text-xs text-muted-k whitespace-nowrap">{when}</span>
            </li>
        );
