 ...
 *
 * The `done` state below carries the destination address and stays on
 * screen until the person dismisses it, a persistent receipt (spec 3.1)
 * for the sensitive info they gave us. Errors surface as an inline
 * StandingBanner (also persistent) so the retry path is obvious.
 *
 * Props:
...
export default function EmailResultButton({ tool, headline, bodyHtml }) {
    ...
    const submit = async (e) => {
        ...
        try {
            ...
            setError(extractErrorMessage(err, "Could not send. Try again."));
        } finally {
            setBusy(false);
        }
    };
