...
export function ArchetypeIntakeForm({ archetype, intake, onChange, disabled = false }) {
    ...
    // Provider-directed letters capture who the letter is going TO so the
    // draft, cover note and PDF address the right recipient.
    const showRecipient = ["request", "dispute", "complaint", "escalation", "notification"].includes(archetype);
...
function Field({ label, hint, children, testId, required }) {
    return (
        <div data-testid={testId}>
            <label className="flex items-center justify-between gap-2 text-xs uppercase tracking-wider text-muted-k mb-1.5">
                <span>{label}</span>
...
// Shared recipient block for provider-directed letters, so the draft, cover
// note and PDF address the right person. Optional but pre-filled from a tool
...
function RecipientField({ intake, set }) {
    ...
    useEffect(() => {
        ...
        api.get("/lf1/recipients")
            .then(({ data }) => { if (!cancelled) setSaved(data?.recipients || []); })
            .catch(() => { /* noop */ });
    ...
    const selectedId = useMemo(() => {
        const m = saved.find((r) => (r.name || "") === (intake?.recipient_name || "") && (r.email || "") === (intake?.recipient_email || ""));
        return m ? m.id : "";
    }, [saved, intake?.recipient_name, intake?.recipient_email]);

    const onPickSaved = (id) => {
        if (!id) { set({ recipient_name: "", recipient_email: "" }); return; }
        const rec = saved.find((r) => r.id === id);
        if (rec) set({ recipient_name: rec.name || "", recipient_email: rec.email || "" });
    };

    return (
        <div className="rounded-xl border border-kindred bg-surface-2 p-3.5" data-testid="lf1-intake-recipient">
            <div className="text-xs uppercase tracking-wider text-muted-k mb-2.5 flex items-center gap-1.5">
                <MailCheck className="h-3.5 w-3.5" /> Who is this going to?
            </div>
            {saved.length > 0 && (
                <div className="mb-3" data-testid="lf1-intake-recipient-saved-wrap">
                    <Field label="Choose a saved provider or recipient" testId="lf1-intake-recipient-saved">
                        <select
                            value={selectedId}
                            onChange={(e) => onPickSaved(e.target.value)}
                            data-testid="lf1-intake-recipient-saved-select"
            ...
            <div className="grid sm:grid-cols-2 gap-3">
                <Field label="Provider / recipient name" testId="lf1-intake-recipient-name">
                    <TextInput
                        value={intake?.recipient_name}
                        onChange={(v) => set({ recipient_name: v })}
                        placeholder="e.g. Blueberry Care"
                        testId="lf1-intake-recipient-name-input"
                    />
                </Field>
                <Field label="Recipient email (optional)" testId="lf1-intake-recipient-email">
                    <TextInput
                        type="email"
                        value={intake?.recipient_email}
                        onChange={(v) => set({ recipient_email: v })}
                        placeholder="e.g. care@provider.com.au"
                        testId="lf1-intake-recipient-email-input"
                    />
                </Field>
            </div>
        </div>
...
function RequestForm({ intake, set }) {
    return (
        <div className="space-y-4">
            <Field label="Participant name" required testId="lf1-intake-participant">
                <TextInput
                    value={intake?.participant_name}
            ...
            <Field label="Type of request" required testId="lf1-intake-change-type">
                <Dropdown
            ...
            <Field label="What has changed and what are you asking for?" required testId="lf1-intake-change-summary">
                <TextArea
...
function DisputeForm({ intake, set }) {
    return (
        <div className="space-y-4">
            ...
            <Field label="What are you disputing?" required testId="lf1-intake-dispute-type">
                <Dropdown
            ...
            <Field label="Summary of the disputed item" required testId="lf1-intake-dispute-summary">
                <TextArea
...
function ComplaintForm({ intake, set }) {
    return (
        <div className="space-y-4">
            ...
            <Field label="Complaint category" required testId="lf1-intake-complaint-category">
                <Dropdown
            ...
            <Field label="What happened, when, and how it affected you or your family member?" required testId="lf1-intake-complaint-summary">
                <TextArea
...
function EscalationForm({ intake, set }) {
    ...
    return (
        <div className="space-y-4">
            ...
            <Field label="Why are you escalating now? Summary." required testId="lf1-intake-escalation-summary">
                <TextArea
...
function NotificationForm({ intake, set }) {
    return (
        <div className="space-y-4">
            ...
            <Field label="Notification type" required testId="lf1-intake-notification-type">
                <Dropdown
            ...
            <Field label="What are you notifying them of?" required testId="lf1-intake-notification-summary">
                <TextArea
...
function ResponseDraftForm({ intake, set }) {
    return (
        <div className="space-y-4">
            <Field label="Who is this reply going to?" required testId="lf1-intake-inbound-from">
                <TextInput
            ...
            <Field label="Paste or summarise what they sent you" required testId="lf1-intake-inbound-summary">
                <TextArea
...
function SafeguardingForm({ intake, set }) {
    return (
        <div className="space-y-4">
            ...
            <Field label="Category of concern" required testId="lf1-intake-safeguarding-category">
                <Dropdown
                    value={intake?.category}
                    onChange={(v) => set({ category: v })}
            ...
            <Field label="What did you observe? When?" required testId="lf1-intake-safeguarding-observation">
                <TextArea
...
export function CrossToolImportPanel({ entryId, onImport }) {
    ...
    const [added, setAdded] = useState([]);  // confirmation tags after a chip is tapped
    ...
    // Short, plain-English tag for the "added" confirmation chip, e.g.
    // "price check $90", "statement April 2026".
    const addedLabel = (tool, fields) => {
        switch (tool) {
            case "statement_decoder": return `statement ${fields.statement_period || "(recent)"}`;
            case "care_plan_reviewer": return `care plan ${fields.care_plan_provider || "(recent)"}`;
            case "provider_price_checker": return `price check $${fields.ppc_rate ?? ""}`.trim();
            case "classification_self_check": return "classification check";
            case "contribution_estimator": return "contribution estimate";
    ...
    const attach = async (tool, record_id, fields, note) => {
        ...
        try {
            ...
            const label = addedLabel(tool, fields);
            setAdded((prev) => [...prev.filter((a) => a.key !== tool), { key: tool, label }]);
            onImport?.(data);
        } catch (err) {
    ...
    return (
        <div data-testid="lf1-cross-tool-panel">
            ...
            {added.length > 0 && (
                <div className="mt-2.5 flex flex-wrap gap-2" data-testid="lf1-cross-tool-added">
                    {added.map((a) => (
                        <span
                            key={a.key}
                            data-testid={`lf1-added-${a.key}`}
                            className="inline-flex items-center gap-1.5 rounded-full bg-sage/15 border border-sage/40 px-3 py-1.5 text-xs font-medium text-sage"
                        >
...
// =====================================================================
// =====================================================================
// Client-side required-field gate — the letter must NOT generate until the
// required intake fields for the situation are filled (dropdowns off "Choose",
// text fields non-empty). Returns friendly labels of what is still missing.
// =====================================================================
export const REQUIRED_INTAKE_BY_ARCHETYPE = {
    request:        [["participant_name", "Participant name"], ["change_type", "Type of request"], ["change_summary", "What has changed and what you're asking for"]],
    dispute:        [["participant_name", "Participant name"], ["dispute_type", "What you're disputing"], ["disputed_charge_summary", "Summary of the disputed item"]],
    complaint:      [["participant_name", "Participant name"], ["category", "Complaint category"], ["complaint_summary", "What happened"]],
    escalation:     [["participant_name", "Participant name"], ["escalation_summary", "Why you're escalating now"]],
    notification:   [["participant_name", "Participant name"], ["notification_type", "Notification type"], ["notification_summary", "What you're notifying them of"]],
    response_draft: [["inbound_from", "Who this reply is going to"], ["inbound_summary", "What they sent you"]],
    guided_pathway: [["participant_name", "Participant name"], ["category", "Category of concern"], ["observation", "What you observed"]],
};

export function missingRequiredIntake(archetype, intake) {
    const req = REQUIRED_INTAKE_BY_ARCHETYPE[archetype] || [];
    const it = intake || {};
    return req
        .filter(([k]) => {
            const v = it[k];
    ...
    const go = async () => {
        ...
        try {
            ...
            if (detail?.error === "source_data_missing") {
                ...
                // Point the user at the fields they still need to complete.
                try {
                    document.querySelector('[data-testid="lf1-intake-form"]')
...
export function OutputFormatSwitcher({ generated, onDownloadPdf, busyPdf, onBodyChange, recipientEmail }) {
    const [mode, setMode] = useState("email");
    const [copied, setCopied] = useState(false);
    const [editing, setEditing] = useState(false);
    if (!generated) return null;
    const isEmail = mode === "email";
    const displayText = isEmail ? generated.body : generated.mac_portal_short_form;

    const copy = async () => {
        const text = isEmail && generated.subject
    ...
    return (
        <div data-testid="lf1-output-formats">
            {/* Controls: view switch + edit + copy + email + PDF */}
            <div className="flex items-center justify-between gap-3 flex-wrap mb-3">
                <div className="inline-flex items-center gap-2 flex-wrap">
                    <span className="text-xs uppercase tracking-wider text-muted-k mr-1">View as</span>
                    <FormatBtn active={isEmail} onClick={() => setMode("email")} icon={FileText} label="Email / letter" testId="lf1-format-email" />
                ...
                <div className="inline-flex items-center gap-2 flex-wrap">
                    {isEmail && onBodyChange && (
                        <button
                            type="button"
                            onClick={() => setEditing((e) => !e)}
                            data-testid="lf1-edit-letter"
                            className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs transition-colors ${editing ? "bg-primary-k text-white border-primary-k" : "border-primary-k text-primary-k hover:bg-primary-k hover:text-white"}`}
                        >
                            <PenLine className="h-3.5 w-3.5" /> {editing ? "Done editing" : "Edit letter"}
                        </button>
                    )}
                    <button
                        type="button"
                        onClick={openInEmail}
                        data-testid="lf1-open-in-email"
                        className="inline-flex items-center gap-1.5 rounded-full border border-sage text-sage px-3 py-1.5 text-xs hover:bg-sage hover:text-white transition-colors"
                    >
                        <Send className="h-3.5 w-3.5" /> Open in email app
                    </button>
                    <button
                        type="button"
                        onClick={copy}
                        data-testid="lf1-copy-letter"
                        className="inline-flex items-center gap-1.5 rounded-full border border-primary-k text-primary-k px-3 py-1.5 text-xs hover:bg-primary-k hover:text-white transition-colors"
                    >
                    ...
                    <button
                        ...
                        className="inline-flex items-center gap-1.5 rounded-full bg-primary-k text-white px-3.5 py-1.5 text-xs hover:bg-[#091D33] disabled:opacity-60"
                    >
            ...
            {/* Letter "paper" so the draft reads like a real letter and stands out */}
            <div className="rounded-2xl border border-kindred bg-white shadow-sm overflow-hidden" data-testid="lf1-letter-paper">
                <div className="h-1.5 bg-gradient-to-r from-primary-k via-[#0E4D52] to-sage" aria-hidden="true" />
                <div className="p-6 sm:p-8">
                    {isEmail && generated.subject && (
                        <div className="mb-5 pb-5 border-b border-kindred">
                            <div className="text-[11px] uppercase tracking-wider text-muted-k">Subject</div>
                            <div className="mt-1 font-heading text-xl sm:text-2xl text-primary-k tracking-tight" data-testid="lf1-generated-subject">
                                {generated.subject}
                            </div>
                        </div>
                    )}
                    {!isEmail && (
                        <div className="text-[11px] uppercase tracking-wider text-muted-k mb-3">MAC portal short form</div>
                    )}
                    {isEmail && editing && onBodyChange ? (
                        <textarea
                            value={generated.body || ""}
                            onChange={(e) => onBodyChange(e.target.value)}
                            rows={16}
                            data-testid="lf1-generated-body-edit"
                            className="w-full rounded-md border border-primary-k/30 bg-surface px-3 py-2 font-body text-[15px] text-primary-k leading-7 focus:border-primary-k focus:outline-none"
                        />
                    ) : (
                        <div
                            className="whitespace-pre-wrap font-body text-[15px] text-primary-k leading-7"
                            data-testid={isEmail ? "lf1-generated-body" : "lf1-generated-portal"}
                        >
                            {displayText}
                        </div>
                    )}
            ...
            {isEmail && editing && (
                <div className="mt-2 text-xs text-muted-k italic inline-flex items-center gap-1" data-testid="lf1-edit-hint">
                    <Info className="h-3 w-3" /> Your edits are kept when you tap &quot;Save as draft&quot; and are used for the PDF and email.
                </div>
            )}
            {!isEmail && (
