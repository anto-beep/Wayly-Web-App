...
export default function FlagCard({
    ...
    const headingDot = heading && !/[.!?]$/.test(heading.trim()) ? `${heading.trim()}.` : heading;
        ...
        <li className={`rounded-xl border ${meta.tint}`} data-testid={testId}>
            ...
            >
                ...
                <span className="flex-1 min-w-0">
                    ...
                    <span className="mt-1 block font-semibold text-primary-k text-[15px] leading-snug">{headingDot}</span>
                </span>
            ...
            {open && hasBody && (
                <div className="px-4 pb-4 -mt-1 space-y-3" data-testid={testId ? `${testId}-body` : undefined}>
                    ...
                    {evidenceList.length > 0 && (
                        <div className="rounded-lg bg-surface-2 border border-kindred px-3 py-2.5" data-testid={testId ? `${testId}-evidence` : undefined}>
                            <div className="text-[11px] font-semibold uppercase tracking-wider text-muted-k mb-1.5 flex items-center gap-1.5">
                                <FileText className="h-3.5 w-3.5" /> From your statement
                            </div>
                            <ul className="space-y-1">
                                {evidenceList.map((e, i) => (
                                    <li key={i} className="text-[13px] text-primary-k/90 leading-snug tabular-nums">{e}</li>
                                ))}
                            </ul>
                        </div>
                    )}
