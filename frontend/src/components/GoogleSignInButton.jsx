 ...
 */
export default function GoogleSignInButton({ label = "Continue with Google", testid = "google-signin", planIntent = null, onBeforeClick = null }) {
    const onClick = () => {
        // Give callers a chance to persist pre-OAuth intent (e.g. second-participant
        // toggle on Signup) before the browser leaves for Emergent OAuth.
        if (typeof onBeforeClick === "function") {
            try { onBeforeClick(); } catch { /* pre-oauth hook failure, non-fatal */ }
        }
