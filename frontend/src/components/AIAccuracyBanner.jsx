...
export default function AIAccuracyBanner({ text, variant = "default", className = "" }) {
    if (variant === "anomaly") {
        return (
            <span
                className={`inline-flex items-center gap-1.5 text-xs text-amber-800 ${className}`}
    ...
    return (
        <div
            className={`flex items-start gap-3 rounded-lg border border-amber-300 bg-amber-50 p-3.5 ${className}`}
...
export const TOOL_DISCLAIMERS = {
    ...
    "reassessment-letter": "This tool drafts letters for your review. Always read the full draft before sending. The AI may make errors, so check all dates, names, and facts against your own records. Sending a letter is your decision. Wayly does not send anything on your behalf without your explicit review and action.",
    "letters-and-follow-ups": "This tool drafts letters for your review. Always read the full draft before sending. The AI may make errors, so check all dates, names, and facts against your own records. Sending a letter is your decision. Wayly does not send anything on your behalf without your explicit review and action.",
