            ...
            <section className="mx-auto max-w-4xl px-6 pb-20">
                ...
                <div className="bg-surface border border-kindred rounded-2xl p-6 space-y-5 mt-4" data-testid="budget-calculator">
                    <div>
                        ...
                        <p className="mt-2 text-xs text-muted-k">Each figure is the <strong className="text-primary-k">annual</strong> Support at Home budget for that classification, before the 10% care-management share. We break it into quarterly, per-stream and lifetime-cap figures once you calculate.</p>
                    </div>
                    ...
                    <div data-testid="bc-supplements" className="rounded-2xl border border-kindred border-l-4 p-5" style={{ backgroundColor: "rgba(165,81,43,0.06)", borderLeftColor: "#A5512B" }}>
                        <span className="text-xs uppercase tracking-[0.16em] text-clay-k font-semibold">Applicable supplements (optional)</span>
                        <p className="text-xs text-muted-k mt-1.5 leading-relaxed">
                            ...
                            {SUPPLEMENT_OPTIONS.map((opt) => {
                                ...
                                return (
                                    <label
                                        ...
                                        className={`flex items-start gap-2 rounded-lg border p-3 transition-colors ${checked ? "border-primary-k bg-primary-k/10" : "border-kindred bg-surface"} ${disabled ? "opacity-50 cursor-not-allowed" : "cursor-pointer hover:bg-surface-2"}`}
                                    >
                                        <input
                                            type="checkbox"
                                            checked={checked}
                ...
                {result && (
                    <div ref={resultRef} className="mt-6 space-y-5 animate-fade-up scroll-mt-20" data-testid="bc-result">
                        <ToolSummary
                        ...
                        <div className="grid sm:grid-cols-3 gap-4">
                            <div className="rounded-xl p-5 text-white shadow-sm" style={{ backgroundColor: "#A5512B" }} data-testid="bc-quarterly-gross">
                                ...
                                <div className="mt-2 font-heading text-3xl text-white tabular-nums"><NumberMono>{formatAUD2(result.quarterly_gross ?? (result.annual_total / 4))}</NumberMono></div>
                                <div className="text-xs text-white/75 mt-1.5">The figure printed on your statement (annual ÷ 4).</div>
                            </div>
                            <div className="rounded-xl p-5 text-white shadow-sm" style={{ backgroundColor: "#0E4D52" }} data-testid="bc-care-management">
                                <div className="text-xs uppercase tracking-[0.14em] font-semibold text-white/70">Care management (10%)</div>
                                <div className="mt-2 font-heading text-3xl text-white tabular-nums">−{formatAUD2(result.care_management_quarterly ?? ((result.quarterly_gross ?? result.annual_total/4) - result.quarterly_usable))}</div>
                                <div className="text-xs text-white/75 mt-1.5">Provider&apos;s care management slice.</div>
                            </div>
                            <div className="rounded-xl p-5 text-white shadow-sm" style={{ backgroundColor: "#425F47" }} data-testid="bc-quarterly-usable">
                                <div className="text-xs uppercase tracking-[0.14em] font-semibold text-white/70">Usable for services</div>
                                <div className="mt-2 font-heading text-3xl text-white tabular-nums"><NumberMono>{formatAUD2(result.quarterly_usable)}</NumberMono></div>
                                <div className="text-xs text-white/75 mt-1.5">What you can spend on care this quarter.</div>
                            </div>
                        </div>
                        {(() => {
                            ...
                            const usablePctR = Math.round((usable / gross) * 100);
                            const cmPctR = 100 - usablePctR;
                            return (
                                <div className="panel-sage rounded-xl p-5" data-testid="bc-usable-bar">
                                    <div className="text-xs uppercase tracking-[0.16em] font-semibold text-sage">How your quarterly budget splits</div>
                                    <p className="text-sm text-primary-k mt-1.5 leading-relaxed">
                                        Of your <strong>{formatAUD2(gross)}</strong> gross budget each quarter, <strong className="text-sage">{usablePctR}%</strong> is yours to spend on care and <strong className="text-clay-k">{cmPctR}%</strong> goes to the provider for care management.
                                    </p>
                                    <div className="mt-3 h-8 w-full rounded-lg overflow-hidden flex text-[11px] font-semibold" role="img" aria-label={`Usable ${formatAUD2(usable)} (${usablePctR}%), care management ${formatAUD2(cm)} (${cmPctR}%)`}>
                                        <div className="h-full flex items-center justify-center text-white" style={{ width: `${usablePct}%`, backgroundColor: "#425F47" }}>
                                            {usablePct > 14 && <span>{usablePctR}%</span>}
                                        </div>
                                        <div className="h-full flex items-center justify-center text-white" style={{ width: `${100 - usablePct}%`, backgroundColor: "#A5512B" }}>
                                            {(100 - usablePct) > 12 && <span>{cmPctR}%</span>}
                                        </div>
                                    </div>
                                    <div className="mt-3 grid grid-cols-2 gap-3">
                                        <div className="rounded-lg bg-surface border border-kindred p-3">
                                            <span className="inline-flex items-center gap-1.5 text-xs text-muted-k"><span className="h-2.5 w-2.5 rounded-full inline-block" style={{ backgroundColor: "#425F47" }} /> Usable for care</span>
                                            <div className="mt-1 font-heading text-lg text-sage tabular-nums">{formatAUD2(usable)}</div>
                                        </div>
                                        <div className="rounded-lg bg-surface border border-kindred p-3">
                                            <span className="inline-flex items-center gap-1.5 text-xs text-muted-k"><span className="h-2.5 w-2.5 rounded-full inline-block" style={{ backgroundColor: "#A5512B" }} /> Care management</span>
                                            <div className="mt-1 font-heading text-lg text-clay-k tabular-nums">{formatAUD2(cm)}</div>
                                        </div>
                                    </div>
                                </div>
                            );
                        })()}
                        <div className="sect-teal border border-kindred rounded-xl p-4 flex items-baseline justify-between" data-testid="bc-annual-summary">
                        ...
                        {(result.applied_supplements?.length > 0 || result.supplement_warnings?.length > 0) && (
                            <div data-testid="bc-supplements-result" className="rounded-xl border border-kindred border-l-4 p-5" style={{ backgroundColor: "rgba(165,81,43,0.06)", borderLeftColor: "#A5512B" }}>
                        ...
                        <div className="rounded-xl p-5 text-white shadow-sm" style={{ backgroundColor: "#425F47" }} data-testid="bc-lifetime-panel">
                            <div className="text-xs uppercase tracking-[0.16em] font-semibold text-white/70">Lifetime cap projection</div>
                            <div className="mt-3 flex items-baseline justify-between flex-wrap gap-3">
                                <div>
                                    <div className="font-heading text-3xl text-white"><NumberMono>{formatAUD2(result.lifetime_contributions)}</NumberMono></div>
                                    <div className="text-xs text-white/70 mt-1">of {formatAUD(result.lifetime_cap)}</div>
                                </div>
                                <div className="text-sm text-white/90 tabular-nums font-semibold">{result.lifetime_pct.toFixed(1)}% used</div>
                            </div>
                            <div className="mt-3 h-2.5 w-full rounded-full overflow-hidden" style={{ backgroundColor: "rgba(255,255,255,0.2)" }}>
                                <div className="h-full rounded-full" style={{ width: `${Math.min(100, result.lifetime_pct)}%`, backgroundColor: "#E89A6F" }} />
                            </div>
                            <div className="mt-3 grid sm:grid-cols-2 gap-3 text-xs">
                                <div data-testid="bc-lifetime-remaining" className="rounded-md px-3 py-2" style={{ backgroundColor: "rgba(255,255,255,0.12)" }}>
                                    <div className="text-white/70">Remaining before cap</div>
                                    <NumberMono className="text-white text-base">{formatAUD2(result.lifetime_remaining ?? Math.max(0, (result.lifetime_cap || 0) - (result.lifetime_contributions || 0)))}</NumberMono>
                                </div>
                                {result.expected_annual_contribution != null && (
                                    <div data-testid="bc-expected-contribution" className="rounded-md px-3 py-2" style={{ backgroundColor: "rgba(255,255,255,0.12)" }}>
                                        <div className="text-white/70">Estimated annual contribution</div>
                                        <NumberMono className="text-white text-base">{formatAUD2(result.expected_annual_contribution)}</NumberMono>
                                    </div>
                                )}
                            </div>
                            {result.years_to_cap != null && (
                                <p className="text-sm text-white/85 mt-3">
                                    At your current pace, you&apos;d reach the lifetime cap in approximately <span className="font-semibold text-white tabular-nums">{result.years_to_cap.toFixed(1)} years</span>.
                                </p>
                            )}
                        </div>

                        <div className="panel-teal rounded-xl p-5">
                            <div className="text-xs uppercase tracking-[0.16em] font-semibold text-primary-k">Rollover</div>
                        ...
                        {access !== "allowed" && (
                            <div className="bg-surface-2 rounded-xl p-5 border border-kindred">
                                ...
                                <div className="mt-3 flex items-center gap-3 flex-wrap">
                                    <Link to="/signup" className="text-sm bg-primary-k text-white rounded-lg px-5 py-2.5 hover:bg-[#091D33]">Start free trial</Link>
