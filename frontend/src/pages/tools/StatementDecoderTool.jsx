...
export default function StatementDecoderTool() {
    ...
    // Draft a real, pre-filled letter from a single decoder anomaly.
                    ...
                    {result && (
                        <div ref={resultRef} className="mt-6 animate-fade-up scroll-mt-20" data-testid="decoder-result">
                            {result.identity_warning && (
                                <div className="mb-5 flex items-start gap-3 rounded-2xl border-2 border-terracotta bg-terracotta/10 px-4 py-3.5" data-testid="decoder-identity-warning" role="alert">
                                    <AlertTriangle className="mt-0.5 h-5 w-5 text-terracotta flex-shrink-0" aria-hidden="true" />
                                    <div className="flex-1 text-sm text-primary-k">
                                        <p className="font-semibold text-terracotta">{result.identity_warning.title}</p>
                                        <p className="mt-1 leading-relaxed">{result.identity_warning.message}</p>
                                    </div>
                                </div>
                            )}
                            {result.duplicate_warning && (
                                <div className="mb-5 flex items-start gap-3 rounded-2xl border border-wayly-gold-300 bg-wayly-gold-50 px-4 py-3" data-testid="decoder-duplicate-warning" role="status">
                                    ...
                                    <div className="flex-1 text-sm text-[#5A4A1C]">
                                        ...
                                        {result.duplicate_warning.existing_statement_id && (
                                            <button
                                                type="button"
                                                onClick={() => navigate(`/app/statements/${result.duplicate_warning.existing_statement_id}`)}
                                                data-testid="decoder-duplicate-view"
                                                className="mt-1 inline-block font-semibold underline"
                                            >
                                                Open the statement you already have
                                            </button>
                                        )}
                                    </div>
                                </div>
                            ...
                            {result.publishable !== false && (() => {
                                ...
                                return (
                                    <SmartAISummary
                                        pageKey="statement-decoder-run"
                                        context={{
                                            provider: ss.provider_name || ss.provider || result.extracted?.provider_name || null,
                                            period: ss.period_label || result.extracted?.period_label || null,
                                            gross_aud: ss.total_gross ?? null,
                                            you_paid_aud: ss.total_participant_contribution ?? null,
                                            government_paid_aud: ss.total_government_paid ?? null,
                                            anomaly_count: (result.anomalies || result.audit?.anomalies || []).length,
                                        }}
                                    />
                                );
                            })()}
                            {/* DEC-1 publish gate: charts consume the untrusted
                                totals, so hide them when the statement is blocked. */}
                            {result.publishable !== false && (
                                <>
                                    <div className="mt-4" />
                                    <StatementInsightGraphics stmt={{ audit_json: result.audit, anomalies: result.anomalies || result.audit?.anomalies }} />
                                </>
                            )}
                            <div className="mt-5" />
                            ...
                            {result.persisted_statement_id && (() => {
                                ...
                                return (
                                    <>
                                        <div className="mt-5" />
                                        <StatementAskWayly
                                            statementId={result.persisted_statement_id}
                                            providerName={ss.provider_name || ss.provider || null}
                                            periodLabel={ss.period_label || null}
                                        />
                                        <div className="mt-5" />
                                        <StatementRightsPanel statementId={result.persisted_statement_id} />
                                    </>
                                );
                            })()}
