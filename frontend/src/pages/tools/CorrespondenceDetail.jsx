...
// Match the capitalisation of the Letters & Follow-Ups situation cards: strip a
// trailing full stop and capitalise the first letter, keeping the same wording.
function capitaliseSituation(label) {
...
function RecipientDirectory({ entryId, currentSpecific, onApplied, providerName, providerEmail }) {
    ...
    // Auto-address on open: if this letter has no recipient yet and the
    // participant has a saved provider email, pre-fill it once so the draft is
    // addressed before they touch anything. The one-tap button still lets them
    // switch to another recipient.
    const autoAppliedRef = useRef(false);
    useEffect(() => {
        if (autoAppliedRef.current) return;
        if (!currentName && !currentEmail && providerEmail) {
            autoAppliedRef.current = true;
            apply({ id: "__provider__", name: providerName || "Your provider", email: providerEmail });
        }
    }, [currentName, currentEmail, providerEmail]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    return (
        <div className="bg-surface border border-kindred rounded-2xl p-5" data-testid="lf1-recipient-directory">
            ...
            {providerEmail && !(currentName === (providerName || "") && currentEmail === providerEmail) && (
                <button
                    type="button"
                    onClick={() => apply({ id: "__provider__", name: providerName || "Your provider", email: providerEmail })}
                    disabled={busyId === "__provider__"}
                    data-testid="lf1-recipient-use-provider"
                    className="mt-3 w-full flex items-center gap-2 rounded-lg border-2 border-primary-k bg-primary-k/[0.04] px-3 py-2.5 text-left transition-colors hover:bg-primary-k/[0.08] disabled:opacity-60"
                >
                    <Users className="h-4 w-4 text-primary-k flex-shrink-0" />
                    <div className="min-w-0 flex-1">
                        <div className="text-sm font-medium text-primary-k">Address to {providerName || "your saved provider"}</div>
                        <div className="text-xs text-muted-k">{providerEmail}</div>
                    </div>
                    <span className="text-xs font-semibold text-primary-k">One tap</span>
                </button>
            )}
            {currentName && (
...
export default function CorrespondenceDetail() {
    ...
    // Client-side gate: the letter cannot be drafted until required intake fields are filled.
    const requiredMissing = missingRequiredIntake(entry?.archetype, intake);
    ...
    return (
        <div className="min-h-screen bg-kindred">
            ...
            <section className="mx-auto max-w-5xl px-6 pt-10 pb-4">
                ...
                >
                    {loading ? "Loading…" : (capitaliseSituation(entry?.situation_label) || "Correspondence")}
                </h1>
                {entry && (
            ...
            <section className="mx-auto max-w-5xl px-6 pb-16 space-y-5">
                {loading && (
                    ...
                    <>
                        ...
                        {!isGuidedPathway && (
                            <RecipientDirectory
                                ...
                                providerName={activeParticipant?.provider_name}
                                providerEmail={activeParticipant?.provider_email}
                            ...
                            {(intake?.prefill_source === "hardship" || intake?.prefill_source === "voice_check") && (
                                <div className="mb-4 rounded-xl border border-primary-k/15 bg-primary-k/[0.03] p-4" data-testid="lf1-detail-prefill-context">
                                    ...
                                    {intake.situation_label && (
                                        <p className="text-sm text-primary-k mt-1">{capitaliseSituation(intake.situation_label)}</p>
                                    )}
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="mt-3">
                                        {requiredMissing.length > 0 ? (
                                            <div data-testid="lf1-required-gate">
                                                <button
                                                    type="button"
                                                    disabled
                                                    data-testid="lf1-generate-button"
                                                    className="inline-flex items-center gap-2 rounded-full bg-primary-k text-white px-4 py-2 text-sm opacity-50 cursor-not-allowed"
                                                >
                                                    <MessageSquare className="h-4 w-4" />
                                                    {isGuidedPathway ? "Build the record" : isResponseDraft ? "Draft my reply" : "Generate letter"}
                                                </button>
                                                <div className="mt-3 bg-clay/10 border border-clay/25 rounded-xl p-3 text-sm text-primary-k" data-testid="lf1-required-missing">
                                                    <div className="font-medium">Complete these required fields before drafting:</div>
                                                    ...
                                                    <button
                                                        type="button"
                                                        onClick={() => { try { document.querySelector('[data-testid="lf1-intake-form"], [data-testid="lf1-intake-fieldset"]')?.scrollIntoView({ behavior: "smooth", block: "start" }); } catch { /* noop */ } }}
                                                        className="mt-2 text-xs font-medium underline text-primary-k"
                                                        data-testid="lf1-required-missing-jump"
                                                    >
                        ...
                        {generated && (
                            <div className="space-y-5" data-testid="lf1-generated-output">
                                ...
                                <OutputFormatSwitcher
                                    ...
                                    busyPdf={busyPdf}
                                    onBodyChange={(body) => { setGenerated((g) => ({ ...(g || {}), body })); setDirty(true); }}
                                    recipientEmail={entry?.recipient_specific?.email || intake?.recipient_email || ""}
                                />
                                <button
                                    type="button"
