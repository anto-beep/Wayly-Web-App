...
function QuestionCard({ index, question, persona, value, onChange }) {
    ...
    // Show the anchor for whatever is hovered, otherwise the selected option.
    // Rendered in a fixed-height slot BELOW the grid so revealing it never
    // resizes the option buttons (which used to cause the whole list to shake).
    const activeAnchorKey = hoverValue || value;
    const activeAnchor = activeAnchorKey ? question.anchors?.[activeAnchorKey] : null;
    return (
        <div className="border-b border-kindred pb-5" data-testid={`csc-q-${question.id}`}>
                ...
                {question.scale.map((opt) => {
                    ...
                    return (
                        <button
                            ...
                            onFocus={() => setHoverValue(opt.value)}
                            onBlur={() => setHoverValue(null)}
                            data-testid={`csc-q-${question.id}-${opt.value}`}
                            className={`text-left text-sm rounded-lg border-2 px-3 py-2.5 transition-colors duration-150 ${
                                selected
            ...
            {/* Fixed-height hint slot — content changes but height does not */}
            <div className="mt-2 min-h-[1.5rem] text-xs text-muted-k leading-snug" aria-live="polite" data-testid={`csc-q-${question.id}-anchor`}>
                {activeAnchor || ""}
            </div>
        </div>
    );
...
function ResultActions({ result, onRerun }) {
    ...
    return (
        <div className="rounded-2xl border border-kindred bg-surface p-5" data-testid="csc-actions">
            <div className="text-xs uppercase tracking-[0.18em] text-muted-k font-semibold mb-3">What next?</div>
            <div className="grid gap-3 sm:grid-cols-3">
                {/* Save as PDF — teal */}
                <button
                    onClick={downloadPdf}
                    ...
                    className="inline-flex items-center justify-center gap-2 bg-primary-k text-white text-sm font-semibold rounded-full px-4 py-2.5 hover:bg-[#091D33] transition-colors disabled:opacity-60"
                >
                ...
                <button
                    ...
                    className="inline-flex items-center justify-center gap-2 bg-gold text-white text-sm font-semibold rounded-full px-4 py-2.5 hover:brightness-95 transition-[filter] disabled:opacity-60"
                >
                ...
                <button
                    ...
                    className="inline-flex items-center justify-center gap-2 bg-[#6d907d] text-white text-sm font-semibold rounded-full px-4 py-2.5 hover:bg-[#5c7c6b] transition-colors"
                >
...
// ============================================================================
// Save this check + Saved checks list
// ============================================================================

function SaveCheckCard({ runId, onSaved }) {
    ...
    const save = async () => {
        ...
        setError(null);
        try {
            await api.patch(`/public/csc/runs/${runId}`, { name: name.trim() || null, saved: true });
            setSaved(true);
            onSaved?.();
        } catch (e) {
            setError(e?.response?.data?.detail || "Could not save this check.");
        } finally {
            setSaving(false);
        }
    };
    ...
    if (saved) {
        return (
            <div className="rounded-2xl border border-[#6d907d]/40 bg-[#6d907d]/10 p-5" data-testid="csc-save-done">
                <div className="text-sm text-primary-k inline-flex items-center gap-2">
                    <CheckCircle2 className="h-4 w-4 text-[#6d907d]" /> Saved{name.trim() ? ` as “${name.trim()}”` : ""}. You'll find it under Your saved checks next time.
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="rounded-2xl border border-kindred bg-surface p-5" data-testid="csc-save-card">
            <div className="text-sm font-semibold text-primary-k">Save this check</div>
            <p className="text-xs text-muted-k mt-1">Give it a name so you can compare it against a future run, e.g. after a fall or a hospital stay.</p>
            <div className="mt-3 flex flex-col sm:flex-row gap-2">
                <input
                    ...
                    maxLength={120}
                    data-testid="csc-save-name"
                    className="flex-1 rounded-lg border border-kindred bg-surface px-3 py-2.5 text-sm focus:outline-none focus:ring-2 ring-primary-k"
                />
                <button
                    onClick={save}
                    disabled={saving}
                    data-testid="csc-save-submit"
                    className="inline-flex items-center justify-center gap-2 bg-primary-k text-white text-sm font-semibold rounded-full px-5 py-2.5 hover:bg-[#091D33] transition-colors disabled:opacity-60"
                >
                    {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <BookmarkPlus className="h-4 w-4" />}
                    Save check
                </button>
            </div>
            {error && <div className="text-xs text-clay-k mt-2" data-testid="csc-save-error">{error}</div>}
        </div>
    );
}

function SavedChecksPanel({ checks, onReopen, onRerun, onDelete, busyId }) {
    const [open, setOpen] = useState(true);
    if (!checks?.length) return null;
    const bandLabel = (c) => (c?.range_low === c?.range_high ? `Classification ${c?.primary}` : `Classification ${c?.range_low}–${c?.range_high}`);
    return (
        <div className="mb-6 rounded-2xl border border-kindred bg-surface" data-testid="csc-saved-panel">
            <button
                type="button"
                onClick={() => setOpen((v) => !v)}
                className="w-full flex items-center justify-between p-5 text-left"
                data-testid="csc-saved-toggle"
            >
                <span className="font-semibold text-primary-k inline-flex items-center gap-2">
                    <FolderOpen className="h-4 w-4 text-muted-k" /> Your saved checks
                    <span className="text-xs font-medium text-muted-k bg-surface-2 rounded-full px-2 py-0.5">{checks.length}</span>
                </span>
                {open ? <ChevronUp className="h-5 w-5 text-muted-k" /> : <ChevronDown className="h-5 w-5 text-muted-k" />}
            </button>
            {open && (
                <div className="px-5 pb-5 space-y-2.5">
                    {checks.map((c) => (
                        <div key={c.csc_run_id} className="rounded-xl border border-kindred bg-surface-2 p-4" data-testid={`csc-saved-item-${c.csc_run_id}`}>
                            <div className="flex flex-wrap items-center justify-between gap-2">
                                <div className="min-w-0">
                                    <div className="text-sm font-semibold text-primary-k truncate">{c.name || "Untitled check"}</div>
                                ...
                                <div className="flex items-center gap-2">
                                    <button
                                        onClick={() => onReopen(c)}
                                        data-testid={`csc-saved-reopen-${c.csc_run_id}`}
                                        className="text-xs font-medium text-primary-k border border-kindred rounded-full px-3 py-1.5 hover:bg-kindred transition-colors"
                                    >
                                        Edit
                                    </button>
                                    <button
                                        onClick={() => onRerun(c)}
                                        data-testid={`csc-saved-rerun-${c.csc_run_id}`}
                                        className="text-xs font-semibold text-white bg-[#6d907d] rounded-full px-3 py-1.5 hover:bg-[#5c7c6b] transition-colors"
                                    >
                                        Re-run
                                    </button>
                                    <button
                                        onClick={() => onDelete(c)}
                                        disabled={busyId === c.csc_run_id}
                                        data-testid={`csc-saved-delete-${c.csc_run_id}`}
                                        className="text-muted-k hover:text-clay-k transition-colors disabled:opacity-50 p-1.5"
                                        aria-label="Delete saved check"
                                    >
                                        {busyId === c.csc_run_id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
                                    </button>
                                </div>
                            </div>
                        </div>
...
export default function ClassificationCheck() {
    ...
    const loadSavedChecks = React.useCallback(() => {
        ...
        api.get("/public/csc/runs")
            .then(({ data }) => setSavedChecks(data?.runs || []))
            .catch(() => { /* not signed in / none yet */ });
    }, [access]);
    useEffect(() => { loadSavedChecks(); }, [loadSavedChecks]);
    ...
    const submit = async (overrideAnswers, overrideCurrent) => {
        ...
        try {
            const { data } = await api.post("/public/csc/run", {
                persona,
                current_classification: useCurrent ? parseInt(useCurrent) : null,
                answers: useAnswers,
            });
            setResult(data);
            try {
    ...
    const deleteCheck = async (check) => {
        ...
        try {
            await api.delete(`/public/csc/runs/${check.csc_run_id}`);
        ...
        <div className="min-h-screen bg-kindred">
            ...
            {result && (
                <section ref={resultRef} className="mx-auto max-w-5xl px-6 pb-20 space-y-5 animate-fade-up scroll-mt-20" data-testid="csc-result">
                    ...
                    <ResultActions result={result} onRerun={resetDraft} />
