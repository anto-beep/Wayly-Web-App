    ...
    return (
        <div className="min-h-screen bg-kindred">
            ...
            <section className="mx-auto max-w-4xl px-6 pb-20">
                ...
                {/* ------- Result region ------- */}
                {result && (
                    <div ref={resultRef} className="mt-6 space-y-5 animate-fade-up scroll-mt-20" data-testid="pc-result">
...
function QualityGuardPanel({ guard, onContinue, onAfterHours, testId }) {
    return (
        <div className="rounded-2xl border-2 border-clay bg-white shadow-[0_12px_32px_rgba(165,81,43,0.16)] overflow-hidden" data-testid={testId}>
            <div className="h-2 bg-gradient-to-r from-terracotta via-clay to-primary-k" aria-hidden="true" />
            <div className="p-6 flex items-start gap-4">
                <div className="flex-shrink-0 h-11 w-11 rounded-full bg-clay/15 flex items-center justify-center">
                    <AlertTriangle className="h-6 w-6 text-clay" aria-hidden="true" />
                </div>
                <div className="flex-1">
                    <div className="text-xs font-bold uppercase tracking-[0.14em] text-clay">Quick sanity check</div>
                    <p className="text-base sm:text-lg font-medium text-primary-k mt-1.5 leading-relaxed">{guard.prompt}</p>
                    <div className="mt-5 flex flex-wrap gap-3">
                        {guard.after_hours_toggle_available && (
                            <button
                                type="button"
                                onClick={onAfterHours}
                                className="px-4 py-2.5 rounded-full bg-sage text-white text-sm font-semibold shadow-sm hover:bg-[#4d6b52] active:scale-[0.98] transition-all"
                        ...
                        {guard.allow_continue && (
                            <button
                                type="button"
                                onClick={onContinue}
                                className="px-5 py-2.5 rounded-full bg-primary-k text-white text-sm font-semibold shadow-[0_6px_16px_rgba(14,42,71,0.28)] hover:bg-[#091D33] active:scale-[0.98] transition-all"
...
function ResultCard({ result, onAdmOpen, provider, selectedSnapshot, ceState, inlinePension }) {
    ...
    return (
        <>
            <div className="rounded-2xl border border-kindred border-l-4 border-l-[#0E4D52] bg-[#0E4D52]/[0.06] p-6" data-testid="pc-how-this-compares">
                ...
                {(() => {
                    ...
                    const pctOf = (v) => Math.min(100, Math.max(2, (v / domain) * 100));
                    const overRange = hi > 0 && chg > hi;
                    const chgColor = overRange ? "#B23A2E" : "#425F47";
                    ...
                    const Bar = ({ label, value, color, testid }) => (
                        <div className="flex items-center gap-3" data-testid={testid}>
                            <span className="w-28 shrink-0 text-xs text-muted-k text-right">{label}</span>
                            <div className="flex-1 h-7 rounded-lg bg-primary-k/5 relative overflow-hidden">
                                <div className="absolute inset-y-0 left-0 rounded-lg flex items-center justify-end px-2" style={{ width: `${pctOf(value)}%`, backgroundColor: color }}>
                                    <span className="text-xs font-semibold text-white whitespace-nowrap">{formatAUD2(value)}{u}</span>
                                </div>
                            </div>
                        </div>
                    );
                    return (
                        <div className="mt-5 rounded-2xl bg-surface border border-kindred p-4 sm:p-5" data-testid="pc-compare-bars">
                            <div className="flex items-center justify-between mb-2 gap-2 flex-wrap">
                                <span className="text-xs uppercase tracking-wider text-muted-k">What your provider charges vs typical</span>
                                {overRange
                                    ? <span className="text-xs font-semibold text-white bg-[#B23A2E] rounded-full px-2.5 py-0.5">Above the range</span>
                                    : <span className="text-xs font-semibold text-white bg-[#425F47] rounded-full px-2.5 py-0.5">Within the range</span>}
                            </div>
                            {med > 0 && diffPct !== null && (
                                <p className="text-sm text-primary-k mb-4 leading-relaxed" data-testid="pc-compare-plain">
                                    You pay <strong>{formatAUD2(chg)}{u}</strong>. That is{" "}
                                    <strong style={{ color: chgColor }}>{Math.abs(diffPct)}% {diffPct >= 0 ? "more than" : "less than"}</strong>{" "}
                                    the typical rate of {formatAUD2(med)}{u}.
                                </p>
                            )}
                            <div className="space-y-2.5">
                                {chg > 0 && <Bar label="You pay" value={chg} color={chgColor} testid="pc-charge-pin" />}
                                {med > 0 && <Bar label="Typical median" value={med} color="#0E4D52" />}
                                {hi > lo && (
                                    <div className="flex items-center gap-3">
                                        <span className="w-28 shrink-0 text-xs text-muted-k text-right">Usual range</span>
                                        <div className="flex-1 h-7 rounded-lg bg-primary-k/5 relative">
                                            <div className="absolute inset-y-0 rounded-lg bg-[#425F47]/25 border border-[#425F47]/40 flex items-center justify-center" style={{ left: `${pctOf(lo)}%`, width: `${Math.max(6, pctOf(hi) - pctOf(lo))}%` }}>
                                                <span className="text-[10px] font-medium text-[#2f4633] whitespace-nowrap px-1">{formatAUD2(lo)}–{formatAUD2(hi)}</span>
                                            </div>
                                        </div>
                                    </div>
                                )}
            ...
            <div className="grid sm:grid-cols-3 gap-3">
                ...
                {(result.lower != null && result.upper != null) ? (
                    <div className="sect-sage border border-kindred rounded-xl p-4" data-testid="pc-range">
                    ...
                    <div className="sect-sage border border-kindred rounded-xl p-4" data-testid="pc-range-unavailable">
                        <div className="text-xs uppercase tracking-wider text-muted-k">Indicative range</div>
...
function StatCard({ label, amount, unit, testId }) {
    return (
        <div className="sect-teal border border-kindred rounded-xl p-4" data-testid={testId}>
...
function ShareStatCard({ share, unit }) {
    ...
    if (share.mode === "picker") {
        return (
            <div className="sect-clay border border-kindred rounded-xl p-4" data-testid={testId}>
