...
import {
    ...
    FileText, ShieldAlert, Clock, ChevronRight, ChevronDown, Info, PenLine, Sparkles,
} from "lucide-react";
...
// The 12 situations grouped into six plain-English families, each with its own
...
// Each choice maps to a representative situation so the same create flow runs.
const BLANK_TYPES = [
    { label: "Request a Change",            situationId: 6,  icon: FileText },
    { label: "Dispute a Charge Or Decision", situationId: 3, icon: AlertTriangle },
    { label: "Make a Complaint",            situationId: 4,  icon: MessageSquare },
    { label: "Escalate To a Regulator",     situationId: 10, icon: ShieldAlert },
    { label: "Send a Notification",         situationId: 7,  icon: Mail },
    { label: "Reply To a Message",          situationId: 12, icon: MessageSquare },
];

// Per-accent Tailwind class sets (brand palette only, subtle usage).
const ACCENT = {
    teal:       { dot: "bg-[#0E4D52]",   icon: "text-[#0E4D52]",   hover: "hover:border-[#0E4D52]", soft: "bg-[#0E4D52]/[0.05]", bar: "bg-[#0E4D52]", headBg: "bg-[#0E4D52]/[0.07]", border: "border-[#0E4D52]/25", chipBg: "bg-[#0E4D52]/10" },
    gold:       { dot: "bg-gold",        icon: "text-gold",        hover: "hover:border-gold",      soft: "bg-gold/[0.06]",      bar: "bg-gold",       headBg: "bg-gold/[0.10]",     border: "border-gold/30",     chipBg: "bg-gold/15" },
    terracotta: { dot: "bg-terracotta",  icon: "text-terracotta",  hover: "hover:border-terracotta", soft: "bg-terracotta/[0.05]", bar: "bg-terracotta", headBg: "bg-terracotta/[0.07]", border: "border-terracotta/25", chipBg: "bg-terracotta/10" },
    sage:       { dot: "bg-sage",        icon: "text-sage",        hover: "hover:border-sage",      soft: "bg-sage/[0.07]",      bar: "bg-sage",       headBg: "bg-sage/[0.10]",     border: "border-sage/30",     chipBg: "bg-sage/15" },
    clay:       { dot: "bg-clay",        icon: "text-clay",        hover: "hover:border-clay",      soft: "bg-clay/[0.08]",      bar: "bg-clay",       headBg: "bg-clay/[0.10]",     border: "border-clay/30",     chipBg: "bg-clay/15" },
};
    ...
    return (
        <div className="min-h-screen bg-kindred">
            ...
            <section className="mx-auto max-w-5xl px-6 pb-16">
                ...
                {!loading && (
                    <>
                        {/* Start a blank letter — letters are not limited to scenarios. */}
                        <BlankLetterPanel
                            situations={situations}
                            busySituationId={busySituationId}
                        ...
                        <div className="space-y-4" data-testid="lf1-situation-grid">
                            {GROUPS.map((group) => {
                                const groupSituations = personalisedSituations.filter((s) => group.archetypes.includes(s.archetype));
                                if (groupSituations.length === 0) return null;
                                const accent = ACCENT[group.accent] || ACCENT.teal;
                                return (
                                    <CollapsibleGroup
                                        key={group.key}
                                        group={group}
...
function CollapsibleGroup({ group, accent, situations, busySituationId, onPick }) {
    const [open, setOpen] = useState(false);
    const Icon = group.icon || FileText;
    return (
        <div
            className={`rounded-2xl border ${accent.border} overflow-hidden`}
            data-testid={`lf1-group-${group.key}`}
        >
            <button
                type="button"
                onClick={() => setOpen((o) => !o)}
                aria-expanded={open}
                className={`w-full flex items-center gap-3 px-4 sm:px-5 py-4 text-left ${accent.headBg} transition-colors`}
                data-testid={`lf1-group-toggle-${group.key}`}
            >
                <span className={`h-10 w-10 rounded-xl ${accent.chipBg} flex items-center justify-center flex-shrink-0`}>
                    <Icon className={`h-5 w-5 ${accent.icon}`} aria-hidden="true" />
                </span>
                <span className="flex-1 min-w-0">
                    <span className="font-heading text-lg sm:text-xl text-primary-k tracking-tight block">{group.title}</span>
                    <span className="text-sm text-muted-k block mt-0.5">{group.desc}</span>
                </span>
                <span className={`text-xs font-semibold ${accent.icon} tabular-nums`}>{situations.length}</span>
                <ChevronDown className={`h-5 w-5 text-muted-k transition-transform ${open ? "rotate-180" : ""}`} aria-hidden="true" />
            </button>
            {open && (
                <ul className="grid sm:grid-cols-2 gap-3 p-4 sm:p-5 bg-surface border-t border-kindred">
                    {situations.map((s) => (
                        <SituationCard
                            key={s.id}
                            situation={s}
...
function BlankLetterPanel({ situations, busySituationId, onPick }) {
    const findSit = (id) => situations.find((s) => String(s.id) === String(id));
    return (
        <div
            className="rounded-2xl border border-primary-k/15 bg-gradient-to-br from-[#0E4D52]/[0.06] to-transparent p-5 sm:p-6"
            data-testid="lf1-blank-letter-panel"
        >
            <div className="flex items-start gap-3">
                <span className="h-9 w-9 rounded-full bg-primary-k text-white flex items-center justify-center flex-shrink-0">
                    <PenLine className="h-4.5 w-4.5" />
                </span>
                <div className="min-w-0">
                    <h2 className="font-heading text-xl text-primary-k tracking-tight">Start a blank letter</h2>
                    <p className="mt-1 text-sm text-muted-k leading-relaxed">
                        Know what you need? Pick a letter type and write it your way — you choose the recipient and the details.
                    </p>
                </div>
            </div>
            <div className="mt-4 grid sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
                {BLANK_TYPES.map((t) => {
                    const sit = findSit(t.situationId);
                    const busy = sit && busySituationId === sit.id;
                    const Icon = t.icon;
                    return (
                        <button
                            key={t.label}
...
function SituationCard({ situation, accent, busy, onClick }) {
    ...
    return (
        <li>
            ...
            >
                <div className="flex items-start gap-3">
                    <span className={`h-9 w-9 rounded-xl ${a.soft} flex items-center justify-center flex-shrink-0`}>
                        <Icon className={`h-4.5 w-4.5 ${a.icon}`} aria-hidden="true" />
                    </span>
                    <div className="flex-1 min-w-0">
                        <div className="text-primary-k leading-snug font-medium">{situation.label}</div>
