...
const GROUPS = [
    ...
    { key: "safety",   title: "A safety concern",           desc: "Worried about someone's safety? Start here — we guide you to call first.", accent: "clay",       archetypes: ["guided_pathway"] },
];
...
const ACCENT = {
    teal:       { dot: "bg-[#0E4D52]",   icon: "text-[#0E4D52]",   hover: "hover:border-[#0E4D52]", soft: "bg-[#0E4D52]/[0.05]", bar: "bg-[#0E4D52]" },
    gold:       { dot: "bg-gold",        icon: "text-gold",        hover: "hover:border-gold",      soft: "bg-gold/[0.06]",      bar: "bg-gold" },
    terracotta: { dot: "bg-terracotta",  icon: "text-terracotta",  hover: "hover:border-terracotta", soft: "bg-terracotta/[0.05]", bar: "bg-terracotta" },
    sage:       { dot: "bg-sage",        icon: "text-sage",        hover: "hover:border-sage",      soft: "bg-sage/[0.07]",      bar: "bg-sage" },
    clay:       { dot: "bg-clay",        icon: "text-clay",        hover: "hover:border-clay",      soft: "bg-clay/[0.08]",      bar: "bg-clay" },
};
                    ...
                    <>
                        ...
                        <div className="space-y-8" data-testid="lf1-situation-grid">
                            {GROUPS.map((group) => {
                                ...
                                return (
                                    <div key={group.key} data-testid={`lf1-group-${group.key}`}>
                                        <div className="flex items-center gap-2.5">
                                            <span className={`h-2.5 w-2.5 rounded-full ${accent.dot}`} aria-hidden="true" />
                                            <h2 className="font-heading text-xl text-primary-k tracking-tight">{group.title}</h2>
                                        </div>
                                        <p className="mt-1 ml-5 text-sm text-muted-k">{group.desc}</p>
                                        <ul className="mt-3 grid sm:grid-cols-2 gap-3">
                                            {groupSituations.map((s) => (
                                                <SituationCard
                                                    key={s.id}
                                                    situation={s}
                                                    accent={accent}
