...
export default function SupportAtHomeLevels() {
    ...
    const faqs = [
        ...
        { q: "Can my Support at Home level change?", a: "Yes. If needs grow you can request a support plan review through My Aged Care. The Wayly Classification Self-Check helps you decide whether to ask for one, and Wayly Letters & Follow-ups drafts the formal request." },
        { q: "Are these levels paid quarterly or annually?", a: "Each level's annual budget is paid as a quarterly budget. You get the full quarter's amount at the start of the quarter, with 10% set aside for care management." },
        { q: "What happens to unspent funding at the end of a quarter?", a: "You can carry over the higher of $1,000 or 10% of the quarterly budget to the next quarter. Anything above that is returned to the system." },
    ];
    const faqLd = {
        "@type": "FAQPage",
    ...
    const articleLd = {
        ...
        author: { "@type": "Person", name: "Wayly Editorial" },
            ...
            <main className="mx-auto max-w-7xl px-6 py-16">
                ...
                <div className="mt-10 overflow-x-auto rounded-2xl border border-kindred bg-surface" data-testid="levels-table">
                    <table className="w-full text-left text-sm min-w-[900px]">
                        <thead className="bg-surface-2 text-primary-k">
                            <tr>
                                <th className="px-5 py-3 font-heading whitespace-nowrap w-24">Level</th>
                                <th className="px-5 py-3 font-heading whitespace-nowrap">Annual funding</th>
                                <th className="px-5 py-3 font-heading whitespace-nowrap">Quarterly budget</th>
                                <th className="px-5 py-3 font-heading hidden md:table-cell">Who it suits</th>
                                <th className="px-5 py-3 whitespace-nowrap w-24"></th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-kindred">
                            {SUPPORT_AT_HOME_LEVELS.map((l) => (
                                <tr key={l.slug} data-testid={`row-${l.slug}`}>
                                    <td className="px-5 py-4 font-medium text-primary-k whitespace-nowrap">Level {l.number}</td>
                                    <td className="px-5 py-4 tabular-nums whitespace-nowrap">${l.annual.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                    <td className="px-5 py-4 tabular-nums whitespace-nowrap">${l.quarterly.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                ...
                <section className="mt-14" data-testid="hub-related">
                    ...
                    <ul className="mt-3 space-y-2 text-sm">
                        ...
                        <li data-testid="hub-related-chsp"><Link className="text-primary-k underline" to="/chsp/vs-support-at-home">On CHSP, not Support at Home? See how the two programs compare</Link></li>
                    </ul>
                </section>
