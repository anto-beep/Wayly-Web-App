...
export default function BillingSuccess() {
    ...
    const poll = useCallback(async (attempt = 0) => {
        ...
        try {
            // Try the new payments endpoint first, fall back to the legacy
            // /billing/status path for older one-time-payment sessions.
            let data;
            try {
                ({ data } = await api.get(`/payments/checkout/status/${sessionId}`));
            } catch {
                ({ data } = await api.get(`/billing/status/${sessionId}`));
            }
            // Subscription checkouts with a trial return payment_status
            // "no_payment_required" (no card was charged yet); treat them
            // as successful when session.status === "complete".
            const paid = ["paid", "no_payment_required"].includes((data.payment_status || "").toLowerCase());
            const complete = String(data.status || "").toLowerCase() === "complete";
            if (paid || complete) {
                setStatus("paid");
                setPlan(data.plan);
                await refreshUser();
                ...
                {status === "paid" && (
                    <>
                        ...
                        <h1 className="font-heading text-4xl text-primary-k tracking-tight mt-6" data-testid="billing-success-heading">
                            You are on Wayly {plan === "family" ? "Family" : "Solo"}.
                        </h1>
