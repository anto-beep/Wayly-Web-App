                            ...
                            <div className="p-5 space-y-4" data-testid="add-preview-step">
                                ...
                                {addPreview.branch === "solo_to_family" && (
                                    <div className="space-y-3" data-testid="branch-solo-to-family">
                                        ...
                                        <p className="text-xs text-muted-k">You&apos;ll be charged the prorated difference (about $25 per fortnight, adjusted for days remaining) now. From your next charge, you&apos;ll be billed $49.50 per fortnight instead of $24.50.</p>
                                        <div className="flex gap-2 flex-wrap">
                        ...
                        {step === "done" && lastAdded?.participant && (
                            <div className="p-5 space-y-3" data-testid="add-done-step">
                                ...
                                {lastAdded.addon?.id && (
                                    ...
                                    >
                                        Pay $24.50 per fortnight add-on now
                                    </button>
                                )}
                                {lastAdded.addon?.id ? (
                                    <button
                                        onClick={async () => {
                                            // Explicit rollback: unpaid add-ons must not silently
                                            // linger and add the participant to the count. Closing
                                            // the modal without paying = the user opted out of this
                                            // add-on right now, so cancel the pending charge and
                                            // archive the participant we optimistically created.
                                            if (!window.confirm(`Skip payment for now? ${lastAdded.participant?.first_name || "This person"} will be removed from your account so you're not charged. You can add them again any time.`)) return;
                                            try {
                                                await api.post("/billing/v2/cancel-pending-addon");
                                                toast.success(`${lastAdded.participant?.first_name || "Participant"} removed. No charge.`);
                                            } catch (e) {
                                                toast.error(extractErrorMessage(e, "Could not roll back the add-on"));
                                            }
                                            closeAdd();
                                            await loadAll();
                                            await refresh();
                                        }}
                                        className="w-full border border-kindred bg-white text-primary-k rounded-md px-4 py-2 text-sm hover:bg-surface-2"
                                        data-testid="skip-payment-btn"
                                    >
                                        Skip payment and remove {lastAdded.participant?.first_name || "them"}
                                    </button>
                                ) : (
