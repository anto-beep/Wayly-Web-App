...
/** Post timestamp: DD/MM/YYYY h:mm with capitalised AM/PM, no seconds. */
function formatPostTime(value) {
    ...
    h = h % 12 || 12;
    return `${date} ${h}:${pad(d.getMinutes())} ${ampm}`;
}
...
export default function FamilyWall() {
    ...
    return (
        <div className="space-y-6" data-testid="family-wall-page">
            <PageIntro
                eyebrow="Family Wall"
                title={`A Digital Fridge Door for ${active?.name || "Your Participant"}`}
                    ...
                    <article key={p.id} className="rounded-2xl p-4 space-y-3 border border-kindred shadow-sm" style={{ backgroundColor: ["rgba(14,77,82,0.07)","rgba(165,81,43,0.08)","rgba(66,95,71,0.09)","rgba(95,78,118,0.09)","rgba(178,58,46,0.07)"][pi % 5], borderLeft: `5px solid ${["#0E4D52","#A5512B","#425F47","#5F4E76","#B23A2E"][pi % 5]}` }} data-testid={`wall-post-${p.id}`}>
                        <div className="flex items-start justify-between gap-2">
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    <div className="text-sm font-semibold text-primary-k">{p.author_name}</div>
                                    <div className="text-[11px] text-primary-k/60">{formatPostTime(p.created_at)}</div>
                                </div>
                            </div>
                            {p.author_id === user?.id && (
                                <button onClick={() => del(p.id)} className="text-primary-k/60 hover:text-primary-k p-1 rounded hover:bg-primary-k/10" title="Delete" data-testid={`wall-delete-${p.id}`}>
