    ...
    return (
        <div className="space-y-6" data-testid="family-wall-page">
            ...
            <div className="space-y-3" data-testid="wall-feed">
                {posts.map((p, pi) => (
                    <article key={p.id} className="rounded-2xl p-4 space-y-3 force-white shadow-sm" style={{ backgroundColor: ["#0E4D52","#A5512B","#425F47","#5F4E76","#B23A2E"][pi % 5] }} data-testid={`wall-post-${p.id}`}>
                        <div className="flex items-start justify-between gap-2">
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <div className="text-[11px] opacity-75">{new Date(p.created_at).toLocaleString()}</div>
                                </div>
                            </div>
                            {p.author_id === user?.id && (
                                <button onClick={() => del(p.id)} className="opacity-80 hover:opacity-100 p-1 rounded hover:bg-white/15" title="Delete" data-testid={`wall-delete-${p.id}`}>
