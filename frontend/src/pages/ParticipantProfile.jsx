...
function ArtefactCard({ kind, artefact }) {
    ...
    return (
        <Link
            ...
            className="block rounded-xl border border-kindred bg-surface p-4 hover:shadow-md hover:border-primary-k/30 transition"
        ...
        >
            ...
            <p className="text-sm text-primary-k/75 line-clamp-2">{artefact.summary_line}</p>
            <div className="flex items-center gap-2 text-xs text-muted-k mt-2">
                <Clock className="w-3 h-3" aria-hidden />
                {formatDate(artefact.created_at)}
                {artefact.status && <span className="ml-auto px-2 py-0.5 rounded-full bg-surface-2 text-primary-k/70">{artefact.status}</span>}
            </div>
        </Link>
    );
}
...
function TimelineList({ events, showAll = false }) {
    ...
    // Collapsed by default: show a summary button; expand to reveal the list.
    if (!showAll && !open) {
        return (
            <button
                type="button"
            ...
            >
                <span>Show {major.length} key milestone{major.length === 1 ? "" : "s"}</span>
        ...
        <div className="max-w-6xl mx-auto p-6 lg:p-8" data-testid="core1-participant-profile">
            ...
            <header data-testid="core1-profile-header" className="mb-6 rounded-2xl border border-kindred bg-[#0E4D52]/[0.05] p-5 sm:p-6 shadow-sm">
                <div className="flex items-center gap-4 flex-wrap">
                    ...
                    <div className="sm:ml-auto"><ClassificationBadge classification={p.classification} /></div>
                </div>
                <p className="text-sm text-primary-k/60 mt-3 max-w-2xl">
                    ...
                    >
                        <summary className="flex items-center justify-between mb-4 cursor-pointer list-none [&::-webkit-details-marker]:hidden">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <ChevronDown className="h-4 w-4 text-primary-k/50 transition-transform group-open:rotate-180" aria-hidden />
                            </div>
                        </summary>
