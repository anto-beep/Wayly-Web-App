    ...
    return (
        <div
            className={`rounded-2xl p-4 space-y-3 ${adjusted
                ? "panel-clay ring-2 ring-[#A5512B]/40 shadow-md"
                : "panel-teal shadow-sm"}`}
            data-testid={testid}
        >
            <div className="flex items-center justify-between">
                <p className={`text-xs uppercase tracking-wider font-bold ${adjusted ? "text-clay-k" : "text-primary-k"}`}>{title}</p>
                {adjusted
                    ...
                    : <span className="text-[11px] rounded-full bg-primary-k text-white px-2.5 py-0.5" data-testid={`${testid}-badge`}>Class {data.classification}</span>}
            </div>
