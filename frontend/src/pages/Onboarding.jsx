...
import { GaugeRing, Orb } from "@/components/BrandVisuals";
    ...
    return (
        <div className="min-h-screen bg-kindred">
            ...
            <div className="mx-auto max-w-3xl px-4 md:px-6 py-6 md:py-10">
                ...
                {/* Progress ring glow-up */}
                <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#0E4D52] to-[#10363A] p-5 mb-6 flex items-center gap-5" data-testid="onboarding-progress-ring">
                    <Orb className="wayly-float" style={{ width: 170, height: 170, top: -70, right: -30 }} from="#F0B267" to="#A5512B" />
                    <GaugeRing pct={Math.round((step / STEPS.length) * 100)} size={96} stroke={9} label={`${Math.round((step / STEPS.length) * 100)}%`} sub="set up" />
                    <div className="relative flex-1 min-w-0">
                        <div className="text-[11px] uppercase tracking-[0.16em] text-white/70">Getting you set up</div>
                        <div className="font-heading text-xl text-white mt-1 truncate">{STEPS[step - 1].label}</div>
                        <div className="text-sm text-white/80 mt-1">Step {step} of {STEPS.length} · a few gentle questions, nothing tricky.</div>
                    </div>
                </div>
                {/* Stepper */}
