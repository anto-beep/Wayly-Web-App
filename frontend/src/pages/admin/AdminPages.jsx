                ...
                {!detail ? <p style={{ color: "var(--admin-muted)" }}>Loading…</p> : (
                    <>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 16 }}>
                            ...
                            <div className="admin-card" style={{ padding: 10, background: "var(--admin-bg)" }} data-testid="admin-email-verified-card">
                                <div style={{ fontSize: 10, textTransform: "uppercase", color: "var(--admin-muted)", letterSpacing: "0.06em" }}>Email verified</div>
                                <Badge tone={u.email_verified ? "green" : "amber"}>
                                    {u.email_verified ? (u.verification_grandfathered ? "Yes (legacy)" : "Yes") : "No"}
                                </Badge>
                            </div>
                        </div>
                        <div style={{ marginTop: 20, paddingTop: 16, borderTop: "1px solid var(--admin-border)" }}>
                            ...
                            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                                ...
                                <button
                                    disabled={busy}
                                    onClick={() => act(
                                        u.email_verified ? "Email verification revoked" : "Email verified (admin override)",
                                        () => adminApi.put(`/admin/users/${userId}/email-verified`, { email_verified: !u.email_verified })
                                    )}
                                    className="admin-btn admin-btn-secondary"
                                    data-testid="admin-action-toggle-email-verified"
                                    title="Override the user's email_verified flag, for emergency unlocks or QA cleanup"
                                >
                                    {u.email_verified ? "Unverify email" : "Mark email verified"}
                                </button>
