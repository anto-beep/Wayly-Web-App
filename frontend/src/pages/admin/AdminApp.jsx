...
function AdminShell({ children }) {
    ...
    // Shared theme state (top-bar + /admin/preferences stay in sync via
    // a custom "wayly:admin-theme" event). Default = "light".
    ...
    return (
        <div className="admin-root" data-theme={theme} data-density={density} style={{ display: "flex", minHeight: "100vh" }}>
            {/* Mobile overlay */}
            {drawerOpen && (
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
                <header className="admin-header" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px", position: "sticky", top: 0, zIndex: 20 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                        <div className="admin-theme-toggle" role="group" aria-label="Theme">
                            <button aria-pressed={theme === "light"} onClick={() => setTheme("light")} data-testid="admin-theme-light" title="Light">L</button>
                            <button aria-pressed={theme === "system"} onClick={() => setTheme("system")} data-testid="admin-theme-system" title="System">S</button>
                            <button aria-pressed={theme === "dark"} onClick={() => setTheme("dark")} data-testid="admin-theme-dark" title="Dark">D</button>
                        </div>
...
import { P0P1_ROUTES, P0P1_BUILT_PATHS } from "./AdminPhaseB";
    ...
    return (
        <AdminShell>
            <Routes>
                ...
                {/* P0 + P1 pages (AdminPhaseB) */}
                {P0P1_ROUTES.map((r) => <Route key={r.path} path={r.path} element={r.element} />)}
                {/* Fallback placeholders for any nav entry that remains unbuilt */}
