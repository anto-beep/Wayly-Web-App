...
export default function Reports() {
    ...
    return (
        <div className="space-y-6" data-testid="reports-page">
            <PageIntro
                eyebrow="Reports"
                title={`Reports${activeParticipant ? ` · ${activeParticipant.first_name}` : ""}`}
                            ...
                            <tbody>
                                {items.map((r) => (
                                    <tr key={r.id} className="border-t border-kindred" data-testid={`report-row-${r.id}`}>
                                        ...
                                        <td className="px-4 py-3 text-xs text-muted-k">{formatDateTime(r.created_at)}</td>
                                        <td className="px-4 py-3">
    ...
    return (
        <div className="space-y-4">
            ...
            <Section title="Recent concerns">
                <Table head={["Date", "Type", "Headline", "Status"]} rows={(data.recent_concerns || []).map((c) => [formatDate(c.created_at), c.type || ", ", c.title || c.headline || ", ", c.status || "open"])} empty="No concerns recorded." />
            </Section>
            <Section title="Hospitalisation (last 12 months)">
                {data.hospitalisations?.length ? (
                    <Table head={["Admitted", "Discharged", "Hospital", "Duration", "RCP"]} rows={data.hospitalisations.map((h) => [formatDate(h.admitted_at), h.discharged_at ? formatDate(h.discharged_at) : ", ", h.hospital_name || ", ", h.duration_days ? `${h.duration_days} d` : ", ", h.rcp_requested ? "yes" : "no"])} />
        ...
        <div className="space-y-4">
            ...
            <Section title="Timeline of anomalies">
                {data.timeline?.length ? (
                    <Table head={["Date", "Severity", "Headline", "Value", "Status"]} rows={data.timeline.map((t) => [t.date ? formatDate(t.date) : ", ", <Badge key={t.headline + t.date} sev={t.severity} />, t.headline, money(t.value), t.status])} />
    ...
    if (data.locked) {
        return (
            <Section title="Provider Performance">
                <Empty>This report requires at least {data.statements_needed} decoded statements. You have {data.statements_available}. Keep decoding.</Empty>
            </Section>
        );
    }
    return (
        ...
        <div className="space-y-4">
            ...
            <Section title="Concerns">
                {data.concerns?.length ? (
                    <Table head={["Date", "Type", "Title", "Severity", "Status"]} rows={data.concerns.map((c) => [formatDate(c.created_at), c.type || ", ", c.title || c.headline || ", ", c.severity || ", ", c.status || "open"])} />
                ) : <Empty>No concerns recorded in this period.</Empty>}
            </Section>
            <Section title="Correspondence history">
                {data.correspondence?.length ? (
                    <Table head={["Date sent", "Type", "Recipient", "Response"]} rows={data.correspondence.map((c) => [c.sent_at ? formatDate(c.sent_at) : ", ", c.type || ", ", c.recipient || ", ", c.response_received_at ? formatDate(c.response_received_at) : "No response"])} />
                    ...
                    {data.events.map((ev, i) => (
                        <div key={i} className="flex gap-3 p-3 bg-surface border border-kindred rounded-xl">
                            ...
                            <div className="flex-1">
                                <div className="text-sm font-medium text-primary-k">{ev.date ? formatDate(ev.date) : ", "} · {ev.headline}</div>
