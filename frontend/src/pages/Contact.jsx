...
export default function Contact() {
    ...
    const submit = async (e) => {
        ...
        try {
            ...
            setSent(true);
            toast.success(isDemo ? "Booked, we will be in touch within one business day." : "Thanks, we will be in touch within one business day.");
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <div className="min-h-screen bg-kindred">
            <SeoHead {...SEO.contact} />
            <MarketingHeader />
            <section className="relative overflow-hidden mx-auto max-w-6xl px-6 pt-10 pb-6" data-testid="contact-page">
                <Orb className="wayly-float" style={{ width: 260, height: 260, top: -110, right: -60 }} from="#DCEAE9" to="#C4D4C8" />
                <Orb className="wayly-float-slow" style={{ width: 200, height: 200, bottom: -100, left: -60 }} from="#F3E7DE" to="#E9D4C6" />
                <div className="relative">
                    <span className="overline">{isDemo ? "Book a demo" : "Contact"}</span>
                    <h1 className="font-heading text-4xl sm:text-5xl text-primary-k tracking-tight mt-3 leading-tight" data-testid="contact-heading">
                        {isDemo ? "A 20‑minute walkthrough." : "Talk to a real person."}
                    </h1>
                    <p className="mt-3 text-base text-muted-k max-w-2xl leading-relaxed">
                        {isDemo
                            ? "We will show you how Wayly decodes statements, watches the budget, and keeps the whole family on the same page, using a sample household so you can see it work end‑to‑end. No card, no commitment."
                            : "Fill in the form for a written reply, or scroll down to pick a time and we will meet you there."}
                    </p>
                </div>
            </section>

            <section className="mx-auto max-w-6xl px-6 pb-10 grid lg:grid-cols-3 gap-6 items-start">
                            ...
                            {isDemo ? (
                                ...
                                <label className="block">
                                    ...
                                    <textarea
                                        ...
                                        className="mt-1 w-full rounded-md border border-kindred bg-surface px-3 py-2.5 text-sm placeholder:text-sm focus:outline-none focus:ring-2 ring-primary-k"
                                    />
                                </label>
                            )}
                ...
                <aside className="space-y-3" data-testid="contact-info-cards">
                    ...
                    <div className="relative overflow-hidden bg-surface-2 border border-kindred rounded-2xl p-5">
                        ...
                        <div className="h-10 w-10 rounded-xl bg-terracotta flex items-center justify-center shadow-sm"><Phone className="h-5 w-5 text-white" /></div>
            ...
            <section className="mx-auto max-w-6xl px-6 pb-16" data-testid="contact-calendly-section">
                <div className="bg-surface border border-kindred rounded-2xl overflow-hidden">
                    <div className="px-6 pt-6 pb-4 border-b border-kindred flex items-start gap-3">
                        <Calendar className="h-5 w-5 text-primary-k mt-1 shrink-0" />
                        <div>
                            <h2 className="font-heading text-2xl text-primary-k tracking-tight">Book a 30‑min call</h2>
                            <p className="mt-1 text-sm text-muted-k">Pick a time that suits you, the call is a friendly walkthrough tailored to your situation.</p>
                        </div>
                    </div>
