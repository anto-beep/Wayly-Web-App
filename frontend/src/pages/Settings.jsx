                ...
                <div className="panel-solid-teal force-white rounded-2xl p-6" data-testid="current-plan-card">
                    <div className="flex items-start justify-between flex-wrap gap-4">
                        <div>
                            ...
                            {activeSub && sub.trial_ends_at && new Date(sub.trial_ends_at).getTime() > Date.now() && (() => {
                                const ms = new Date(sub.trial_ends_at).getTime() - Date.now();
                                const days = Math.max(0, Math.ceil(ms / 86_400_000));
                                const ending = days <= 2;
                                return (
                                    <div className="mt-3 rounded-xl bg-[#F0B267] px-4 py-3 shadow-sm" data-testid="billing-trial-remaining">
                                        <div className="flex items-center justify-between gap-3 flex-wrap">
                                            <div className="flex items-center gap-2 text-[#0E2A2E]">
                                                <Sparkles className="h-4 w-4" />
                                                <span className="text-sm font-bold">Free trial · {days} day{days === 1 ? "" : "s"} left</span>
                                            </div>
                                            <span className="text-xs font-semibold text-[#0E2A2E]/75">
                                                {ending ? "Ends " : "Ends "}{new Date(sub.trial_ends_at).toLocaleDateString("en-AU", { weekday: "short", day: "numeric", month: "short" })}
                                            </span>
                                        </div>
                                        <p className="mt-1.5 text-xs text-[#0E2A2E]/80">
                                            {ending
                                                ? "Your trial is almost up. You will move to your chosen plan when it ends. Cancel any time before then."
                                                : "You will not be charged until your trial ends. Cancel any time before then."}
                                        </p>
                                    </div>
                                );
                            })()}
                        ...
                        {activeSub && !sub.cancel_at_period_end && (() => {
                            const isTrialing = sub?.trial_ends_at && new Date(sub.trial_ends_at).getTime() > Date.now();
                            return (
                                <button
                                    onClick={cancel}
                                    disabled={busy}
