...
- `curl https://wayly-care-review.preview.emergentagent.com/sitemap.xml` returns 200 with the correct XML (113 URLs).
...
- **Env** — `FRONTEND_URL=https://wayly-care-review.preview.emergentagent.com` added to backend `.env` so verification links point at the right host. (Production needs the same key set to `https://wayly.com.au`.)
