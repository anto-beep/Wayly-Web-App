...
**Just shipped (Iter 329, web + shared backend, verified iteration_327):** Dashboard Wayly Summary now matches the budget graphics (over-budget wording, never "available to spend" when over) and counts only the LATEST statement's anomalies; Things To Know split into Statements + Invoices sections scoped to the latest of each ("Based on your latest statement and invoice"); statement-detail summary enriched with real cross-tool data (complaints, letters, latest invoice); STRICT document blocking on Statement Decoder + Care Plan Reviewer (full-screen block, no numbers, never "continue anyway"); Care Plan Reviewer "Wayly Summary" + "what we could not check"; Letters renamed "Letters & Follow-Ups" with highlighted situation prompt, tidied labels, fields locked while generating, and NO draft until "Generate letter" is clicked; Ask Wayly answers use short paragraphs + inline bold and "Report an Issue" moved below the chat bar. See CHANGELOG Iter 329.

**Next up (backlog):** MOBILE UI parity for all of Iter 329 (dashboard Things To Know, strict block screens, care plan Wayly summary, letters editor lock/no-auto-draft, Q&A report button + formatting); Invoice Checker prominent wrong-doc prompt; Statement Decoder remove "Forward by email"; Care Plan Reviewer draft-email + auto-save + capitalise "Your Saved Plans"; Provider Price Checker polish (capitalisation, prefill provider, required fields, "Your Price History" button); Classification Self-Check polish (required fields, persona headings, save/name/delete/edit/re-run, button colours, hover glitch); convert aw2 STREAMING Q&A off-loop.
