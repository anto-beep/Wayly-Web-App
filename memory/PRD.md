...
**Just shipped (Iter 344-346, Jun 2026 — Wizard Everywhere + Mobile Estimator parity + Phase G Overcharge Alerts; web+mobile+backend):**
- **Classification Self-Check → 3-step wizard (WEB, verified iter344):** rebuilt `pages/tools/ClassificationCheck.jsx` from a single long form into a premium guided wizard — Step 1 Personal Care (teal/Droplet/rings, Q1-4), Step 2 Everyday Tasks (sage/Home/waves, Q5-9), Step 3 Health/Mind & Safety (terracotta/HeartPulse/pie, Q10-16). Sticky stepper (`csc-stepper`, `csc-step-care/home/health`) with clickable jumps, per-step icon+SVG-motif header, Continue/Back nav (`csc-wizard-next`/`csc-wizard-back`), submit only on last step (`csc-submit`, disabled until all 16 answered). Swapped the plain `<h1>` for the shared `<ToolHero ... wide />` banner. All prior logic intact (autosave/resume, saved checks, results, PDF/email). Fixed the `<span> in <option>` hydration warning. All 11 acceptance criteria PASS.
- **Contribution Estimator → 3-step wizard (MOBILE, verified iter345):** refactored `mobile/src/components/tools/ContributionEstimator.tsx` from a flat form into the same 3-step wizard as web (Step 1 Your Situation teal/Compass, Step 2 Your Money gold/Wallet, Step 3 Service Mix sage/SlidersHorizontal) using native RN primitives (View/Text/Pressable/StyleSheet), theme-aware (dark-mode safe). Stepper (`ce-stepper`, `ce-step-situation/money/mix`), Continue/Back (`ce-next`/`ce-back`), `ce-submit` only on last step, scroll-to-top on step change. All logic + testIDs preserved (CE state save/load, /ce2/calculate, result screen). Full E2E parity PASS.
- **Phase G · Overcharge Alerts (backend + both surfaces, verified iter346 — FINAL item of the 17-item batch):** NEW `GET /api/invoices/overcharge-alerts` (`routes/invoices.py`, registered BEFORE the parameterized `/invoices/{invoice_id}` route) flags providers who overcharged on 2+ invoices in the user's history. Overcharge checks = C1/C2/C4/C6/C11/C12 at tier>=3; returns `{count, threshold:2, alerts:[{provider_name, invoice_count, total_invoices, total_amount, check_types, invoices, last_seen}]}`. NEW shared banner component — web `components/invoices/OverchargeAlerts.jsx` (panel-terracotta tokens) + mobile `src/components/invoices/OverchargeAlerts.tsx` (theme-aware) — rendered atop the invoice history (web `/app/invoices` InvoicesList; mobile `/invoices`), each with `overcharge-alerts`, `overcharge-alert-{idx}`, `overcharge-count-{idx}`, `overcharge-view-{idx}` (Show-these filters the list to that provider). Hides when no repeated-overcharge providers. 401 without auth; route ordering keeps bogus-id 404.
- **This completes the full 17-item batch (Phases A-G).** Known non-blocker carried over: React hooks/exhaustive-deps eslint warnings (pre-existing, non-blocking) on ClassificationCheck/LettersFollowUps/ReassessmentLetter.

**Just shipped (Iter 343, Jun 2026 — Illustrations Rollout + recap P0 + full-width tool pages; WEB):**
- **P0 fix — Contribution Estimator "Estimate Recap":** the classification chip was gated on `assessment_status === "classified"` (a value that never exists) so it never rendered; corrected to `"have_classification"`. Verified via testing_agent iter343: recap now shows the Class chip (e.g. "Class 5").
- **Illustrations Rollout (web):** every AI tool page now uses the shared premium `<ToolHero>` banner (soft brand-tinted banner + inline-SVG motif + accent icon badge) instead of bespoke `<h1>` headers. Swapped: ContributionEstimator, BudgetCalculatorTool, PriceCheckerTool, StatementDecoderTool, CarePlanReviewer, FamilyCoordinator, LettersFollowUps, ReassessmentLetter (InvoiceChecker + StatementDecoder already used it). Preserved each tool's affordances beneath the banner (Price Checker: `pc-history-link` + `pc-how-it-works-link`; Letters: `lf1-log-link`; Care Plan Reviewer: `link-saved-plans`; AboutBackLink where present).
- **Full-width tool pages (user request):** ALL AI tool pages widened to `max-w-[1720px]` with `<ToolHero ... wide />` to match Statement Decoder + Invoice Checker (heroes + main form/result containers; ToolExplainer/RelatedLinks stay `max-w-5xl` as in the reference tools). Includes ClassificationCheck sections (its header ToolHero swap still deferred to the wizard rebuild).
- **2 HIGH bugs fixed (from iter343 report) + screenshot-verified:**
  - `ReassessmentLetterRedirectOrPage` (App.js) wrongly redirected logged-in users to the Letters hub because it read `user` before `/auth/me` resolved — now waits on `loading` first.
  - `TOOL_CONTENT["reassessment-letter"].name` was mistakenly "Letters & Follow-ups" (+ wrong one-liner); corrected to "Reassessment Letter Drafter" with a matching heroOneLiner.
- **Also:** InvoiceChecker upload heading title-cased ("Upload Your Invoice") per an inline visual edit.
- **STILL PENDING (this batch's remaining plan):** Wizard Everywhere — rebuild Classification Self-Check into a 3-step guided wizard (and swap in its ToolHero header there); Mobile Contribution Estimator wizard (parity); Phase G — Overcharge Alerts (P2). Known non-blocker: console hydration warning `<span> cannot be a child of <option>` on some tool `<select>`s.

**Just shipped (Iter 341, Jun 2026 — 17-item batch Phases E/D/F + nav move; web+mobile+backend, verified iteration_341):**
- **Phase E · Premium Motion (Calm/Headspace feel, both surfaces):** Web added soft, reduced-motion-safe keyframes to `index.css` (`wayly-fade-up`, `wayly-fade-in`, `wayly-breathe`, `wayly-aurora` slow gradient drift, stagger delay helpers). Applied to `Login.jsx` (drifting aurora background + breathing orbs + card fade-up entrance), `CaregiverDashboard.jsx` (fade-in + staggered greeting), and the `App.js` `Loading` app-open screen (breathing Wayly mark on aurora). Mobile used `react-native-reanimated`: `app/index.tsx` rebuilt into a `CalmSplash` (breathing brand mark, FadeIn) for app-open; `app/login.tsx` brand mark + form `FadeInDown`; `app/(tabs)/index.tsx` greeting + action bar + AI summary `FadeInDown` stagger.
- **Phase D · Tailored Onboarding (both surfaces):** New "Whose care are you setting up?" question — self vs someone else. Self-managed HIDES the "relationship to the participant" field and sets `caregiver_relationship="self"`, prefills the account holder's name, and adapts copy (about you vs about the participant). Web: `Onboarding.jsx` (selfManaged state threaded to `StepEssentials`/`StepRecommended`), testIDs `onboarding-whose-care`, `onboarding-whose-care-self|other`. Mobile: `app/onboarding.tsx` same, relationship chips gated on Step 3.
- **Phase F · Bulk Findings Letter PDF (both surfaces + backend):** New `POST /api/chsp1/findings-letter/pdf` (`routes/chsp1.py` + `lib/chsp1/chsp_pdf.py::render_findings_letter_pdf`) renders a branded Wayly PDF matching the other CHSP artefacts (401 no-auth, 400 blank letter, 200 attachment `Wayly-CHSP-Findings-Letter_<provider>.pdf`). Web + mobile `OverchargeLetterModal` gained a "Download PDF" button (`chsp-letter-download-pdf`).
- **Nav move:** Family Wall removed from the "Their Care" web sidebar group (`Layout.jsx`); now only under "Today" (mobile already had it only under Today).
- **Verified:** testing_agent iteration_341 — all PASS on web+mobile+backend (new pytest `tests/test_chsp_findings_letter_pdf_iter341.py`, 3 green). No parity gaps.
- **STILL PENDING from the 17-item batch (user order E→D→F→C→G):**
  - **Phase C (P0) · Family Wall restyle + Timeline redesign** — restyle Family Wall (`FamilyWall.jsx` + mobile `family.tsx`) to the calm old-Care-Team look; redesign Timeline (`ParticipantTimeline.jsx` + mobile `timeline.tsx`) so items are collapsed by default with explicit filter/search.
  - **Phase G (P2) · Overcharge Alerts** — flag repeated provider overcharges in invoice history.
  - Minor P2 nit: match web Calendar required-field labels to mobile ("Required").
  - Item #15 (letter delivery via email) is CANCELLED — do NOT build.



**Just shipped (Iter 336, Jun 2026 — AT/HM + CHSP overhaul + form-labelling + mobile ReportSheet refactor; web+mobile+backend, verified iteration_336):**
- **AT/HM Projects (web `pages/AthmProjects.jsx` + mobile `app/athm.tsx`):** 'New Project' is now a centered, wide MODAL (Title Case), with explicit required-field `*` markers, a Project-type dropdown, a Primary Needs dropdown (`athm-project-need-select`) that pre-fills an EDITABLE need-summary, and invoice/document upload DURING creation (`athm-project-file-pick` → uploaded to Document Vault `ot_referral` then attached to the new project). Project list gained colour-toned cards + type filter (`athm-filter-type-*`) and status filter (web `athm-filter-status-*`; mobile horizontal chip scroller `athm-type-filter`). Item + HM-modification add forms now use category dropdowns (with 'type your own') + required markers.
- **CHSP Tools (web `pages/ChspTools.jsx` + mobile `app/chsp-tools.tsx` + backend `routes/chsp1.py`):** NEW invoice upload+parse — `POST /api/chsp1/fee-check/parse-invoice` reuses `document_extract` + the shared LLM (`llm_wrapper.chat_send`, claude-haiku-4-5) to read an uploaded invoice and return `{fields, plain_summary, next_steps}` which pre-fill the WS-1 Fee Check (`chsp-ws1-upload` → `chsp-ws1-parse-summary` + `chsp-ws1-next-steps`). Rate-effective + billed-period fields are now native date pickers (web `<input type=date>`); explicit required `*` markers on service-type/units/amount. NEW save/list/delete: `POST /api/chsp1/fee-check/save`, `GET /fee-check/saved`, `DELETE /fee-check/saved/{id}` (collection `chsp_saved_checks`, user-scoped) drive a 'Save this check' button + a 'Past checks' accordion (`chsp-ws1-past`). Page reskinned with colour-blocked (teal/clay/sage) cards instead of all-white.
- **Global required-field labelling:** AT/HM + CHSP forms now mark required fields (`*`); the app's canonical `components/RequiredHint.jsx` (Required/Optional badges) already covers Participants/Signup/HandoverPack/Amendments/Onboarding. Mobile uses `required`/`optional` labels via shared Field/Select + LInput.
- **Mobile refactor:** `app/(tabs)/ask.tsx` now uses the shared `src/components/ReportSheet.tsx` (identical testIDs) instead of a duplicated inline sheet — verified end-to-end (real ticket WAY-0044 minted).
- **Re-verified (user-requested clean pass):** web Onboarding DOB `DateField` auto-masks typed digits → DD/MM/YYYY (+ calendar fallback, emits ISO); Aged Care Q&A answers render bold highlight keyphrases + a 'Next step' link (live-confirmed).
- **Verified:** testing_agent iteration_336 — all 10 review items PASS on web + mobile + backend (4 new pytest in `tests/test_iter336_chsp_athm.py`). Optional non-blocker: web uses `*` vs mobile 'Required' badge styling (both satisfy the rule).



**Just shipped (Iter 334, Jun 2026 — re-reported batch: mobile onboarding blocker + Aged Care Q&A + letters gating; web+mobile+backend):**
- FIXED P0 mobile onboarding blocker — the DOB field (`mobile/src/components/ui.tsx` DateField, Expo-web input) never validated typed digits, so `onboarding.tsx` step1Valid stayed false and users were stuck on 'Set up your household'. Now auto-masks digits → DD/MM/YYYY and emits a valid ISO date so Continue enables. (NOTE: fresh mobile signup is gated behind Stripe before onboarding — pre-existing product flow, not a regression.)
- Aged Care Q&A (web `FamilyCoordinator.jsx`, /ai-tools/family-coordinator) — answers render as markdown (bold/links/short paragraphs) via shared MarkdownText instead of raw `**`/`[](...)`; the 'Something not right? / Report an Issue' button moved BELOW the input bar (outside the answer card). Backend `AGED_CARE_QA_SYSTEM` prompt (server.py ~6120) rewritten for short, scannable, **bold**-highlighted answers ending in a **Next step:** line.
- Letters required-field GATE (web `CorrespondenceDetail.jsx` + mobile `letters/[id].tsx`) — drafting is blocked until all required intake fields for the situation are filled: Generate disabled + `lf1-required-missing` list (+ `lf1-required-gate` wrapper, `lf1-required-missing-jump` link). Shared `missingRequiredIntake()` archetype map. Fixes 'still generating with Choose selected'.
- Letter title capitalisation (web+mobile) — situation title capitalised to match the Letters & Follow-Ups cards via `capitaliseSituation`.
- Auto-address letters (web+mobile) — one-tap `lf1-recipient-use-provider` in the recipient directory pre-addresses to the participant's saved provider_email (now mapped into mobile ParticipantContext; web /account and /participants already return it).
- GoogleSignInButton — removed the redundant 'Sign in with Google' caption (login + signup).
- Verified by testing_agent iter334: all P0s PASS on web + mobile. Report: /app/test_reports/iteration_334.json. Known remaining parity gap: the mobile PUBLIC Aged Care Q&A tool (`AgedCareQA.tsx`) has no in-tool report button yet (web has one).



**Just shipped (Iter 333, Jun 2026 — participant-guard SURFACED + mobile parity + price-checker sanity redesign; web+mobile+backend):**
- **Participant/Identity Guard now VISIBLE on the client.** The backend already returned `identity_warning` (anti account-sharing: name on uploaded doc vs active participant/household) but NOTHING rendered it. Added a prominent terracotta alert banner on the Statement Decoder run (`decoder-identity-warning`) and Invoice Checker (`inv1-identity-warning`) on BOTH web (`StatementDecoderTool.jsx`, `InvoiceCheckerTool.jsx`) and mobile (`app/tool/[slug].tsx`, `components/invoices/InvoiceResultView.tsx`). Advisory/non-blocking. Guard logic (`lib/identity_guard.py`) verified deterministically: mismatch→`no_match` warn, active-match→clear, other-participant→`other_participant` named warn, no-name→`no_name` warn.
- **Provider email on Participant details** (user's chosen replacement for a standalone Recipient Directory screen): optional `provider_email` field on the add + edit participant forms — web `form-provider-email`/`edit-provider-email` (`pages/extended/Participants.jsx`), mobile `form-provider-email` (`app/participants.tsx`). Persisted via `/api/v2/participants` (batch3_models/routes) AND `/api/participants/{id}` (participant_profile). Both curl-verified.
- **Provider Price Checker "Quick sanity check" redesigned** to stand out (terracotta/clay/teal accent bar + icon badge, `border-2 terracotta`) with a high-contrast SOLID-TEAL "Continue anyway" button that reads in light & dark — web `PriceCheckerTool.jsx` `QualityGuardPanel` (`pc-guard-continue` rgb(14,77,82); after-hours solid sage) + mobile `components/tools/ProviderPriceChecker.tsx` `pc-quality-guard`. Both runtime-verified via screenshots.
- **Mobile Notifications:** viewing a notification now REMOVES it from the list and "Mark all read" CLEARS the list (parity with web; user chose gone-from-list). `src/context/NotificationsContext.tsx` (reload filters unread; markRead drops item; markAllRead → []).
- **Mobile letter editor** (`app/letters/[id].tsx`): added inline body editing (`lf1-edit-letter` → `lf1-generated-body-edit` TextInput, edits persist on Save as draft) + "Open in email" mailto launcher (`lf1-open-in-email`, pre-addressed from `recipient_specific.email`) — parity with web `OutputFormatSwitcher`.
- **Verified:** testing_agent iter333 (web price-checker guard PASS + full source audit of all testIDs, no regressions); self-verified web add-participant `form-provider-email` field renders (Family add flow gates the form behind a billing step), mobile price-checker guard runtime screenshot, identity-guard logic unit test, backend provider_email persistence curl. Mobile notifications remove/clear + letter-editor edit/open-in-email are source-verified mirrors of the verified web behavior (not yet exercised in an Expo runtime pass).
- **NOTE:** the full LLM statement-decode E2E for `identity_warning` was not run to completion — the LLM decode is slow and repeated runs tie up the backend executor (known ROADMAP LLM-contention tech-debt). Guard logic + wiring proven separately.



**Infra (Jun 2026 — Expo SDK 54 → 57, verified iteration_331):** Mobile app upgraded to Expo SDK 57 (React Native 0.86.3, React 19.2.3, react-native-reanimated 4.5.1, react-native-worklets 0.10.1, expo-router 57, expo 57.0.22) so it opens in the latest Expo Go. Breaking-change fixes: expo-router 57 dropped its `@react-navigation/native` dep — `useScrollToTop` now imported from `expo-router` (4 tab screens); `usePreventRemove` in `app/letters/[id].tsx` replaced with a `navigation.addListener("beforeRemove", …)` listener. `app.json` cleaned for the SDK 57 schema (removed `newArchEnabled` + `android.edgeToEdgeEnabled`, now defaults). Deprecated colored-button `shadow*` in `src/components/ui.tsx` converted to `Platform.select` `boxShadow`. SDK-54 backup of package.json/yarn.lock kept at `/app/.sdk_backup/`. Full-app mobile regression passed (all screens render, 0 runtime errors). Note: the native unsaved-changes leave prompt (beforeRemove) can only be fully validated in Expo Go on a device, not the web preview.

**Just shipped (Iter 330, Jun 2026 — mobile parity + 2 new P1 features, web+mobile+backend, verified iteration_330):**
- **Admin readability bug VERIFIED FIXED** (iteration_329): the MutationObserver in `frontend/src/pages/admin/AdminApp.jsx` (`useSyncHtmlToAdminTheme`) re-asserts the admin light theme whenever the consumer app mutates `<html>`; 4 consecutive hard-refreshes stay light + readable, consumer dark theme restored on exit.
- **MOBILE parity with Iter 329**: dashboard now has a prominent budget-status pill (`dashboard-budget-status`/`glance-budget-status`, over-budget vs left-to-spend), Things To Know split into Statements (`ttk-statements-section`) + Invoices (`ttk-invoices-section`) sections scoped to the LATEST of each ("Based on your latest statement and invoice", `things-to-know-count`), and a collapsible Wayly Chat card (`chat-preview-toggle`). SmartAISummary context now budget-accurate + latest-only (over_budget, budget_percent_used, amount_over/remaining, latest_statement_open_anomalies, latest_invoice_issue_count). Letters editor: NO auto-draft (draft only appears after tapping "Generate letter"), intake fields locked while generating, button relabelled "Generate letter"/"Regenerate letter". Care Plan Reviewer: overview card retitled "Wayly Summary" (Sparkles) + "What we could not check" gaps list (`cp-couldnt-check`) + bundle button relabelled "Draft an email to your provider". Decoder: upload_guard wrong-doc block wired.
- **NEW P1 · Recipient Directory** (web+mobile+backend): save a provider's name+email once so every future letter is pre-addressed. Backend CRUD `GET/POST/PATCH/DELETE /api/lf1/recipients` (collection `lf1_recipients`, user-scoped, de-dupes by name+email). Letter editor "Deliver to" card (`lf1-recipient-directory`) saves + selects recipients; selecting one PATCHes the entry's `recipient_specific` which `build_cover_note()` consumes so the drafted letter is addressed correctly.
- **NEW P1 · Blocker Letters Bundle** (web+mobile): on a publish-blocked statement the decoder block panel shows a single "Draft one email to your provider covering all N blockers" button (`decoder-publish-block-email-all`, renders when N>1) that POSTs every blocker to the existing `/api/care-plans/letter-from-findings` and opens ONE consolidated letter. Wired on mobile decoder tool + saved statement detail, and web StatementDecoderTool + StatementDetail.
- **P2 · Short-Term Pathways**: already built + wired on web (`/app/short-term-pathways`) and mobile (`/short-term-pathways`, nav group) from a prior session — verified present.

**Next up (backlog):**
- **Production DB test-account cleanup** — `backend/scripts/prod_cleanup_test_accounts.py` + runbook `backend/scripts/README_prod_cleanup.md` are READY but MUST be run against the PRODUCTION Mongo (preview uses local `test_database`, cannot run from here). Also: keep preview and prod on SEPARATE `DB_NAME` values (deploy-config action) so test data never leaks into prod again.
- Optional: mirror the web "Report an issue" below-the-chat-bar button onto mobile Q&A (mobile already has per-message "Report incorrect" + markdown formatting, so largely at parity).
...
**P1 — next**: transparency flags (surface Wayly's OWN recategorisations/reclassifications as "Wayly changed X→Y based on INDEX-1 rule Z", never as a provider error — esp. the shower-stool AT-HM move and the group-social→Independence move); one-issue-one-flag unbundling (split the aggregate "Everyday Living doesn't add up" into its root causes); low-extraction-confidence banner (<0.7) at the top of the summary + per-line confidence notes; a small visual "cannot publish" panel on web + mobile keyed off `publishable`.

**P2 — after**: extraction robustness (stop dropping/merging/zeroing rows; preserve the source's OWN stream so recategorisation is detectable; recover package-management $178 + care-management $215 as lines; keep duplicates) + missing rule checks (cancellation-policy already via G6, travel double-billing, AT-HM pathway assessment, GST verify, category transition 01/10/2026 personal-care→Clinical) + INDEX-1 package-management 5% cap row (from 01/07/2026).

**Validation target**: the 18-item Sam Burke checklist. P0 + existing rules currently catch ~11/18 and BLOCK publication so no wrong numbers reach the user; P1/P2 close the remainder.
...
---
## Iteration 300 (Jun 2026) — Features + Landing premium visual rebuild, Journeys glow-up, premium buttons, P2 fixes
- **WEB Features page rebuilt** (`pages/Features.jsx`): brand colour-block sections (Teal/Clay/Sage full-bleed gradients with dot texture + floating orbs), an image-free hero "glance card" (animated gauge ring + compare bars + anomaly callout), colour-accented tool cards, staggered reveals. All testIDs preserved.
- **WEB Landing brought to the SAME visual language** (user changed mind, liked Features): Problem cards + How-It-Works steps now colour-accented tiles; Big-Number band is a teal gradient with dot field, orbs and an animated `$1,847/yr` gauge ring; Dashboard strip swapped the static PNG for the live on-brand `ScreenshotDashboard` mockup in a `BrowserFrame`; AI-tools teaser cards colour-accented. Kept structure + prior copy fixes. Shared primitives extracted to `components/BrandVisuals.jsx` (DotField, Orb, GaugeRing, CompareRow).
- **Landing inline-comment tweaks** (via annotations): DualFlagshipHero kicker → prominent teal pill; flagship eyebrows → standout coloured pills; ToolClusterGrid heading fixed to one colour across light/dark; How-It-Works mentions "statement or invoice"; how-it-works-cta → "Start your free 7-day trial"; reports strip redesigned into 8 colourful plain-English cards + A- grade callout (removed screenshot + CTA button); corrected misleading "free" copy in AI-tools teaser + tile labels ("7-day free trial, then Solo or Family").
- **Guided Journeys glow-up**: WEB `pages/journey/Journey.jsx` (decorative orbs, per-step brand colours/icons, colour persona cards, celebratory complete state); MOBILE `app/journeys.tsx` (LinearGradient hero + SVG progress ring, colour-coded step tiles, reanimated FadeInDown entrances).
- **Premium buttons system-wide**: WEB shadcn `Button` variants (gradient sheen, brand-tinted shadows, hover lift, active press) + global CSS feel (smooth transitions, subtle active-press, branded focus ring); MOBILE shared `Button` primitive (brand-tinted shadow, press-scale, letter-spacing). New motion keyframes in `index.css` (wayly-float/draw/grow-bar/pulse-ring), reduced-motion safe.
- **P2 dark-mode billing legibility** (`pages/Settings.jsx`): status-notice callouts (cancel-scheduled / pending-change / past-due) carried `bg-*` so `.force-white` skipped them, leaving dark ink on the teal panel — switched to white text on a dark tint so they read in both themes.
- **P2 expired-plan access_state** (`server.py::_compute_access_state`): legacy paid-plan fallback now drops to `view_only` when a recorded `current_period_end` has elapsed (never locks out genuinely active subs — they have status=active or a future period end).
- **Verified**: frontend compiles clean; backend healthy; testing_agent iter296 = web testIDs + billing dark-mode + mobile journeys PASS, backend access_state PASS for cathy(active)/cancelled.vo(view_only)/trial30909(view_only) — the only "fail" was trialactive fixture DATA DRIFT (DB has expired/free), not a code bug. Landing rebuild smoke-verified via screenshots (hero, how-it-works, big-number gauge, dashboard mockup, reports strip).
- **Known/ deferred**: re-seed `trialactive@example.com` for a deterministic trial fixture; cosmetic RN responder-prop console warnings on mobile journeys (web-preview only).


---
## Iteration 299 (Jun 2026) — Apple Sign-In on signup (matches Google flow)
- Added the native Apple button to `app/signup.tsx` (iOS-only, `signup-apple-button`, `SIGN_UP` type). `onApple` mirrors `onGoogle` EXACTLY: `loginWithApple()` → `router.replace("/(tabs)")` — no separate plan/billing gate, so Apple social signup follows the same process as Google social signup (the email/password path keeps its Stripe card gate; both social paths go straight into the app, as they did before). Verified: signup renders, Google present, Apple hidden on web (iOS-only) — compiles clean.
- Prod cleanup: `scripts/clean_prod_users.py` ready; must be run in a PRODUCTION context (not this local preview DB) after a backup. Not runnable from here.
- STILL PENDING (large, next pass): Landing/Features rebuild (web) + Guided Journeys overhaul (web + mobile) with custom brand graphics.


---
## Iteration 298 (Jun 2026) — Sign in with Apple (mobile/iOS) + env/DB clarification + prod cleanup tooling

- **Sign in with Apple (Expo/iOS)** — new backend `POST /api/auth/apple` (`server.py`, mirrors the Google social-login flow): verifies the Apple identity token server-side via Apple JWKS (`apple_auth.py`, audience = `APPLE_BUNDLE_ID`), then **links to an existing account by verified email or creates a new one** (stores `apple_sub`, `auth_method:"apple"`), and issues the app's own JWT + refresh token. Verified via curl (401 on invalid token). Mobile: `expo-apple-authentication` installed; `loginWithApple()` in `AuthContext.tsx`; native `AppleAuthenticationButton` on `app/login.tsx` (iOS-only, hidden on web preview by design); `app.json` updated (`ios.usesAppleSignIn`, `expo-apple-authentication` plugin). New env `APPLE_BUNDLE_ID=com.emergent.prorationpreview.x214np` in `backend/.env`.
  - REQUIRES a real iOS build (Expo Publish) + an Apple Developer App ID with "Sign in with Apple" enabled for that bundle id. Does NOT work in Expo Go or the web preview.
- **Environment/DB clarification** — this preview environment uses a LOCAL Mongo (`mongodb://localhost:27017` / `test_database`), NOT the production Atlas DB (`mongodb+srv://llm-timeout-fix…`). Production cleanup cannot be done from here; must be run against prod (via the Emergent MongoDB Viewer or a prod-context script). Preview DB setup is a platform action ("Configure Preview App" in the MongoDB Viewer).
- **Prod user cleanup tooling** — `scripts/clean_prod_users.py`: dry-run by default, keeps admins (hello@wayly.com.au, support@wayly.com.au, hello@techglove.com.au), `--apply` to delete non-admins, `--purge-data` to also remove their data. Validated in dry-run; NOT applied (local DB holds test accounts).

### Still pending (large, next pass) — user approved "do all four in one big pass"
- Landing & Features pages rebuild (bold brand panels, custom-generated brand graphics, white text) — web + mobile.
- Guided Journeys visual overhaul (colour, real graphics, gentle animation) — web + mobile.



---
## Iteration 297 (Jun 2026) — Decoder run/saved parity, statement duplicate detection, friendly rule labels, invoice per-issue letters

- **Fresh-run output == saved statement page (web).** Extracted `StatementInsightGraphics` (two donuts: "Where the money went" + "What we found") into a shared `components/StatementInsightGraphics.jsx` (removed the inline copy from `StatementDetail.jsx`). The Statement Decoder run output (`StatementDecoderTool.jsx`) now renders `SmartAISummary` ("Wayly Summary") + `StatementInsightGraphics` above the shared `DecoderResultView` — replacing the old ToolSummary — so the run and the saved page (`/app/statements/{id}`) look identical. Verified live.
- **Statement duplicate detection fixed & surfaced (web + mobile).** Root cause: `_persist_decoded_statement_for_user` (server.py) already deduped by `extracted_fingerprint` and silently returned the existing id, so the user got NO warning. Added a `dup_signal` out-param; `_run_decode_job` now attaches `result.duplicate_warning` when a re-decode matches an existing statement. Curl-verified: 2nd decode of the same text returns the warning + the existing statement id (no duplicate row). Dismissable banner with "Open the statement you already have" on web (`decoder-duplicate-warning`) and mobile (`decoder-duplicate-warning`).
- **Friendly rule labels (mobile).** New `ruleLabel()` in `format.ts` maps codes like `RULE_4_AT_HM_STREAM_MISCODING` → "Wrong funding category" (Possible duplicate / Care management fee / Totals check / Contribution check / Category check / etc.), shown as a subtle chip instead of the raw uppercase code.
- **Invoice per-issue "Draft a letter" (web).** Threaded the existing `/invoices/{id}/findings/{i}/letter` handler through `InvoiceIssueRegister → SeverityGroup → FlagCard` (`onDraftFinding`), so each invoice finding now has its own draft-letter button. Mobile already had per-issue draft letters.
- **Card content fix (web + mobile).** Rewrote `cleanTitle`/`plainBody` (dropped sentence-slicing that broke `$18.90`→"90. 00" and truncated titles with "…"). Titles are now complete + properly capitalised; explanations are whole, humanised, date-condensed, capped at a sentence boundary.

Still pending (large, next pass): Landing/Features rebuild with custom brand graphics; Guided Journeys visual overhaul.



---
## Iteration 296 (Jun 2026) — Statement Decoder: real letters + card restructure + run/saved parity (web + mobile)

Follow-ups on the approved plan, driven by live user review of the Statement Decoder:

- **Statement Decoder summary redesigned (web + mobile).** Web `StatementDecoderTool` ToolSummary body no longer dumps the whole `result.summary` wall — now a clean headline + concise 1-2 sentence overview. Web `DecoderResultView` banner gained a `decoder-who-paid` split bar (Govt% vs You%) matching mobile. Mobile keeps a short 2-sentence "IN PLAIN ENGLISH" card + the who-paid bar + "Where the money went".
- **Found-item (anomaly) cards fully restructured — shared `FlagCard` (web) + mobile decoder card.** New `plainText`/`format` helpers: `humanize`/`humanizeText` (AU dates, AT-HM/ATHM → "government-funded support", drop snake_case field names), `condenseDates` (long inline date lists → "(N dates)"), `cleanTitle` (whole-sentence headline → short, properly-capitalised title; drops trailing parenthetical/second clause; NO mid-word "…"), `plainBody` (complete explanation capped at a SENTENCE boundary — never mid-number, fixing the "$90.00"→"90. 00" bug). Cards now show: clean title, one tight explanation, one "What to do" step, and a per-card **"Draft a letter about this"** button. Removed the confusing filler signal bullets (pension status, affected streams, INDEX-1). Verified live on web + mobile.
- **Real letter from each finding (web + mobile).** Decoder anomalies now POST `/care-plans/letter-from-finding` with `source_tool:"statement-decoder"` (new optional backend field labels the carried-issues panel correctly) → web navigates to `editor_path`, mobile to `/correspondence/{id}` → letter auto-drafts.
- **Run output == saved-statement page (web + mobile).** `StatementDetail` (web `pages/StatementDetail.jsx`, mobile `app/statement/[id].tsx`) already shared `DecoderResultView`; now both pass `onDraftLetter`, so the saved page is identical to the fresh decode (same summary banner, who-paid bar, budget continuity, redesigned cards, draft-letter buttons). Web legacy fallback FlagCards also get draft buttons.

Verified live (Playwright screenshots, cathy@example.com): fresh decode anomaly cards (web), mobile decoder cards, and mobile saved statement 428da6ad — all clean, complete sentences, correct AU dates, per-card draft buttons.



---
## Iteration 295 (Jun 2026) — Real letter drafting + Decoder redesign + Plain-English + Duplicate detection

Approved multi-item plan; this pass shipped the functional/plain-English half (both surfaces, shared backend):

- **#1 (P0) "Draft a letter" now produces a REAL, ready-to-send letter with pre-filled fields.** Root cause: `letter-from-findings`/`letter-from-finding` created the LF1 entry with empty `intake`, so `/lf1/.../generate` raised `SourceDataMissing` (422) → auto-draft silently failed → blank form + no letter. Fix: new `_compose_letter_intake()` in `routes/care_plans.py` builds a populated intake (participant_name + archetype-specific summary composed from the issue titles/questions; field map for request/dispute/complaint/escalation/notification; safe "the participant" fallback so generation never 422s). Both clients now pass `participant_id`. **Curl-verified live**: findings → generate returns a full multi-paragraph letter (SUBJECT + greeting + each issue as a clear request + citation + sign-off). Mobile auto-drafts on `/correspondence/[id]`; web auto-drafts on CorrespondenceDetail.
- **#2 (P1) Statement Decoder summary redesigned (mobile)** `src/components/DecoderResultView.tsx`: removed the `decoder-plain-english-summary` wall-of-text card; money banner now has 4 tiles (Total billed / Your share / Government paid / Budget left this quarter) + a new `decoder-who-paid` split bar (Govt% vs You% + one-line sentence). Breakdown heading `STREAM BREAKDOWN` → `WHERE THE MONEY WENT`. (Web decoder was already card-based.)
- **#3 (P1) Plain-English pass** Invoice check titles rewritten on web + mobile (e.g. C10 "Lifetime cap indicative check" → "You may be getting close to your lifetime cap"; C6 → "The maths on a line doesn't add up"; C4/C7/C8 plainer). Decoder "Where the money went".
- **#4 (P1) Correspondence header cleanup (web)** `CorrespondenceDetail.jsx`: replaced "Archetype: Request · Recipient: provider cm" with a plain sentence "This is a request to your provider."
- **#5 (P1) Duplicate INVOICE detection** `routes/invoices.py` invoice-check response now returns `duplicate_warning` (exact file `file_sha256`, or same provider+invoice_date). Dismissible gold notice + "View the earlier check" link on web (`inv1-duplicate-warning`/`inv1-duplicate-view`) and mobile (`DuplicateNotice`). Non-blocking (doesn't touch the save path).

Testing: testing_agent iteration_295 = code-verification PASS on all 4 UI features (live runs skipped for token budget); backend #1 curl-verified live; mobile runtime smoke-screenshot OK (no crash, header ring intact).

### STILL PENDING from the approved plan (next pass)
- **#6 Landing & Features pages rebuild** (bold brand panels, custom-generated brand graphics, white text) web + mobile — LARGE visual build, not started.
- **#7 Guided Journeys refresh** (colour, graphics, light animation) web + mobile — LARGE, not started.
- **#5b Duplicate STATEMENT detection** — deferred (statement decode is an async job with an internally-computed fingerprint; needs threading a warning through the job result). Invoice dedup done.
- **#8 Expired-plan reactivate prompt** — BOTH web `ReadOnlyBanner` (mounted in Layout) and mobile `ViewOnlyBanner` (mounted in _layout) are correctly wired to `access_state === "view_only"` / `viewOnly`. The "not showing" is a TRIGGER/data issue: `_compute_access_state` (server.py:3169) may keep a lapsed paid user "active" if `user.plan` still says a paid plan and no view-only sub_status was recorded. Needs an actually-expired Stripe fixture to reproduce before changing the fallback (risk of locking out active users).



---
## Iteration 294 (Jun 2026) — MOBILE PARITY P0: Account Health header + Price Checker compare bars + Draft-letter carry-over

User's #1 frustration was that big web UI changes weren't mirrored to /app/mobile. Ported the three flagged web features to mobile (web untouched, source of truth):

- **Account Health Score ring → global mobile header** (`src/components/WaylyHeader.tsx`): % ring `header-health-ring` + terracotta count badge `header-health-badge` now render top-right on EVERY tab/screen (next to the bell), reading `GET /account/health`. Tapping opens a bottom-sheet `account-health-menu` (teal header + ring + "Finish setting up") with a scrollable `account-health-list` of outstanding items (`account-health-item-<id>` → routes to /participants or /(tabs)/settings) and struck-through done items; `account-health-close` dismisses. Removed the now-redundant inline dashboard ring + `AccountHealthCard` from `app/(tabs)/index.tsx` (parity with web, which moved it to the header). **Verified live** (ring shows 11% + badge 6; sheet lists 8 outstanding items).
- **Provider Price Checker comparison bars** (`src/components/tools/ProviderPriceChecker.tsx`, new `CompareBars`): mirrors web `pc-compare-bars` — plain-English line `pc-compare-plain` ("You pay $X. That is N% more/less than the typical rate of $Y"), a "You pay" bar `pc-charge-pin` (sage/terracotta by position), a "Typical median" bar (teal), a "Usual range" band, and an Above/Within pill. **Verified live** (Personal care @ $100 → bars render, match stat cards).
- **Draft-a-letter carry-over + auto-prefill** (`src/components/tools/CarePlanReviewer.tsx` + `app/correspondence/[id].tsx`): added "Draft one letter covering all N findings" button `cp-draft-letter-all` (shows when findings>1) → POST `/care-plans/letter-from-findings` → `/correspondence/{entry_id}`. The mobile letter editor now reads `entry.source_import` → renders a teal `lf1-carried-issues` panel (white text, bullet list) and AUTO-DRAFTS the letter on arrival (`lf1-autodraft-busy` → populates `letter-body-input`) without a manual Generate tap — mirrors web CorrespondenceDetail. Backend `letter-from-findings` creates the entry with no `content_draft`, so the auto-draft guard fires correctly. **Verified**: testing_agent iteration_294 code-review PASS on all 3; live screenshots confirmed header ring/sheet + price-checker bars.

### Remaining from this session's plan (NOT yet done — next up)
- P1: Plain-English pass (Invoice Checker + tool outputs, web + mobile).
- P1: Landing & Features pages rebuild (bold brand panels, custom-generated brand graphics, white legible text — user chose custom-generated graphics) web + mobile.
- P1: Guided Journeys visual overhaul (colour, graphics, light animations) web + mobile.
- P2: Fix expired-plan reactivate prompts (reported not showing) — both.
- P2: Dark-mode text legibility on Plan & Billing pages — web.
- LOW (nit, deferred): `mobileFixRoute` duplicated in WaylyHeader.tsx + AccountHealth.tsx — hoist to shared util.
...
---
## Iteration 230-231 (Jun 2026) — DOC-PARITY-1 v2 (downloadable artefact parity + severity ordering/collapse + mobile archive)

User report: decoded documents differed web vs mobile; mobile lacked an Archive button; findings weren't ordered High→Med→Low; wanted collapsible severities (default expanded). Audit found DOC-PARITY-1 had NOT been implemented. Shipped in two phases, all behind default-ON flags (DOCPARITY_ORDERING, DOCPARITY_SERVER_ARTIFACT).

### Phase 1 — ordering, collapse, dates, mobile archive
- **Canonical order** (`backend/lib/anomaly_order.py`, new): four bands High → Medium → Low → Informational (info + advisory collapse into Informational); secondary sort ↓dollar then ↑date. Applied once in `agents._add_parse_warnings` so both surfaces + the artefact consume the same ordered list; `anomaly_count` now includes `informational`.
- **Collapsible severity groups, default EXPANDED** on web (`DecoderResultView.jsx` — `SeverityGroup`/`AnomalyCard`) and mobile (`DecoderResultView.tsx`), testIDs `severity-group-*` / `severity-group-toggle-*`.
- **Mobile Archive/Restore** on `mobile/app/statement/[id].tsx` (inline confirm) wired to `DELETE /statements/{id}/archive` + `POST /statements/{id}/restore` — matches web.
- **DD/MM/YYYY** everywhere: DecoderResultView period header (both surfaces), web statements register/detail (`statementFields.periodCompact` rewritten), mobile statement page title.

### Phase 2 — server-authoritative documents (hash-identical)
- **`POST /api/decoder/artifact?fmt=pdf|csv`** (`server.py`) renders the canonical decoded-statement PDF (`lib/decoded_statement_pdf.py`, ReportLab via `pdf_branding`) + CSV. Both web (`decoderExport.js`) and mobile (`decoderExport.ts`) POST the decode payload and download these exact bytes → **hash-identical**, identical filename. Client-side renderers kept as fallback.
- **Deterministic PDF**: `pdf_branding.make_doc` sets `doc.invariant`; render sets `rl_config.invariant=1` + deterministic footer date → same payload = same SHA-256 (fixed the iter230 non-determinism blocker).
- **Shared filename function** (`lib/artifact_naming.py`, new): `Wayly-Decoded-Statement_<slug>_<DD-MM-YYYY-to-DD-MM-YYYY>.<ext>`. Also routed the **complaint evidence bundle** (`routes/cmp1.py`) and **care-plan export** (`routes/care_plans.py`) through it. (Invoice Checker has no standalone PDF download endpoint to route.)

### Verified
- testing_agent iteration_230 (found + I fixed: PDF non-determinism, spelled-out dates) → **iteration_231 PASS** both surfaces: hash-identical PDF, DD/MM/YYYY headers, ordered collapsible groups default-expanded, mobile archive/restore, downloads call the endpoint. New pytest `test_docparity_naming.py`, `test_docparity_artifact_iter230.py`, updated `test_sderrc_golden.py` — 22 green. Web statements register confirmed rendering DD/MM/YYYY via screenshot.

### Not done / out of scope this round
- Invoice-Checker downloadable report (no such endpoint exists yet).
- Wayly Summary LLM prose still says "July to September" (generated text, not a date field).


---
## Iteration 229 (Jun 2026) — Statement Decoder + Invoice Checker remediation pack (DEC-FINDINGS-1 v3 / IC-FINDINGS-1 v2 / SD-ERR-C-golden v2)

### Shipped (backend; deterministic financial engine — all behind per-workstream env flags DEC1_REMEDIATION_A/B/C/D, default ON, roll back with =0)
- **New `/app/backend/lib/services_base.py`** — ONE shared services-base definition reused by BOTH the Statement Decoder (`agents.py`) and the Invoice Checker (`lib/inv1/checks.py`). Care-services base excludes AT-HM + care management + prohibited fees. Also hosts the per-line GOVT-PAID identity, INDEX-1-sourced contribution bands, the 1 Oct 2026 personal-care reclassification, and the feature flags.
- **A · Reconciliation fix** (`agents.py` RULE_25): reconciles the declared services total against whichever interpretation (full line-item sum vs care-services-only sum) is closest to the declared figure — kills the phantom $286.00 AT-HM gap on SD-ERR-C while still catching genuine gaps and not false-firing on AT-HM-only statements. Cap check (RULE_1) now resolves its base through the same module.
- **B · Contributions** (`agents.py` RULE_9): the Decoder no longer emits dollar-level RULE_9_CONTRIBUTION_MISMATCH / RULE_9_INCONSISTENT_RATE. Contribution observations collapse into ONE informational note (`RULE_9_CONTRIBUTION_INFO`) listing affected streams + dates, pointing to the Contribution Estimator, no dollar variance, no refund wording. Rates sourced from INDEX-1 (`program_reference`) with effective dating; the Invoice Checker (INV-1) remains the authoritative dollar-level contribution checker.
- **C · Aggregate + care-mgmt %** (`server.py`): the "money worth querying" aggregate sums only same-direction recoverable overcharges (excludes info/low, reconciliation/parse artefacts, undercharges) → $28.60 on SD-ERR-C. Plain-English care-management % now driven from the same services base as the HIGH flag → reads 11.5%, never 10.0%.
- **D · Per-line GOVT PAID identity** (`server.py` `_line_govt_paid`): every line GOVT PAID = GROSS − YOU PAID (column sums $2,160.00; summary government-paid $2,376.00 = column + $216.00 care mgmt). Footer program-reference LOW flag (RULE_29) removed.
- **E · Golden fixture + IC** — new `tests/test_sderrc_golden.py` freezes the corrected SD-ERR-C decode (drift fails CI). Invoice Checker C4 care-management base routed through the shared module (AT-HM excluded consistently); parity test asserts both tools resolve $1,874.00 through the one module.

### Verified
- testing_agent iteration_229.json: **199 passed, 2 skipped, 0 failed**. SD-ERR-C golden: exactly two findings (HIGH cap $28.60 + INFO contribution), no phantom reconciliation, no footer, no MEDIUM, 11.5% prose, GOVT-PAID column $2,160.00, aggregate $28.60, no refund wording, rates trace to INDEX-1. Decode + invoice API endpoints 200, no 500s from the new code. HIGH cap catch + extraction unchanged.
- Existing decoder tests that encoded the OLD contribution behaviour were updated to the new INFO-note semantics (test_pension_rates, test_dec1_v5_phase2, test_dec1_v5_nwo_athm, test_iter17_async_job, test_dec1_v7_7).

### Next (user-flagged follow-ups, not yet started)
- CHSP invoice photo OCR (auto-fill fee-check fields); CMP-1 evidence-bundle open in-app; PSW-1 switch-detail actions (generate-notice + stage-advance) on mobile.
- DOC-PARITY-1 v2 artefact/filename/DD-MM-YYYY parity + severity ordering (PDF/CSV export layer) — deferred; the golden asserts the deterministic decode, not the rendered artefact bytes.

## Iteration 210 (Jun 2026) — PARITY-1 Phase 1: Statement Decoder (mobile → web parity)

### Shipped (mobile /app/mobile, additive only; web untouched)
- **New `src/components/DecoderResultView.tsx`** — full port of web `DecoderResultView.jsx`. Reads the correct nested backend payload (`audit.statement_summary`, `audit.anomalies` + `anomaly_count`, `audit.stream_breakdown`, `extracted.line_items`). Fixed the prior data-parity DEFECT where mobile read wrong keys (`a.title`/`li.description` vs real `a.headline`/`li.service_name`). Renders: plain-English summary, money banner (Gross/You/Govt/Budget), budget-continuity panel, anomaly panel (top banner + severity/rule/headline/detail/evidence/suggested-action cards + advisories bucket), expandable stream breakdown, collapsible full line-item table, partial-result/redaction/scan notices.
- **New `src/lib/decoderExport.ts`** — CSV export (`shareTextFile`) + PDF export (`expo-print` `printToFileAsync` + `expo-sharing`), mirroring web `decoderExport.js` artefact output. Installed `expo-print`.
- **`app/tool/[slug].tsx` StatementDecoderTool** rebuilt: 3 input modes (Paste text / Upload file or photo / Forward by email), logged-out **free-use counter** (`/free-tool/usage?tool=STATEMENT_DECODER`) + limit/cooldown gate with trial CTA, abuse-flag guardrail, download bar (CSV/PDF).

### Verified
- Backend payload confirmed via curl (identical shape both surfaces). testing_agent (mobile): decode of sample statement rendered every section with real data (1 high/1 med/1 low anomaly, 3 streams, plain-English summary), CSV+PDF export fired without crash, usage banner + cooldown gate correct. No blocking bugs.

### PARITY-1 remaining (per /app/PARITY-1-PHASE0-AUDIT.md, worst-first)
- CHSP-1: mobile screen is informational-only (0 API) vs functional web → build out.
- CSC-1: add PDF/email artefacts + IAT domains on mobile.
- Care Plan Reviewer: add file-upload path on mobile.
- Provider Price Checker: add history/milestones/snapshots depth.
- LF-1: add follow-ups + cross-tool-signals.
- Confirm endpoint aliases for CMP-1/PSW-1/ATHM-1; clarify AW-2 (Ask Wayly) scope.


---
## Iteration 209 (Jun 2026) — Mobile Add-Participant Form Parity with Web

### Shipped (mobile /app/mobile)
- **Reusable `Select` dropdown** added to `src/components/ui.tsx` — a tappable field opening a bottom-sheet option list (kebab testIDs, active check-mark, matches DateField styling). First shared select primitive for mobile.
- **Add-Participant form (`app/participants.tsx`) brought to full parity with web `frontend/src/pages/extended/Participants.jsx`:**
  - **Date of birth** field added (existing DD/MM/YYYY `DateField`, `form-dob`).
  - **Classification** switched from a number-pad `Field` to a `Select` dropdown ("Not sure yet" + Class 1–8, `form-classification`).
  - **Provider** switched from a plain text input to an inline `ProviderPicker` matching web exactly: if sibling participants already have providers, show a dropdown of them (`form-provider-select`) + "Add a different provider" toggle (`form-provider-add-new`) to a free-text input (`form-provider`) with a back-link (`form-provider-pick-existing`); if NO existing providers, fall back to a plain text input (user-confirmed: match web).
  - **Statement delivery** dropdown added ("Not sure yet" / Email PDF / Provider portal / Paper, `form-statement-format`).
- Submit payload unchanged (`classification` → Number|null, `statement_format`, `date_of_birth`) — backend contract untouched.

### Verified
- testing_agent iteration_209.json: PASS, no bugs. Full flow on `cathy@example.com` (Family): preview branch → form → all 6 fields render in web order, both ProviderPicker modes work (dropdown of existing providers + free-text), submitted a participant with Class 4 + Acacia Aged Care + Email PDF + DOB → POST /v2/participants 200, done step + forwarding email, new card shows "Classification 4 · Acacia Aged Care". (Seed participant `TESTIter209 Parity` left in cathy's account.)

### Next up (user-confirmed order)
- **AI Tools Parity (web → mobile)** per `AI Tools PARITY-1-v1.md` (in /app/memory or artifact xzphenit_AI Tools PARITY-1-v1.md): Phase 0 = audit gate (report only, no code) across 15 tools; then Phase 1 = port one tool at a time starting with **Statement Decoder**, 100% data/feature/artefact parity. Web is source of truth, not modified.
...
- `curl https://final-verify-web.preview.emergentagent.com/sitemap.xml` returns 200 with the correct XML (113 URLs).
...
- **Env** — `FRONTEND_URL=https://final-verify-web.preview.emergentagent.com` added to backend `.env` so verification links point at the right host. (Production needs the same key set to `https://wayly.com.au`.)
...
### Parity sweep progress update (2026-06, iter_170 PASS)
DONE + testing-agent verified (mobile): Dashboard, AI Tools, Family Wall, Statements (+Export CSV +Archived), Participants, Support, Plan & Billing, Missing-details banner (dismissible + 24h). Per-participant data isolation re-confirmed on switch (X-Participant-Id).
STILL TO PORT (web order): Money → Quarterly Pacing, Invoices (done earlier), Budget Alerts, Budget Scenarios, Reports; Guided Journeys (Ask Wayly, Carer Self-Check, Handover Pack, Classification Prep, AT & HM Projects, CHSP Tools, Letters Mailbox, Switch Provider); Their Care (Care Team, Key Contacts, Calendar, Hospital Mode, Care Plans, Care-Plan Changes, Log a Scenario, Timeline); Providers & Paperwork (Documents, Correspondence, Compare Providers, Ratings); Your Account (Referrals, Audit Log, Settings, Profile hub verify). Then a dark-mode contrast pass on remaining detail screens.

### Parity sweep update (2026-06, iter_171 PASS)
DONE + verified: Dark-mode sweep (all mobile screens now theme-aware — ask/upload/decode/invoice converted; zero static-palette or hardcoded-hex left). Money category (Quarterly Pacing themed pace colors, Budget Alerts web copy, Budget Scenarios + Reports PageIntro). Guided Journeys PageIntro (Carer Self-Check, Handover Pack, AT & HM, CHSP Tools, Letters, Care Plans). Their Care: Care Plans done; calendar/hospital/amendments/scenarios/timeline/care-team/key-contacts have no web PageIntro and are already theme-aware = at parity.
REMAINING (web order): Providers & Paperwork (Documents, Correspondence, Compare Providers, Ratings) parity check; Your Account (Referrals, Audit Log, Settings, Profile hub) parity check; optional defence-in-depth: add ?participant_id to reports.tsx & handover-pack.tsx fetches (currently rely on X-Participant-Id header, works fine).

### Parity sweep update (2026-06, iter_172 PASS)
DONE + verified: Providers & Paperwork — Documents + Compare Providers PageIntro added; Correspondence + Ratings at parity (no web PageIntro). Your Account — Referrals/Audit Log/Settings/Profile hub at parity (no web PageIntro, theme-aware). Provider Switch REBUILT into the 5-step guided walkthrough (Why You Might Switch → Before You Decide → Comparing Providers → Giving Notice+draft letter → Handover) on /provider-switch, draft persists & resumes at saved stage. Dark mode verified.
SWEEP STATUS: The full web sidebar (Today, AI Tools, Money & Statements, Guided Journeys, Their Care, Providers & Paperwork, Your Account) has now been brought to mobile parity across iterations 166–172. Optional follow-ups: Provider Switch notice PDF (web has a PDF export; mobile ships Copy + .txt); defence-in-depth ?participant_id on reports/handover-pack fetches.

### AI Tools full-parity rebuild + prefill/persona (2026-06, iter_177–181 PASS)
GOAL: Replace the generic mobile FormTool/ToolScreen renderer so EVERY AI tool is a dedicated screen mirroring the web tool, with participant prefill and caregiver/participant persona voice.
DONE + testing-agent verified (mobile):
- iter177: fixed iter176 P0s — Classification Self-Check result panel (reads real csc.payload.v1 shape: classification band, confidence pill, budget range, profile_summary, top_drivers w/ domain labels, branch A/B/C next-step, repeat nudge, rerun) + primary Button label white (#fff) on teal/clay.
- iter178: Provider Price Checker (grouped service picker, transport unit, grandfathered gate, inline pension picker, quality-guard soft-confirm, how-this-compares + 3 stat cards + how-this-works) and Contribution Estimator (5 entry paths, HCP fee/level follow-ups, assessment status, classification picker, pension, household, progressive financials, service mix; result: point/range/fee-exempt headline, govt-share bar, rate breakdown, safety-net cap, Oct-2026 comparison, HCP comparison, also-worth-knowing).
- iter179: Letters & Follow-ups triage (12 situations grid, terms footer, elder-abuse safeguarding gate w/ tel contacts) + NEW correspondence detail/editor at /letters/[id] (archetype-driven intake form, generate draft via /lf1/correspondence/{id}/generate incl. 422 missing-fields handling, copy). Situation labels personalised ({name}→participant first name).
- iter180/181: Aged Care Q&A chat (/tool/family-coordinator → /public/aged-care-chat), Support Plan Reviewer (/public/care-plans/review, findings by severity), Invoice Checker (expo-document-picker → /invoices/upload, verdict banner + metadata + summary + tier findings). PREFILL from useParticipants().active (name, provider, classification) via useEffect sync on active.id (fixes init-timing bug). PERSONA via new src/hooks/usePersona.ts (role→caregiver/participant) applied to CSC copy + questions.
Dispatcher: /app/mobile/app/tool/[slug].tsx now routes all 8 tools to dedicated components; FormTool no longer used for any real tool. New dep: expo-clipboard.
STATUS: All 8 AI tools at exact mobile parity. 
BACKLOG (P1/P2, not yet done): (1) Settings/"Your Account" subscreen copy parity sweep vs web Settings tabs; (2) end-to-end behaviour/output test of each tool + guided workflows against web; (3) verify Stripe workflows on mobile vs web.

### Web Downgrade UI — parity with mobile (2026-06, iter_200 PASS)
DONE + testing-agent verified (web + backend):
- frontend/src/pages/Settings.jsx BillingTab: Family→Solo now calls POST /api/payments/schedule-downgrade (real Stripe Subscription Schedule) instead of legacy /billing/upgrade. Keeps window.confirm().
- Pending-change banner (data-testid billing-pending-change) "Your plan changes to Solo on <date>. You keep Family access until then." reads sub.pending_plan / sub.pending_effective from GET /api/billing/subscription.
- "Keep Family" button (billing-keep-plan) calls POST /api/payments/cancel-scheduled-change.
- Solo plan-option card shows "Scheduled for <date>" (billing-plan-scheduled-solo) and Switch buttons disabled while a downgrade is pending.
- Env limitation (unchanged): Stripe webhooks not delivered to preview; seeded accts w/o real stripe_subscription_id return 400 on schedule-downgrade (expected, not a bug).
STATUS: Web now at parity with mobile plan-billing.tsx downgrade UX.
NEXT (P1, in order): (1) Upgrade Consolidation — move Solo→Family + add-ons off one-time checkout onto Stripe subscription.update w/ exact proration; (2) Mobile Signup/Onboarding Parity & Cross-App Resume (2nd participant on Family signup; cross-app draft resume). P2: notification prefs per member; content polish; demo refresh.

### Upgrade Consolidation (2026-06, iter_201 PASS)
DONE + testing-agent verified (both surfaces). Every upgrade/add-on path now applies REAL Stripe proration; legacy one-time checkouts removed.
- NEW backend POST /api/payments/change-plan (routes/payments_advanced.py): immediate solo→family via stripe.Subscription.modify(proration_behavior=create_prorations); 409 on downgrade (use schedule-downgrade), 400 no-sub, 422 bad plan; mirrors plan→db.subscriptions + users.
- Web Settings "Switch to Family" now calls /payments/change-plan (was no-op /billing/upgrade).
- Web + mobile Participants add-flow: removed legacy /billing/v2/upgrade-checkout + /billing/v2/addon-checkout + cancel-pending-addon redirects/rollbacks. solo→family "Continue" goes straight to form; the real prorated update happens via POST /v2/participants + /payments/sync-plan-to-participants. Done step shows addon-prorated-note. Removed testIDs: pay-addon-btn, skip-payment-btn, skip-checkout-solo-to-family.
- Legacy batch3_billing.py v2 checkout endpoints now unused by clients (safe to deprecate later).

### Mobile Signup Parity + Cross-App Resume (2026-06, iter_202 PASS)
DONE + testing-agent verified (both surfaces).
- Mobile signup (mobile/app/signup.tsx): Family plan captures a 2nd participant (signup-second-participant-block/toggle/first-name + relationship chips), mirroring web. On submit pre-creates the 2nd participant stub (POST /v2/participants is_primary:false) + PATCHes relationship. Resets when switching to Solo.
- Cross-App Resume: mobile onboarding (mobile/app/onboarding.tsx) now GET/PUT/DELETE the shared user-scoped /api/onboarding/draft with the SAME {tier1,tier2,auth:{confirmed},step} shape web uses — start on web, finish on mobile (and vice-versa). Debounced autosave (800ms), restore-on-mount with onboarding-draft-status pill, clears draft on finish. Verified end-to-end cross-client.
STATUS: All three P1 tasks complete. NEXT (P2): notification prefs per member; content polish (Services/Guides/Glossary); demo refresh (Margaret class 4 seed + hero dashboards).

### Stripe TEST volume seeding (2026-06, one-off, run_id wayly-10k-20260817-151053)
- Tool: /app/scripts/seed_stripe_volume.py (reads secret key from env STRIPE_SEED_KEY; never hardcoded). Concurrent (thread pool) with idempotency keys + exponential backoff so re-runs never double-charge; resumable via START.
- Seeded 10,000 TEST-mode customers: 70/30 solo/family (7,000 × $24.50, 3,000 × $49.50 AUD fortnightly), 50/50 web/mobile via metadata signup_source. Each = 1 active subscription (past trial) + 4 fortnight charges (cycle 1 from the sub invoice + 3 explicit Charges).
- Result: 40,000 successful charges, GROSS VOLUME $1,280,000 AUD, 0 failures. All customers/charges tagged metadata seed_run=<run_id> for cleanup (search customers by that key to bulk-delete).
- Secret key was provided by the user for this one-off, used only at runtime and shredded from the pod afterward. NOTE: this ran against the USER'S Stripe test account directly (preview STRIPE_API_KEY is only the placeholder sk_test_emergent, swapped at deploy).

### Data Cleanup + Content Polish (2026-06)
- Stripe cleanup: /app/scripts/cleanup_stripe_seed.py deleted all 10,000 seeded test customers (metadata seed_run=wayly-10k-20260817-151053) + their subscriptions. Result: 10,000 matched, 10,000 deleted, 0 failures, 0 remaining. Past charges/gross-volume history are unaffected (Stripe cannot delete charges). Key used at runtime only, shredded after.
- Content Polish (web-only, frontend/src/data): Glossary +20 terms (44→64) in resources.js; deepened all 8 Services (services.js) and all 8 Caregiver Guides (guides.js) with an extra substantive section each. AU-English editorial rules preserved. Lint clean; render verified on /services/cleaning. Mobile has no Services/Guides/Glossary pages, so no mobile change.

### Repo secret removal + homepage/guides polish (2026-06)
- SECURITY: real Stripe test key was committed in backend/.env (line 21) and hardcoded in backend/tests/billing/test_iter197_billing_consolidation.py. Restored .env to placeholder sk_test_emergent; test file now reads os.environ; sanitized a partial key ref in test_reports/iteration_138.json. Working tree is clean (0 matches). NOTE: prior git history still contains the key — user should rotate it in Stripe.
- Homepage hero (DualFlagshipHero.jsx): "Aged Care, Made Clear." now on one line (removed forced <br>) and removed the decorative underline bar.
- Footer/hub: "Explore the Rest of Wayly" (SeoHubLinks.jsx) redesigned — soft background band, larger centred heading, lead line, elevated cards, clay "Explore →" CTA; all item labels Title Cased (Support at Home Levels, Service Explainers, Policy Explainers, Caregiver Guides, FAQ, Ask Wayly). This strip is the "footer Caregiver guides" the user saw.
- Capitalisation: guides.js/services.js overlines → "Caregiver Guide"/"Service Explainer"; GuidesHub visible overline, SEO title and jsonLd → "Caregiver Guides".
- Content depth (SEO): added a second substantive section to all 8 Caregiver Guides (funding, safety wins, costly errors, what moves with you, third voice, writing it down, free supports, authorised contact). Grammar: fixed missing "$" on price ranges in services.js.

### Mobile signup Stripe gate + never-Free fix (2026-06, iter_203 PASS)
Bug: mobile Family signup didn't route to Stripe and left users on a "Free" plan.
Root causes + fixes:
- mobile/app/signup.tsx swallowed checkout errors and proceeded to onboarding as free. Now: card capture is REQUIRED — after account creation the user hits a payment gate (signup-payment-gate) and only reaches onboarding once a card is on file (status trialing/active). Retry via signup-complete-payment-btn; back via signup-change-plan-btn.
- Preview backend only had placeholder STRIPE_API_KEY (sk_test_emergent), so /payments/checkout 503'd. Added a PRODUCTION-SAFE fallback in server.py: if the process env key is the placeholder, resolve the real key from the git-ignored backend/.env. Preview now returns real checkout.stripe.com URLs. No-op in production (real key injected there). backend/.env is git-ignored so the key is never pushed.
- Never-Free display: mobile plan-billing.tsx and (tabs)/settings.tsx now show "No active plan" / "PAYMENT REQUIRED" + Start-free-trial CTA for subscription-less users; manage/cancel/change hidden. Web signup unchanged (already hard-redirects to Stripe).
Caveats: full card capture + trialing subscription validated on production/real device (openAuthSessionAsync deep-link return isn't fully automatable in the web preview). Mobile app (Expo) points at the PREVIEW backend, which now has the real test key. Minor pre-existing item flagged: /payments/checkout accepts unauthenticated callers (hardening candidate).

### Mobile duplicate-participant + Complete-now + heading fixes (2026-06, iter_204 PASS)
Reported on mobile (maud@test.com, Family): double "Participants" heading; 5 participants instead of 2 (3 duplicate Maud/Felix → $123/fortnight); "Complete now" for Felix opened the authorisation step for Maud.
Root cause + fixes:
- mobile/app/onboarding.tsx submitParticipant ALWAYS POSTed a new participant on submit → duplicates on re-submit/re-entry. Now PATCHes when participantId already exists, POSTs only on first create.
- Onboarding now supports deep-link edit mode /onboarding?pid=<id> (mirrors web ?pid=): loads that participant, prefills tier1/tier2, does NOT restore the signup draft, authorisation step names the correct person, and Save PATCHes (no new participant).
- mobile/src/components/MissingDetailsBanner.tsx "Complete now" now routes to /onboarding?pid=<incomplete participant id> (was /onboarding with no pid → generic primary flow).
- mobile/app/participants.tsx: removed the in-body duplicate "Participants" heading (sticky AppHeader is the single title).
- HARDENING: POST /api/payments/checkout now requires auth (401 without token); web+mobile signup create the account first so the caller is always authenticated. Closes the iter203 unauth-checkout note.
Data: removed maud's 4 duplicate participant docs (now Maud primary + Felix = 2; participant_count reset to 2; page shows $49.50). Stripe sub addon quantity reconciles to 2 on next Participants-page visit (sync-plan-to-participants). maud is trialing so no wrong charge occurred.
Note: these are preview fixes — redeploy for production (mobile needs a new Publish build).

### Second-Participant Onboarding + Stripe key rotation (2026-06, iter_205 PASS)
- Rotated Stripe TEST key wired into git-ignored backend/.env; authed checkout returns real checkout.stripe.com URLs, 401 without auth. Old key retired.
- Feature: after completing the FIRST participant's onboarding, mobile (mobile/app/onboarding.tsx) now checks for any other participant with requires_completion=true and shows an All Done "One more to go" card + "Complete <name>'s details" (onboarding-complete-next) that chains IN PLACE to step 1 for that participant (prefilled, auth reset), plus "I'll do this later" (onboarding-finish-later). When none remain, shows "Go to dashboard" (onboarding-finish). Completing chained participants PATCHes (no duplicates). Verified live on Expo preview + 5/5 backend tests.

### Mobile UX fixes batch (2026-06, iter_206 PASS) + pending parity items
Done + verified (mobile):
- Header avatar (WaylyHeader.tsx) is now a Pressable -> opens Settings tab (testID header-avatar).
- Global keyboard avoidance: shared Screen (ui.tsx) wraps content in KeyboardAvoidingView (iOS padding / Android height) so inputs aren't covered. NATIVE-ONLY effect (no-op on web preview).
- Onboarding DOB DateField (ui.tsx): picker now in a surface-colored bordered container with textColor + themeVariant='light' + iOS Done button so it's legible (was blending into bg). NATIVE-ONLY styling.
PENDING (requested, NOT yet done — next up):
- P0 Plan & Billing PARITY (mobile plan-billing.tsx): must show Base plan / Participants (2 included) / Add-ons (N x $24.50) / Fortnightly total and reflect extra participants correctly (currently shows only base $49.50 even with a 3rd participant). Mirror web billing breakdown + participant list.
- P0 Add-participant form PARITY (mobile participants.tsx bottom sheet): full fields with dropdowns matching web — First/Last, DOB, Classification dropdown, Provider dropdown WITH "add a different provider", Statement delivery — same behaviour incl. provider list from backend + add-new.

### Plan & Billing breakdown parity + add-modal keyboard (2026-06, iter_207/208 PASS)
...
## Iter271 — PERMS v3 Milestone: Accept Invite Flow (signup-via-invite + approval) ✅ DONE (web+backend, curl e2e + screenshot verified)
Replaced the legacy logged-in accept endpoints with the v3 email-bound signup-via-invite + account-holder approval.
Backend (server.py): GET /api/invite/{token} (public details: inviter/household/kind/relationship/expired/already_registered), POST /api/invite/accept (email-bound signup reusing hash_password/create_access_token; caregiver → household_memberships status 'pending_approval' + notifies owner; participant_login → activates immediately + links participant_record.user_id), POST /api/household/memberships/{id}/approve (activates + creates caregiver_participant_links to all live participants) + /deny (revoked). list_members now returns `pending_approvals`. create_household already bootstraps DM-1 records (iter270).
Web: rebuilt /pages/InviteAccept.jsx (public /invite?token= landing → sets tokens via setAuthToken/setRefreshToken → participant to /app; caregiver → pending-approval screen). Family Members tab: new "Waiting for Your Approval" section with Approve/Deny.
Verified via curl: invite → GET → accept (pending caregiver) → surfaced in pending_approvals → approve clears it. Screenshot: /invite page renders with caregiver copy + prefilled email. (Cleaned cathy's 7 stale pending invites; left 1 approved test caregiver 'Sibling Sam'.)
NOT via testing_agent this milestone (budget) — safe to run a backend regression on the invite loop later.
### Remaining big milestones (order): M-BILLING unblock (raw Stripe SDK + claimable sandbox: portal + resume + subscription items — dedicated turn) → M-PERMS guardrails (authz middleware @requires_role + participant scoping, route guards, caregiver landing = per-participant Family Wall, Ask Wayly caregiver scope + 20 red-team, audit surfacing, transfer-primary, subscription-lapse) → Participants + invites MOBILE parity.

## Iter272 — P0 Onboarding fix + P1 remove duplicate Settings Participants tab ✅ DONE (backend+web, curl e2e + screenshot verified)
Root cause: iter270 added duplicate `/participants` CRUD routes DIRECTLY on `api` in server.py (backed by `participant_records`). Because `@api.*` decorators register at module-definition time, they were added BEFORE `api.include_router(participant_profile_router)`, so they SHADOWED participant_profile.py's `/participants` handlers. Onboarding.jsx POSTs the Tier-1 payload (first_name/last_name/dob/classification_level/pension_status/provider_name/statement_delivery/authorisation_confirmed) which the shadowing route (expecting full_name) rejected → 422 "Could not save participant".
Fix (backend/server.py): deleted the shadowing routes (GET/POST/PATCH /participants, /archive, /deceased, /invite-login) + their now-unused models (ParticipantCreate/Update/InviteBody) and `_participant_role_label`. KEPT `_require_account_holder` (still used by /household/memberships/{id}/approve+deny). Participant CRUD is now single-sourced: participant_profile.py (`/participants`, onboarding Tier-1 schema) + batch3_routes.py (`/v2/participants`, the mature extended/Participants.jsx UI at /app/participants).
Fix (frontend/Settings.jsx): removed the duplicate "Participants" tab (TABS entry, render line, and the whole ParticipantsTab component) — it was the only caller of the deleted server.py routes. Single source of truth = /app/participants (extended/Participants.jsx). Settings tabs now: Profile, Plan and Billing, Family Members, Weekly Digest, Notifications, Appearance, Usage, Security, Danger Zone.
Verified (cathy@example.com): GET /participants → {items[3]}, POST Tier-1 → 201, POST authorisation_confirmed=false → 422 authorisation_required, GET /{id} → 200, PATCH → 200. Settings page screenshot: loads clean, Participants tab gone. (NOT run through testing_agent — deletion-only, fully curl+screenshot verified.)

### NEXT (user-approved, NOT yet started) — M-BILLING unblock: direct-charge participant add-ons
User pain: adding an extra participant creates a "pending add-on" that forces a SEPARATE hosted Stripe Checkout redirect to confirm/pay. User wants: charge the SAVED card immediately, gated by a ONE-TAP "$24.50/fortnight — Confirm" prompt (user chose option b). 
Current flow: batch3_routes.add_v2_participant creates ParticipantAddOn(status ACTIVE, no stripe_subscription_id) → frontend must call batch3_billing `POST /billing/v2/addon-checkout` (emergentintegrations hosted checkout) → webhook stamps stripe_subscription_id. `POST /billing/v2/cancel-pending-addon` cleans unpaid ones.
Planned approach (raw stripe SDK per integration playbook): new backend endpoint that finds the account's Stripe customer + active subscription + default payment method; if a saved card exists → add a fortnightly add-on as a subscription item (or off-session invoice) charging immediately, stamp add-on stripe_subscription_id synchronously (no webhook dependency — off-session PI/invoice returns status inline, so testable in test mode by seeding a customer + pm_card_visa); if NO card/subscription → return needs_checkout flag, frontend falls back to existing hosted addon-checkout. Frontend extended/Participants.jsx: replace the pending-add-on/checkout routing with the one-tap confirm → charge → success, fallback to checkout when needs_checkout. NOTE: real end-to-end against a live saved card can't be exercised in preview (Stripe webhooks aren't delivered to preview), but the immediate off-session charge is verifiable synchronously in Stripe test mode.
Then: M-PERMS caregiver guardrails; Participants + invites MOBILE parity; CHSP invoice-photo OCR (P2); noindex on /app pages (P2); auto sitemap at build (P2).
NOTE: PRD.md is 7100+ lines — should be split into PRD/CHANGELOG/ROADMAP in a future housekeeping pass.

## Iter273 — Direct-charge participant add-ons + Mobile Invites v3 parity ✅ DONE (backend + web + mobile, testing_agent iter271 all PASS + curl/Stripe e2e)
User asked: adding an extra participant must charge the SAVED CARD immediately (one-tap "$24.50/fortnight — Confirm") with a hosted-checkout FALLBACK when no card on file (user chose Option 2 = true off-session saved-card charging). Also: bring mobile Invites up to web v3.
ROOT CAUSE of the "pending add-on to confirm" pain: the app already had the correct infra — `routes/payments.py` (raw Stripe SDK, mode=subscription, real customers + SAVED CARDS, STRIPE_PRICE_ID_ADDITIONAL) + `routes/payments_advanced.py` `POST /payments/sync-plan-to-participants` which reconciles the live subscription (base + add-on quantity) with `create_prorations` (charges the saved card). BUT sync 500'd on `sub.items` (StripeObject subclasses dict → `.items` returned the dict method), and the web/mobile add flows called it fire-and-forget with `.catch()`, silently swallowing the 500 → the add-on never actually got billed and the legacy batch3 `participant_add_ons` row stayed unstamped → showed as a scary "pending add-on / Cancel" banner.
Fix (backend `routes/payments_advanced.py`): (1) fixed all 5 `sub.items` accesses → `sub["items"]["data"]`. (2) new `_stamp_account_addons_active(user_id, sub_id)` — after a successful/idempotent sync, stamps dangling ACTIVE `participant_add_ons` rows (account owner) with the real `stripe_subscription_id` so no stale "pending" banner survives.
Fix (web `extended/Participants.jsx` submitAdd): when the add returns an add-on, AWAIT `/payments/sync-plan-to-participants`; on `{ok:false,reason:'no_active_subscription'}` → POST `/billing/v2/addon-checkout` and redirect (fallback); else toast "Extra participant added · $24.50/fortnight charged to your saved card." (plan_upgraded_to still fire-and-forget syncs.)
Fix (mobile `app/participants.tsx` submitAdd): AWAIT sync after an add-on; on no_active_subscription → Alert routing to /plan-billing. Added root testID="participants-page" (parity).
Mobile Invites v3 (`app/family-members.tsx` full rewrite): invite now posts {email, wayly_role:'caregiver', relationship, note} with a horizontal RELATIONSHIP chip row (RELATIONSHIP_OPTIONS mirrors server.py); added "Waiting for your approval" section (Approve → /household/memberships/{id}/approve, Deny → /deny); pending invites now have Resend (/household/invite/{token}/resend) + Revoke (DELETE /household/invite/{token}); shows relationship + capacity counter. testIDs: invite-relationship-<name>, pending-approvals-card, approve-<email>, deny-<email>, invite-resend-<i>, invite-revoke-<i>.
VERIFIED: main agent ran real Stripe (sam@test.com, real trialing Family sub + saved card pm_…) → sync added the additional-participant subscription item, created a paid invoice ($0 prorated during trial — correct), and stamped the DB add-on. testing_agent iter271: backend 3/3 pytest PASS, web + mobile UI parity PASS. Report iteration_271.
IMPORTANT preview limitation: real off-session card charge is only exercisable on accounts with a genuine Stripe subscription+card; seeded accounts (e.g. cathy) have no real sub so they correctly hit the checkout fallback and still show the legacy pending banner until they subscribe.
### Remaining: M-PERMS caregiver guardrails (authz middleware + route guards + scoped Ask Wayly); CHSP invoice-photo OCR (P2); noindex on /app pages (P2); auto sitemap at build (P2). PRD.md 7100+ lines — split into PRD/CHANGELOG/ROADMAP in a housekeeping pass.

## Iter274 — Dashboard Phase 1 (web+mobile) + Flag/issue plain-language redesign ✅ (web verified compile+dates; testing_agent iter272 PASS for dashboard)
User program (confirmed): surface=BOTH; rename "Voice Check"→"Support Care Goals"; order Phase1→5; run design_agent before Phase 3 colour overhaul.
PHASE 1 dashboard cleanup DONE web (CaregiverDashboard.jsx, DashboardActionBar.jsx) + mobile (app/(tabs)/index.tsx, src/components/DashboardActionBar.tsx): removed header block (last-updated pill, plan badge, Share-with-family, Upload-a-statement, Key Contacts); hero now "Good <morning/afternoon/evening>, <Name> — what would you like to do?"; action-bar shortcut tiles each get a distinct teal/clay/deep-teal solid background + white text (removed duplicate heading); removed "Latest statement, in plain English" card; moved Family Thread ABOVE Recent Statements; "Things To Know" restyled to a standout teal-gradient header (white text) + lightbulb icon on suggested actions. testing_agent iter272 = PASS on both surfaces.
...
## Feature — Jun 2026 (MOBILE parity with web: Statement Decoder + Letters & Follow-ups)

Brought the Expo mobile app to full parity with the recent web overhaul. User chose: consolidate to ONE letter detail screen; implement all 5 phases. Verified in iteration_319 (mobile, Expo web preview) — all phases PASS, no bugs.

### Phase 1 — Statement Decoder publish gate (`mobile/src/components/DecoderResultView.tsx`)
- When `result.publishable === false`: render `PublishBlockPanel` (testID `decoder-publish-block`) with per-blocker evidence (`decoder-publish-block-evidence-*`) and "Email my provider about this" buttons (`decoder-publish-block-email-*`), and HIDE the money summary banner, donut graphics, budget-continuity panel and stream breakdown so untrusted totals are never presented as fact.
- `LowConfidenceBanner` (`decoder-low-confidence-banner`) when `low_confidence`; "totals unverified" note (`decoder-table-unverified-note`) on the line-item table when blocked; anomaly cards now show a "FROM YOUR STATEMENT" evidence block (`anomaly-card-*-evidence`).
- NOTE: blocked-state UI verified by code inspection only — no `publishable=false` statement exists on the cathy@example.com test account; non-blocked path verified live.

### Phase 2 — Letters front door (`mobile/src/components/tools/LettersFollowUps.tsx`)
- "Start a blank letter" panel (`lf1-blank-letter-panel`, 6 quick types) + 12 situations grouped into 6 brand-accented families (`lf1-group-*`).

### Phase 3 — Mailbox (`mobile/app/letters.tsx`, rewritten)
- Search (`lf1-log-search`), status filter chips with counts (`lf1-log-filter-*`), type filter Select, delete-with-confirm modal (`lf1-log-delete-modal`), follow-up panel with Escalate (`lf1-followup-escalate-*`). Status chips hide when count=0 by design.

### Phase 4 — Consolidated detail screen
- All draft-from-finding flows (Statement Decoder, Invoice Checker, Support Plan Reviewer, CHSP) now route to `/letters/[id]`. `/app/mobile/app/correspondence/[id].tsx` is now a thin `<Redirect>` to `/letters/[id]`. `letters/[id]` gained a carried-issues panel (`lf1-carried-issues`) + auto-draft-on-arrival for tool-created letters.

### Phase 5 — Required/Optional badges
- Applied to login, forgot-password, reset, profile-edit (signup/participants/onboarding already had them).
