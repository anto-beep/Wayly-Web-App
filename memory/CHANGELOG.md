## Iter 332 — CSC saved-checks + Care Plan auto-save parity + web useRef crash fix (Jun 2026, fork) ✅ verified iteration_332 (backend 12/12 pytest, web all flows green, mobile mounts clean)
Web + mobile + shared backend. From the user's backlog (Message 314) + a reported production crash.
- **FIX (P0, prod crash): "useRef is not defined" on the web Support Plan Reviewer.** `frontend/src/pages/tools/CarePlanReviewer.jsx` used `useRef` (auto-save guard) but imported only `useState, useEffect`. Added `useRef` to the React import. Page now mounts for signed-in family users without the 500/crash.
- **Care Plan Reviewer — mobile AUTO-SAVE parity.** `mobile/src/components/tools/CarePlanReviewer.tsx`: added an `autoSaveRef`-guarded effect that silently saves a completed review to the Care Plans register (calls existing `savePlan()` → `/care-plans/upload` or `/upload-files`) once per review, without the user tapping "Save this plan" (mirrors web). `submit()` resets the guard so each new review re-arms.
- **Classification Self-Check polish (web + mobile).**
  - **Hover shake fixed** (web `QuestionCard`): the per-option anchor text used to render INSIDE the button on hover, growing it and shifting the whole grid. Anchors now render in a fixed-height hint slot BELOW the grid (`csc-q-<id>-anchor`), driven by hovered-or-selected value; option buttons never resize. Verified: Q1 bounding box identical (395×44) at rest vs hover.
  - **Coloured action buttons** (both): Save as PDF = teal (`csc-download-pdf`), Email to Self = gold (`csc-email-self`), Run again = sage (`csc-rerun`). Run again resets to a blank quiz. (Removed the old grey/outline styling + the duplicate bottom rerun link.)
  - **Clearer blocking N-to-go state** (both): the "N to go" line is now a gold callout with an info icon — "Almost there — N question(s) still to answer" (`csc-cta-help`); submit stays disabled until 16/16 (questions remain required, per user choice).
  - **Saved-check management (NEW, both surfaces).** After a result: "Save this check" card (`csc-save-card` → name input `csc-save-name` → `csc-save-submit` → `csc-save-done`). Quiz screen shows "Your saved checks" panel (`csc-saved-panel`) with per-item Edit/reopen (`csc-saved-reopen-<id>` — restores answers into the quiz), Re-run (`csc-saved-rerun-<id>` — re-scores immediately), Delete (`csc-saved-delete-<id>`).
- **Backend CSC CRUD (`routes/csc.py`).** `_store_run` now also persists the raw `answers` + `current_classification` (so a saved check can be re-opened/re-run), and stamps `name:null, saved:false` on insert. New user-scoped endpoints: `GET /api/public/csc/runs` (lists SAVED-only, newest first), `PATCH /api/public/csc/runs/{id}` ({name, saved}), `DELETE /api/public/csc/runs/{id}`. All require auth (401 otherwise), 404 on missing/other-user ids. New pytest `backend/tests/test_csc_saved_runs_iter332.py` (12 green).
- **Verified**: testing_agent iteration_332 — backend 12/12; web full save/reopen/rerun/delete + hover-no-shift + coloured buttons + useRef fix all green; mobile CSC + CarePlanReviewer tool pages mount cleanly (390×844, 0 red-box) with all testIDs present + autoSaveRef guard confirmed. Mobile full runtime save-loop + CPR upload→auto-save NOT exercised end-to-end (token budget) — recommend a manual Expo walkthrough.
- **Note**: fork env has Expo SDK 54 installed in node_modules though `mobile/package.json` pins ^57.0.0 (prior session's upgrade); restored the committed SDK-57 `mobile/yarn.lock` after the testing agent's run rewrote it. No dependency changes made this pass. Known/next (backlog): Recipient Directory management screen, blocker-bundle email preview, Statement Decoder Trust Overhaul (P1).



## Iter 329 — Dashboard accuracy + Strict Doc Blocking + Care Plan Wayly Summary + Letters polish + Q&A formatting (Jun 2026, fork) ✅ verified iteration_327 (web 7/8 browser + backend curl)
WEB frontend + shared backend. Mobile UI parity NOT done this pass (shared backend changes below already benefit mobile where it calls the same endpoints).
- **Dashboard Wayly Summary now matches the graphics.** The summary was telling users money was "available to spend" while the donut showed 187% over budget. Fixed the `/insights/summarise` dashboard context (CaregiverDashboard.jsx) to pass unambiguous fields (over_budget, quarterly_spent_aud, quarterly_budget_aud, budget_percent_used, amount_over_budget_aud, amount_remaining_aud) computed the SAME way as the donut, and added a dashboard prompt override (`_DASHBOARD_SUMMARY_OVERRIDE`) that leads with the over-budget fact and NEVER says "available/left to spend" when over. Curl-verified: "$10,513.10 spent against $5,634.20, 187% used, $4,878.90 over".
- **Open-anomaly count = latest statement only.** Summary + Things To Know no longer aggregate across every statement. `open_anomaly_count` → `latest_statement_open_anomalies` (latest statement) + `latest_invoice_issue_count` (latest invoice).
- **Things To Know rebuilt.** Subtitle "Based on your latest statement and invoice"; two sections — **Statements** (latest statement anomalies) and **Invoices** (latest invoice reconciliation findings, loaded via `/invoices`). Count badge = latest statement + latest invoice only.
- **Statement-detail Wayly Summary is now richer.** `_enrich_context` adds the latest invoice check (provider, verdict, issue count) alongside complaints + letters standing; the two-paragraph override weaves them in. Curl-verified: "three open complaints still active, four letters awaiting a reply … invoice check 17 issues". Other pages' summaries unchanged (single paragraph).
- **Strict document blocking (Statement Decoder + Care Plan Reviewer).** "Always block unless clearly the right type." Backend: care_plans text+file cores + public/authenticated statement decode now return `{upload_guard}` for ANY non-`accept` verdict (wrong_tool, unrelated, low_confidence, unreadable) so no numbers are ever produced for the wrong file. Frontend: `UploadGuardNotice` gained a `strict` full-screen variant (no "Continue anyway"); wired into CarePlanReviewer, StatementDecoderTool, StatementUpload. Verified: a recipe blocks; real statement/care-plan classify accept (no false positives).
- **Care Plan Reviewer Wayly Summary.** The overview card is now branded "Wayly Summary" (sparkle) and adds a "What we could not check" list (unread sections + missing classification/budget/dates/services).
- **Letters & Follow-Ups polish.** Renamed title/SEO/gate to "Letters & Follow-Ups"; the "Or pick the situation…" prompt is now a prominent highlighted pill; situation card labels de-dotted + capitalised (`personaliseSituationLabel`). Editor: intake fields LOCK while generating (`ArchetypeIntakeForm` wrapped in a disabled `<fieldset>`, `GenerateButton` reports busy via `onBusyChange`); the draft NO LONGER auto-appears on arrival — prefill/tool-carry now seed the intake only and the draft shows only after the user clicks "Generate letter".
- **Aged Care Q&A (Ask Wayly).** Added a formatting rule to the aw2 system prompt: short paragraphs + key figures/dates/names bolded inline (rendered via existing MarkdownText). Added a "Report an Issue" button BELOW the chat bar (AskWaylyV2.jsx).
- **Known/next**: mobile UI parity for all of the above; aw2 STREAMING Q&A still on-loop (latent freeze); minor React `<span> in <option>` console warning in CarePlanReviewer classification dropdown (pre-existing, not user-visible).


## Iter 328 — P0 critical bug batch: letter-draft Cloudflare timeout, dashboard freeze, ghost drafts (Jun 2026, fork) ✅ backend 16/16 verified (iteration_326 bug suite)
User's top priority ("bugs first"). All three are production-impacting; fixed on the shared backend + both surfaces.
- **BUG1 — AI letter drafting Cloudflare 524 timeout.** The LF-1 `/generate` endpoint held the HTTP request open for the full 30-70s LLM call, tripping the edge timeout. Converted letter drafting to an async job + poll (mirrors the Care Plan Reviewer pattern): new `POST /api/lf1/correspondence/{id}/generate-async` (returns `{job_id}` instantly) + `GET /api/lf1/generate-jobs/{job_id}` (poll). `_do_generate` runs in the background under a semaphore; the `source_data_missing` gate surfaces via the job's `error_detail.missing_fields`. `/prefill` no longer generates synchronously (seeds intake only, returns `needs_generation`); the client then fires generate-async. Shared helpers `frontend/src/lib/lf1Generate.js` + `mobile/src/lib/lf1Generate.ts` submit+poll. Wired into web `CorrespondenceDetail.jsx` + `GenerateButton` and mobile `app/letters/[id].tsx` (auto-draft, prefill, on-import regen, manual Generate). Curl-verified: generate-async → done with a real letter.
- **BUG2 — production dashboard stuck on skeletons (event-loop freeze).** RCA (troubleshoot_agent): raw `emergentintegrations.LlmChat` calls ran DIRECTLY on the request event loop, so a hung LLM gateway froze the whole single-worker loop and every dashboard GET (and `/api/health`) stalled. Fix: routed the on-loop LLM offenders through `lib/llm_wrapper.chat_send` (dedicated thread executor + `wait_for` hard timeout + circuit breaker): `routes/insights.py::_generate_summary` (the dashboard's Wayly Summary — the direct blocker), plus `agents._llm_chunk_call` (6 parallel statement-decode extractors), the decode auditor, `agents.parse_statement`, `agents.explain_anomalies`, `agents.chat_with_kindred`, and `lib/inv1/summariser.generate_summary`. Added `timeout_sec` passthrough to `llm_wrapper.chat_send`. Verified: concurrent `/auth/me` stays fast while `/insights/summarise` is in flight; all 8 dashboard GETs 200 fast; 20 decoder gate tests still green.
- **BUG3 — "Leave without saving" ghost drafts cluttering the mailbox.** Discard now genuinely deletes brand-new unsaved drafts. Backend: `/autosave` sets `user_saved=true` (explicit save). Both surfaces: the leave-modal Discard deletes the entry via `DELETE /lf1/correspondence/{id}` ONLY when `status==='draft' && !sent_at && !user_saved`; a previously-saved or sent letter is preserved (saveDraft also flips local `user_saved`). Curl-verified: fresh entry user_saved falsy → autosave sets true → delete → 404.
- **Verified**: testing_agent backend suite `test_iter326_bugs.py` 16/16 PASS across all three bugs. Web compiles clean. Mobile UI flow not browser-exercised this pass (shared API contract covered); mobile restarted.
- **Known/next**: `agents.stream_chat_with_kindred` + aw2 streaming Q&A are still raw on-loop (latent freeze risk) — to be converted with the Aged Care Q&A P0 task. `lf1_generate_jobs` has no TTL (tiny rows; sweep later).

### Still pending — the big P0 UI/UX batch (next pass), all Web + Mobile
- Strict document validation (Statement Decoder + Care Plan Reviewer): always block unless clearly the right type, full-screen prompt, no numbers rendered. Invoice Checker prominent wrong-doc prompt.
- Statement Decoder: remove "Forward by email".
- Care Plan Reviewer: add Wayly Summary, state what couldn't be checked, draft-email option, capitalise "Your Saved Plans", auto-save.
- Provider Price Checker: capitalise service entries/fields, prefill provider (editable), required Service & Rate, capitalised "Your Price History" button.
- Classification Self-Check: required fields, persona-aware headings, save/name/delete/edit/re-run, distinct button colours, fix web hover glitch.
- Letters & Follow-Ups: rename title, highlight "Or pick the situation…", de-dot + capitalise category titles, block field edits while generating, don't show draft until Generate clicked.
- Aged Care Q&A: move "Report an Issue" below the chat bar; short paragraphs + inline bold key info (also convert aw2/streaming off-loop here).
- P1: mobile dashboard/profile polish parity; Short-Term Pathways tool (web+mobile); cross-platform parity test.


## Iter 327 — P0 batch: Save-as-Draft (no autosave), Reviewer PDF match, Printable Estimate, mobile funding parity (Jun 2026, fork) ✅ verified iteration_325 (backend+web) + mobile self-test
Four P0 backlog items, web + mobile. User choices: Save-as-Draft persists EVERYTHING (intake + generated body); Printable Estimate built from the Budget & Lifetime Cap Calculator.
- **#1 Save as Draft replaces silent autosave (letter editor).** Removed the 1s debounced silent autosave on BOTH surfaces (web `pages/tools/CorrespondenceDetail.jsx`, mobile `app/letters/[id].tsx`). Added a visible **"Save as draft"** button (`lf1-detail-save-draft`) that PATCHes `/lf1/correspondence/{id}/autosave` with intake + `content_draft` (the generated letter body) + sender authority/complaint mode/atsi, showing a **"Saved"** hint (`lf1-detail-saved-hint`) and an **"Unsaved changes"** hint (`lf1-detail-unsaved-hint`) while dirty. Added a **"Save before leaving?"** prompt (`lf1-detail-leave-modal`, actions `-leave-save` / `-leave-discard` / `-leave-cancel`) that fires when leaving the editor with unsaved changes. Web: `beforeunload` guard for tab close/refresh + a guarded back button (`guardedNav`). Mobile: `usePreventRemove` (hardware/gesture back) + guarded header-back and mailbox-link (`lf-log-link`) → same bottom-sheet prompt; `runPendingNav` clears dirty before dispatching so no re-intercept loop. Verified live on both surfaces.
- **#2 Reviewer PDF Match.** The Support Plan Reviewer downloaded PDF now includes the SAME safety checks + safety-notice the family sees on screen. `services/care_plan_pdf.py`: added a `safety_notice` param (amber box, matches the on-screen banner) + relabelled the verification section to **"SAFETY CHECKS WE RAN"** with on-screen status wording (**All good / Worth a look / Need more info**, flag→clay). Threaded `safety_notice` through `routes/care_plans.py` (`SummaryPdfBody` + `/care-plans/summary.pdf` + saved-plan `/care-plans/{id}/artefact.pdf` via `cpr_safety.safety_notice()`) and through `routes/tool_exports.py::_render_care_plan` (which previously dropped verification_panel/plan_summary/safety_notice entirely — the mobile export bug). Web `CarePlanReviewer.jsx` download now posts `safety_notice`. Verified: PDFs 200, contain the checks + amber notice.
- **#3 Printable Estimate (web).** Added a **"Download printable estimate"** button (`bc-download-estimate`) to the web Budget & Lifetime Cap Calculator result (`pages/tools/BudgetCalculatorTool.jsx`, shown when `access==="allowed"`) that POSTs `/public/exports/pdf {tool:"budget"}` and downloads the existing one-page Wayly-branded `budget_pdf.py` sheet. Mobile already had this export (ResultActions `bc-export`).
- **#4 Mobile funding-figure / $-format parity — audited, already in parity.** Confirmed mobile classification figures (July-2026 indexation) are byte-identical to web (`mobile/src/data/budgetTool.ts` == web fallback: Cl4 annual 30468.51 etc.), the formatters are equivalent (`money`≡`formatAUD2` 2dp, `moneyWhole`≡`formatAUD` whole $), and the Budget Calculator, Contribution Position, and BudgetRunway all use the same per-figure precision as web. Lifetime-cap constants match ($137,917.01 / $86,185.23). No stale pre-July-2026 figures found; no code change needed.
- **Verified**: testing_agent iteration_325 = 4/4 P0 GREEN (backend pytest `test_iter325_p0.py` + web UI). Mobile Save-as-Draft leave-guard self-tested live on the Expo preview (edit → Unsaved changes → tap mailbox link → "Save before leaving?" sheet with all 3 actions). No bugs.

### Still pending (backlog, not in this batch)
- P1 Mobile dashboard/profile polish parity (collapsible Wayly Chat, budget-status pill, profile card tints).
- P1 Short-Term Pathways user tool (Restorative / End-of-Life) — web + mobile (reviewer checks already shipped).
- P2 Blocker Letters Bundle (one email to provider for all statement blockers); Corrected Re-check (mark blocker provider-fixed → re-run one statement); Recipient Directory (save provider name+email → pre-address future letters).



## Iter 326 — Dashboard number sizing + richer statement Wayly Summary (Jun 2026, fork) — agentic edits
- **Dashboard budget figures** (`pages/CaregiverDashboard.jsx`): the "left" and "budget" dollar figures are now the SAME moderate size (`text-3xl`, down from `text-5xl/6xl`) with small connecting words ("of", "this quarter"), and the "spent so far" line is bolder + more visible (`text-base`, number `font-bold`). Both figures use `formatAUD2` so they never overstate.
- **Statement Wayly Summary is now a richer, multi-paragraph overview** (`routes/insights.py` + `components/SmartAISummary.jsx` + `pages/StatementDetail.jsx`): for `page_key="statement-detail"` the backend enriches the context server-side with the participant's cross-tool standing (open complaints, letters total / awaiting-reply / in-draft, queried from `complaints` + `lf1_correspondence`) and a page-specific prompt override produces TWO-THREE short paragraphs (~110 words): para 1 speaks to the statement, para 2 gives the participant's overall standing across Wayly. `_sanitise_prose` now preserves paragraph breaks; SmartAISummary splits on blank lines and renders one `<p>` per paragraph (single-paragraph pages unchanged). StatementDetail now passes `participant_id` in context. Verified via API: 2 paragraphs, cross-tool figures pulled through.


## Iter 325 — Money-accuracy + auth-light + tool→list nav buttons (Jun 2026, fork)
User batch (priority order, no stopping): fix money overstatement, highlight indicative split, disable dark mode on auth pages, add tool→list navigation buttons.
- **Dashboard money overstatement FIXED** (`pages/CaregiverDashboard.jsx`): the headline "left to spend" used `formatAUD` (whole dollars, rounds UP → "$4,113 of $4,112.84" — showed MORE money than exists). Now uses `formatAUD2` (matches the total's 2-dp precision) for the left + spent figures, so it reads "$4,112.84 of $4,112.84" — always matches, never overstates.
- **Indicative-split callout made prominent** (dashboard): when the per-category split is not from a statement, the note is now a bold 2px-gold-bordered callout with an Info icon and an "Indicative split only" pill + a clear line ("These per-category figures are an estimated split, not your provider's actual numbers. Upload a statement to see the real split."). When it IS from a statement it stays a quiet sage note.
- **Auth pages no longer use dark mode** (`App.js` AppearanceScope + `components/AccessibilityWidget.jsx` applyPrefs + `uxf/theme.jsx` applyTheme): /login, /signup, /forgot, /reset, /verify-email always render light (identical to light mode) regardless of the saved appearance pref — fixes the unreadable Google button in dark mode. The guard lives in ALL three theme writers, including the lowest-level `applyTheme` in the UXF ThemeProvider, so a HARD REFRESH on those routes stays light (the ThemeProvider mount-effect was previously re-adding theme-dark after the route effect removed it). Verified via hard reload: theme-dark=false, data-theme=light on /login and /signup even with dark pref set.
- **Tool → list navigation buttons** (new `components/ToolEntriesButton.jsx` web + `src/components/ToolEntriesButton.tsx` mobile): a clear, consistent, premium less-rounded solid button on each tool page linking to where its entries live — Statement Decoder → Statements, Invoice Checker → Invoices, Letters & Follow-ups → My Mailbox, Support Plan Reviewer → Care Plans. Wired into web `ToolHero` + the Statement Decoder custom header, and into all four mobile tool screens. Web verified via screenshot ("View My Statements"); mobile bundles clean.


## Iter 324 — Mobile polish mirrors + mobile letters E2E (Jun 2026, fork) ✅ mobile E2E verified (iteration_324)
Mirrored the remaining web-only polish to mobile and ran a full mobile letters E2E.
- **Mobile download bar** (`src/components/invoices/InvoiceResultView.tsx`): buttons now distinct brand colours — Original=sage, CSV=clay (`gold` token #A5512B), PDF report=teal (primary) — white text (matches web).
- **Mobile duplicate-invoice notice** (`src/components/tools/InvoiceChecker.tsx`): made prominent — 2px gold border, gold icon badge, "ALREADY CHECKED" pill, clearer heading, stacked Open/Check-another buttons.
- **Mobile upload** (`InvoiceChecker.tsx`): empty state is now a large tap-zone (circular Upload badge + "Tap to choose your invoice" + format hint) instead of a bare button (drag-drop N/A on native).
- **Mobile MissingDetailsBanner**: dismissal now persists ~2.5 months (was 24h) to match web ProfileInlinePrompts — no longer nags every visit.
- **Mobile letters E2E (iteration_324) PASS on the headline flow**: tapping "Draft a letter about this" on an invoice finding opens /letters/{id} and auto-drafts a populated letter (carried-issues card + finding narrative pulled through, NO 422 'need more info'); invoice donut parity + no Tier-chip banner confirmed. Transient /generate 502s (LLM gateway) noted, not the intake bug. Mobile has no related-tool cards (that web tint item N/A).


## Iter 323 — "Draft a letter" now prefills on EVERY tool + mobile parity sweep (Jun 2026, fork) ✅ verified 17/17 (iter323_letters_intake) + web iter322
User: "clicking draft a letter about this from inside the invoice isn't creating a draft letter prefilled with all the details — this must work, it's the biggest selling point. Apply the same fix to every draft-a-letter button and every tool, and apply ALL fixes from this whole chat to the mobile app."
- **ROOT CAUSE**: the invoice + CHSP letter bridges inserted the LF-1 correspondence with an EMPTY intake ({}), so `POST /lf1/correspondence/{id}/generate` returned 422 `source_data_missing` and the editor's auto-draft silently failed -> blank letter. (The statement-decoder / care-plan bridge in routes/care_plans.py was already fixed via `_compose_letter_intake` in iter295.)
- **FIX — invoices** (`routes/invoices.py`): both `create_letter_from_finding` and `create_letter_from_all_findings` now call `_compose_letter_intake(...)` (imported from routes.care_plans) with a plain-English `_LETTER_CHECK_TITLES[C1..C12]` map, so the intake carries participant_name + the archetype's required summary field (disputed_charge_summary / escalation_summary) + subject + prior_attempts. Verified: GET correspondence shows a 6-key non-empty intake, generate no longer 422s.
- **FIX — CHSP** (`routes/chsp1.py::create_chsp_letter`): builds a real intake for service_continuity (request → change_summary) and hardship (notification → notification_summary), AND falls back to the household's primary participant when there is no chsp_profiles row (mirrors server._resolve_active_participant) so participant_name always populates.
- **Draft-letter prefill now works end-to-end on BOTH surfaces**: web CorrespondenceDetail and mobile app/letters/[id].tsx both already auto-draft from carried issues on arrival; the backend intake fix makes that draft populate instead of 422.
- **MOBILE parity of this chat's fixes** (`/app/mobile`): (1) severity-inversion bug fixed in `InvoiceResultView.tsx` severityKey + `InsightGraphics.tsx` invBandOf (t>=4→high, t3→medium, t1/2→low/info) — the C10 lifetime-cap note no longer shows as a red HIGH panic flag; (2) C10 title neutralised to "About your lifetime contribution cap"; (3) run-result view now renders the SAME layout as the saved invoice page (added InvoiceInsightGraphics donuts, removed the technical Tier-chip VerdictBanner + duplicate InvoiceMetaCard from the default InvoiceResultView); (4) evidence text humanised via new `plainEvidence()` in utils/format.ts (strips INDEX-1 + RULE codes + snake_case, AU dates, capitalised) used in DecoderResultView. Backend-shared fixes (C10 wording, Wayly Summary prompt, G3 gate plain English, tool_registry.yaml 500 fix) reach mobile automatically.
- Deferred (web-only polish, not yet mirrored / not applicable to mobile native): invoice upload drag-drop redesign (mobile uses native picker), download-bar button colours, duplicate-notice restyle, related-tool tints, ProfileInlinePrompts dismissal persistence (mobile uses a different banner).


## Iter 322 — Invoice Checker: severity-inversion fix, run==saved parity, lifetime-cap panic fix, agentic visual edits (Jun 2026, fork) ✅ verified iteration_322 (web)
Driven by user screenshots of the Invoice Checker.
- **CRITICAL severity-inversion bug FIXED (web)**: backend INV-1 `Tier` is higher=more-severe (`T4_check_before_paying`=4 top … `T1_informational`=1 lowest), but the web frontend mapped tier1→critical and tier4→low — fully inverted. So the informational C10 lifetime-cap note (T1) rendered as a red HIGH "Needs attention" flag (user panic) while the real overcharge (T4) showed as "low". Corrected `_severityKey` in `components/invoices/InvoiceResultView.jsx` and `bandOf` in `components/InvoiceInsightGraphics.jsx` to t>=4→high, t3→medium, t1/2→low/info. Verified: C10-only invoice now sits under "Low / For your info" (green) with banner "1 small note, mostly informational"; rich Meridian (T4×4/T3×11/T1-2×2) groups 4 High / 11 Medium / 2 Low.
- **Run == Saved parity**: extracted a single shared `InvoiceResultBody` (banner → InsightGraphics donuts → metadata strip → download/compare bar → Wayly Summary → draft-all → Issue Register → charges) used by BOTH the fresh-run `InvoiceCheckerTool` and the saved `InvoiceDetail`, so they look identical. Removed the technical `VerdictBanner` ("16 lines read · 1 Tier 4 · 1 Tier 3") + duplicate `InvoiceMetaCard` from the run view; the plain-English "What we found" donut + Issue Register banner now convey "check before you pay".
- **Lifetime-cap flag de-panicked + clarified (backend `lib/inv1/checks.py` C10)**: neutral title "About your lifetime contribution cap"; narrative explains it's a lifetime limit whose final figure the government (Dept of Health and Aged Care) has not published yet, so we cannot show how close you are — "for your information only, not because anything on this invoice is wrong"; action = "Nothing to do right now … Wayly will start showing how close you are once the government publishes it." (Applies to new runs; title fix is immediate on saved too.)
- **Wayly Summary more valuable (backend `lib/inv1/summariser.py`)**: prompt rewritten to lead with the bottom line (fine to pay vs raise first), name only the 1–2 things that matter most with $ figures, end with ONE clear suggested next step; explicitly told not to alarm about the lifetime-cap note. (New runs only.)
- **Download-bar buttons** now each a distinct standout brand colour: Compare amber #F0B267 / Original sage / CSV clay #A5512B / PDF teal (fixed a Compare-vs-CSV colour collision where `bg-gold`==#A5512B). Potential-refund figure bolded to text-3xl.
- **Fixed a pre-existing 500 on `GET /api/tools`**: `backend/data/tool_registry.yaml` line 119 had `name` and `short_description` merged onto one line (YAML ParserError). Split them; endpoint returns 200 again (AI Tools registry restored).
- **Agentic visual edits (web)**: FlagCard "From your statement" evidence now humanised (no INDEX-1/rule/snake_case codes, DD/MM/YYYY, capitalised) + `lib/plainText.js` gained INDEX-1 + RULE-code strippers; `RULE_G3_BUDGET_NOT_IN_INDEX` gate text rewritten to plain English (explains what/why, no jargon); ProfileInlinePrompts dismissals now persist ~2.5 months (localStorage) so a closed prompt no longer reappears every visit; InvoiceCheckerTool upload redesigned to a large drag-drop zone matching the Statement Decoder; its related-tool cards given brand tints; duplicate-invoice notice made prominent with two distinct less-round buttons.


## Iter 321 — Big batch: auth dark-mode, invoice draft parity, fee transparency, button restyle + backend (Jun 2026, fork) ✅ core items verified iteration_321 (web + mobile)
User requested ~17 items "in one go". Completed + verified this session:
- **Auth dark mode (web) [BUG, screenshotted]**: `.auth-shell`/`.auth-card` restyle — lighter warm-teal card with a clay top accent (was near-black on near-black), primary submit buttons + accent links now CLAY (not teal), selected role/plan cards highlighted clay, and a clearly-readable "Sign in with Google" label above the widget (`GoogleSignInButton.jsx`). Forced `color-scheme: light` on the Google iframe so its text isn't white-on-white. NOTE: the GIS widget renders icon-only in preview (origin-restricted) — full text shows in production; the caption guarantees clarity regardless. Mobile auth parity = follow-up.
- **Invoice draft-letter parity (web)**: removed the standalone clay "Draft a letter" (draft-all) button from `InvoiceIssueRegister`; wired per-issue "Draft a letter" on each finding (parity with statement anomalies) via `onDraftFinding`; kept the "Draft one letter covering all N issues" button.
- **Fee Transparency (web + mobile)**: new `FeeBreakdown` "Where the money goes" panel on the statement decoder — Services + Care management + Package management + GST rows + a reconciling "Total billed" (testids decoder-fee-breakdown / decoder-fee-total).
- **Confidence Legend (web + mobile)**: "How we read this" expandable note explaining low-confidence + unverified figures (testid decoder-confidence-legend).
- **App-wide button restyle**: shared shadcn `Button` + a mechanical sweep (153 hover-motion classes removed, 76 transition-all→transition-colors, 82 rounded-full→rounded-lg on buttons/links) = sleek, less-round, no hover animations across the frontend.
- **Prod cleanup script**: `backend/scripts/prod_cleanup_test_accounts.py` (DRY-RUN by default, keeps admins, cascades) to remove test accounts from a PROD db. Prod/test DB separation is a deploy/config action (distinct DB_NAME) — cannot run against prod from preview.
- **Gateway Guardrail / health RCA**: confirmed backend LLM path already runs off the event loop via a dedicated executor with a 75s timeout + circuit breaker, and `/api/health` cheap liveness is dependency-free — so a slow gateway can't freeze the app or stall health. Frontend letter-generate already surfaces an error + re-tryable action.

DEFERRED (explicitly, for next turn): Reviewer PDF Match; Printable Estimate; Letter Preview PDF (true server PDF); Short-Term Pathways Tool (web+mobile); Recipient Directory; Blocker Letters Bundle (statement); Corrected Re-check; Draft letter auto-save prompt; mobile dashboard/profile polish parity (collapsible Wayly Chat, budget pill, profile tints); mobile funding-figure/$-format parity sweep; mobile auth dark-mode parity. Bulk Follow-up was already shipped in iter320.
...
- **Badge Everywhere (mobile)**: swept Required/Optional badges onto shared `Field`/`Select`/`DateField` across 22 forms (74 badges) — complaints, contacts, amendments, hospital, athm, provider-switch, ratings, referrals, scenarios, handover-pack, carer-self-check, security, support, case, athm-project, switch-settlement, chsp-tools, pacing, budget-scenarios, documents, calendar. `(optional)` label text converted to the muted Optional badge; mandatory inputs get the gold Required badge. App-wide consistency with the existing convention.
- **Bulk Follow-up (web + mobile)**: new deterministic (no-LLM) endpoint `POST /api/lf1/follow-ups/bulk-send {entry_ids[]}` — for each outstanding outbound letter it logs a chase-up to `follow_up_history`, resets `follow_up_date` a fresh window forward, moves status → `awaiting_response`, and returns a plain-English chase-up note; skips + reports non-outstanding entries. Mailbox Follow-ups panel gains a "Chase up" select mode with per-row checkboxes, "Select all overdue", and a "Send N chase-ups" action on both surfaces. Seed helper `backend/scripts/seed_followups.py` (3 overdue + 1 upcoming; bulk-send is destructive, re-run to reset).
- **Letter Preview (web + mobile)**: a "Preview letter" button on the letter detail opens a WYSIWYG letter-paper modal built from the draft (`subject`/`body`/`cover_note`) + sender name/email/authority — Wayly letterhead, From/To blocks, date, subject, body paragraphs, OPAN reference, cc line, and an "assistant not legal advice" footer — exactly as the provider will receive it, before sending. Web `LetterPreviewDialog` in `CorrespondenceDetail.jsx`; mobile `LetterPreviewModal` in `app/letters/[id].tsx`.


## Letters & Follow-ups overhaul + login hang fix (Jun 2026, fork) ✅ verified (4 pytest + live UI)
Reworked the whole web Letters & Follow-ups ecosystem plus a critical login fix.
- **Accurate drafts from tools (`routes/care_plans.py`)**: replaced addressee-only routing with `_resolve_situation(source_tool, addressee)` so a finding becomes the RIGHT letter type — statement-decoder / invoice-checker / price-checker → **dispute** (was wrongly "request"); care-plan-reviewer → request; regulator addressees still win. Rewrote `_compose_letter_intake`: uses the participant's **full name** (first+last), a **source-aware lead-in** via `_SOURCE_LEADIN` (fixes the false "came out of a review of the support plan" — now "reviewing … Support at Home statement" etc.), correct `dispute_type`/`change_type` defaults, carried reference number, and `provider_name`/`recipient_name`. Verified by `tests/test_iter318_letters_overhaul.py` (4 pass).
- **Front door redesign (`LettersFollowUps.jsx`)**: 12 situations grouped into 6 plain-English families with brand accents; added a **"Start a blank letter"** panel (letters no longer limited to scenarios); wider (max-w-5xl); "Your mailbox" CTA.
- **Mailbox redesign (`CorrespondenceLog.jsx`)**: rebuilt the log as a mailbox — search, status-filter chips with counts, type filter, compact rows with status chips + type + recipient + follow-up/sent dates, **inline delete** (confirm modal → DELETE /lf1/correspondence/{id}), condensed follow-up panel, corrected grammar/capitalisation, wider.
- **Letter output redesign (`lf1/LetterGeneration.jsx` OutputFormatSwitcher)**: renders as a clean letter "paper" (gradient bar, serif subject, readable body), with **Copy** + Download PDF and an Email/MAC-portal toggle. Added a shared **recipient block** (who it's going to) and marked required fields with the existing app-wide **RequiredBadge** ("Required" chip) — deleted a mistaken red-asterisk duplicate to stay consistent with the Feb-2026 product convention.
- **Removed the duplicate "Letters Mailbox"**: deleted `pages/LettersMailbox.jsx`; `/app/letters` now redirects to `/tools/letters-and-follow-ups/log`; updated the sidebar nav, ShortTermPathways, and InvoiceDetail (its legacy `?compose=1` bulk flow now uses the backend bridge `POST /invoices/{id}/letter`).
- **CRITICAL login hang fixed (`rate_limit.py`)**: the remote Redis (REDIS_URL) is unreachable from preview and `from_url` had no connect timeout, so `_get_redis().ping()` hung EVERY login >90s (all rate-limited endpoints). Added `socket_connect_timeout`/`socket_timeout=0.75` + `retry_on_timeout=False` and a process-level `_redis_disabled` flag so it fails open fast. Login now ~0.8s.
- **Known infra note (not from this change)**: one letter auto-generation froze the backend event loop for ~6 min on an LLM-gateway hang (the 70s provider timeout wasn't honoured that once). Generation otherwise works (~8s) and produced a correct, high-quality dispute letter. Pre-existing LLM-gateway fragility.


Three follow-on improvements to the DEC-1 web decoder (`/api/public/decode-*`). The mobile `/sd3/.../decode-v2/stream` decoder is a separate SSE pipeline that shows per-line results (no trusted-totals banner), so it is unaffected and needs no parity work.
- **Fewer gross-reconcile false blocks (G2 rewrite, `lib/dec1_gates.py`)**: `reported_total_gross` semantics vary by provider (Sam Burke's includes care management + package management + GST; Margaret's is services-only). G2 now reconciles against a BAND — from services-only up to services + every known separate amount (care management, package management, GST, charged cancellations; minus credits). Inside the band → reconciles silently. Above the band (we're short): a modest shortfall (≤20% of total, usually a fee line or one duplicate) is a **non-blocking MEDIUM** flag; only a >20% shortfall hard-blocks. Below the band (we exceed even the services-only total) → over-count hard-block. Result: a normal care-management fee no longer falsely blocks; Sam Burke's G2 dropped from a blocker to a Medium flag (12.9% shortfall) while it still correctly blocks on G1 + G3.
- **Extraction robustness (`agents.py`)**: added header fields `package_management_deducted` + `gst_total` (fees that belong in the total but aren't line items) and a stream-extractor rule to never merge SAME-DATE duplicate rows (e.g. two identical 28/08 domestic-assistance visits). LLM extraction of these remains best-effort/non-deterministic; the G2 band is the deterministic backstop.
- **Fix-It Checklist (`DecoderResultView.jsx`)**: each publish blocker now shows a one-tap "Email my provider about this" button wired to the existing `onDraftLetter` flow.
- **Source Highlighting (`FlagCard.jsx` + publish block)**: every flag and blocker now renders a "From your statement" evidence block showing the exact figures/maths the finding came from (all gate anomalies already carry `evidence`).
- **Verified**: 64 deterministic decoder unit tests pass (fixed a Margaret over-count regression from the CM-add via the band; updated 2 stale RULE_15 tests that the P0 G2 gate supersedes). Live Sam Burke PDF: `publishable:false`, blockers G1 + G3 only, G2 now Medium. UI screenshot (route-stubbed blocked result): publish-block panel + per-blocker email buttons + "From your statement" evidence + low-confidence banner render; summary banner and insight charts correctly hidden; line-item table shows the "unverified" note. Pre-existing (not from this change): `test_iter15_rules.py` em-dash + live-endpoint/daily-limit tests still fail.

## Statement Decoder trust overhaul — P1/P2 gates + frontend publish gate (Jun 2026, fork) ✅ verified (unit + live PDF + frontend E2E)
Completes the P0 work below. Backend gates P1/P2 and the frontend rendering of the publish gate.
- **Backend `lib/dec1_gates.py` (P1/P2)**: added G7 transparency (`RULE_G7_WAYLY_RECATEGORISED`, MEDIUM, `origin:"wayly"`) so Wayly's own stream recategorisations never read as provider errors (uses new `source_stream` extraction field); G8 travel double-billing (`RULE_G8_TRAVEL_DOUBLE_BILL`); G9 upcoming category transition from INDEX-1 `policy_date.personal_care_free` (`RULE_G9_CATEGORY_TRANSITION`, info); G10 package-management 5% cap (`RULE_G10_PKG_MGMT_CAP`) reading new INDEX-1 row `package_management.cap_pct=0.05` (eff 2026-07-01); low-confidence banner (`audit.low_confidence`/`extraction_confidence` when min per-line confidence < 0.7). New schema fields `source_stream` + `charged_amount` added to the extractor prompt in `agents.py`.
- **Frontend publish gate (the real gap this fork closed)** — `components/DecoderResultView.jsx` now reads `publishable`/`publish_block`/`low_confidence`:
  - When `publishable === false`: renders a prominent `decoder-publish-block` panel at the top (blocker headlines + reason), and HIDES the trusted summary-number banner, balance panel, stream breakdown and persona hero so untrusted totals are never presented as fact. Anomaly flags + full line-item table still show, with a `decoder-table-unverified-note`.
  - Low-confidence banner (`decoder-low-confidence-banner`) shown when `low_confidence`.
  - `pages/tools/StatementDecoderTool.jsx`: also gated `SmartAISummary` and `StatementInsightGraphics` behind `publishable !== false` (they consumed the untrusted totals as text/charts).
- **Verified**: `backend/tests/test_dec1_gates.py` 11/11 pass. Live E2E on the real Sam Burke PDF (`scripts/run_sam_burke.py`): `publishable:false` with blockers G1_LINE_ARITHMETIC, G2_GROSS_RECONCILE, G3_BUDGET_NOT_IN_INDEX, G3_QTD_OVER_BUDGET (+ medium G6/G8, info G9). Frontend E2E (iteration_317): 18/18 assertions pass — blocked path shows publish-block panel and hides all trusted-total surfaces; clean path renders normally. Note: the preview's 1-per-120-day free-decode limit is now consumed, so future anonymous live decodes on this fingerprint 429 (the testing agent used route-stubbed payloads matching the real backend contract).



Context: user ran the Sam Burke Aug 2026 SAH statement and the decoder published a self-contradicting summary (budget $33,540 not a real classification, QTD way over budget, line items not summing to header, meal-prep arithmetic wrong, cancellation charged then zeroed). Root cause: no arithmetic backstop, no INDEX-1 budget validation, no publish gate. Approved plan: add deterministic gates (keep the LLM as-is), hard-block self-contradicting summaries. Phase P0 of P0–P2.
- **New `backend/lib/dec1_gates.py`** — deterministic, model-independent gates run at the end of the DEC-1 audit (`agents._add_parse_warnings`, after dedup + before the impact cap):
  - **G1 per-line arithmetic** (`RULE_G1_LINE_ARITHMETIC`, HIGH): rate × units must equal gross; caught meal prep $85.75 vs $85.50. A $0 gross with a valid rate × units is treated as a dropped figure (`RULE_G1_ZERO_GROSS`, HIGH).
  - **G6 charged cancellation** (`RULE_G6_CANCELLATION_CHARGED`, MEDIUM): a line marked cancellation that still carries a fee → policy-review flag ($76.50 caught).
  - **G3 budget validity** (`RULE_G3_BUDGET_NOT_IN_INDEX`, HIGH): the stated quarterly budget must equal a real SAH classification budget (annual/4 from INDEX-1); $33,540 flagged as not recognised. Plus `RULE_G3_BUDGET_CLASS_MISMATCH` and **`RULE_G3_QTD_OVER_BUDGET`** (HIGH) when QTD usage exceeds the true class budget.
  - **G2 gross reconciliation** (`RULE_G2_GROSS_RECONCILE`, HIGH): sum of line grosses must match the header total; supersedes the softer RULE_15 to keep one-issue-one-flag.
- **Publish gate**: any HIGH gate finding sets `audit_result['publishable']=False` + `publish_block{reason,rules,items}`. `server._render_plain_english_summary` now **hard-blocks** — instead of a summary it returns "We cannot give you a plain-English summary of this statement yet" + the blocker list + "once <provider> corrects these, re-run". Applies to both the public tool and the authenticated dashboard decode (same render fn).
- **INDEX-1**: no change needed for P0 — `program_reference`/`budget.classification_annual` already hold the classification budgets (Class 6 = $49,365.27/4 = $12,341.32). (P2 will add the package-management 5% cap row.)
- **Tests**: new `backend/tests/test_dec1_gates.py` (7 tests, offline, loads INDEX-1 from seed) — all pass. Existing DEC-1 golden/regression suite: 122 passed, 0 failures (fixed the impact-cap ordering so gate impacts are capped too). Live end-to-end run on the real Sam Burke PDF (`backend/scripts/run_sam_burke.py`) confirmed all P0 gates fire and the summary hard-blocks; existing rules (AT-HM miscoding, duplicate 28/08, GST review, stream discrepancy, contribution) also fire.
- **Known extraction gaps (→ P2):** the LLM extraction still drops/merges some rows (25 captured vs 29 source: duplicate 28/08 collapsed, package-management $178 + care-management $215 not surfaced as lines). For now these cause the gross-reconciliation BLOCK (correct P0 behaviour — no misleading summary) rather than a silent wrong summary.
- Scope: BACKEND only (web + mobile both consume the same decode API + summary text, so the gate applies to both surfaces automatically). Next: P1 (transparency flags for Wayly's own recategorisations, one-issue-one-flag unbundling, low-confidence banner), then P2 (extraction robustness + cancellation-policy/travel/AT-HM/category-transition rules).


## Dashboard Insights charts redesigned to be genuinely useful (Jun 2026, fork) ✅ verified (iter316)
- Full rewrite of `DashboardInsights.jsx` — the two charts felt meaningless. Both now have clear titles, headline figures, plain-English legends, month-word+year labels, distinct meaningful colours, and a one-line takeaway.
- **Chart 1 "Where the money goes" (was Monthly Spend):** stacked monthly bars split into **Government funded (teal)** vs **Your co-payment (clay)** — answers "how much am I actually paying". Headline = Total billed; secondary = co-payment share (graceful "fully government funded so far" when none recorded). Footer summarises total billed + co-payment over the period. Bars are tappable → statement.
- **Chart 2 "Statement checks" (was Anomalies Over Time):** stacked per-statement bars relabelled in plain English — **Needs attention (terracotta) / Worth a look (amber) / Just so you know (sage)**. Headline = "N need your attention" (or an "All clear" pill). Footer summarises what needs attention. Tappable → statement.
- **Month-word labels:** new `monthYearLabel` robustly derives "Apr '26" from month-word period labels, numeric DD/MM/YYYY (AU) ranges, OR the ISO upload date — fixed the old raw "01/08/" fragments. Removed the old `shortPeriod`/`disambiguateLabels`.
- Colours are now distinct per chart (teal/clay for money; terracotta/amber/sage for checks) so each visual is instantly readable.
- Scope: WEB only. Web compiles clean. Self-verified: labels render as "Aug '26 / May '26 / Sep '26" (no raw dates); both charts render with figures + summaries. Testing agent verification pending (incl. dark mode legibility).


## Header declutter (one bell) + Statement Decoder tweaks (Jun 2026, fork) ✅ verified (iter315)
- **Header: consolidated the two notification bells into ONE** (`Layout.jsx` + `NotificationsBell.jsx`). The header had two adjacent bell-style icons — `NotificationsBell` (personal) and `LCA1AlertsBell` (aged-care legislative updates) — which read as "two notification bells" and felt cluttered. Removed `LCA1AlertsBell` from the header (and its import) and **merged its aged-care-updates feed into `NotificationsBell`**: one bell, one combined unread count (personal + updates), dropdown now has a "Notifications" section and a clearly separated "Aged care updates" section (with per-alert mark-read/dismiss via the same `/lca1/alerts/*` endpoints). No feature lost — the updates feed previously lived ONLY in the removed bell.
- **Cleaner icon separation**: the header utility group went from `gap-0.5` (cramped) to `gap-1.5`, so the bell + account-health ring each stand out; dividers to the participant switcher + account menu kept.
- **StatementDecoderTool**: intro subtitle widened `max-w-2xl` → `max-w-3xl` so it fits in **two lines** (was three). Verified ~2 lines (58px / 29px line-height).
- **AcceptedFormatsPanel**: the "What formats do we accept?" panel is now **collapsed by default in every mode** — removed `defaultOpen={!file}` on the file-upload usage so it stays collapsed even when no file is attached (users manually expand it).
- Scope: WEB only. Web compiles clean. Self-verified: one `nav-bell` (old `lca1-alerts-bell-btn` gone), dropdown shows both sections; subtitle 2 lines; formats panel collapsed. Testing agent verification pending.


## Dashboard declutter + shaded panels + profile summary cleanup (Jun 2026, fork) ⏳ testing
- **Removed the Recent Activity timeline** from the dashboard: dropped `<DashboardTimelinePanel />` (and its import) from `CaregiverDashboard.jsx` Budget Detail section (component file left in place, now unused).
- **Removed the Lifetime Contribution Cap card** from the dashboard Budget Detail section (it duplicated the Lifetime cap mini-stat tile + the dedicated Contribution Position page).
- **Shaded backgrounds** (was plain white `bg-surface`): the two `DashboardInsights` chart cards (Monthly Spend + Anomalies Over Time) → `bg-surface-2`; the dashboard Pathways card → soft `bg-sage/[0.06]` (matches its sage border).
- **ParticipantProfile artefact summary cleanup** (testing agent iter313 minor finding): strip a trailing raw status token from the artefact `summary_line` (e.g. "Invoice from …, check_before_paying" → "Invoice from …") since the humanised status pill already shows it.
- Scope: WEB only. Web compiles clean. Self-verified: timeline panel + lifetime-cap card absent from the DOM, insight charts render shaded. Testing agent verification pending.


## Dashboard donut redesign + ParticipantProfile clarity edits (Jun 2026, fork) ⏳ testing
- **Dashboard At-a-Glance redesigned around a budget DONUT** (`CaregiverDashboard.jsx`, new `BudgetDonut`): replaced the flat linear progress bar with an easy-to-read ring showing "% USED" in the centre, tone shifting sage → clay → red (#C0392B) as it fills/goes over. Beside it: the status label (red when over), the big "left/over" amount (red when over), "of $X this quarter", a single legend line "● $X spent so far" (dot matches donut), and the care-management note. Removed the now-duplicate "N% of budget used" text (the donut shows it). Kept the uncapped `pctUsedDisplay` so the donut/label read e.g. 150% when over. Mini-stat tiles (To review / Statements / Lifetime cap) unchanged.
- **ParticipantProfile edits** (`ParticipantProfile.jsx`):
  - Header now shows the participant's **full name** (`first_name + last_name`, falling back to display/preferred name) instead of the persona short name — verified "Dorothy Smith".
  - Header subtitle condensed to **one line** ("Everything important about {name}, all in one place.", `truncate`).
  - **"Personal Details"** and **"Quick Actions"** section titles made prominent (`text-sm font-bold uppercase text-primary-k`, was tiny muted).
  - Artefact "Latest Activity" cards: **removed the confusing created-date row** (clock + raw date — didn't help users) and the raw snake_case status now renders **humanised** ("check_before_paying" → "Check before paying"; verified "No issues") as a clean pill.
- Scope: WEB only. Web compiles clean. Self-verified via screenshots (donut renders 70% + $5,296 spent; profile shows full name, one-line subtitle, bold titles, humanised status, no dates). Testing agent verification pending (incl. over-budget red donut state).


## Dashboard At-a-Glance clarity pass — red over-budget, remove duplicate pill, bigger/clearer text (Jun 2026, fork) ✅ verified (iter312)
- **Over-budget now shows in RED** (`CaregiverDashboard.jsx` `AtAGlance`): when `left < 0`, both the status label ("Over budget this quarter") and the big amount render in `text-terracotta` (#C0392B); otherwise `text-primary-k`. Strong, unmistakable over-budget signal.
- **Removed the duplicate status pill**: the separate `On track / Getting close / Over budget` rounded pill (added last iter) duplicated the headline label, so it's gone. `data-testid="glance-budget-status"` now lives on the single status label span; the unused `status` object was deleted.
- **More prominent, clearer headline** (the whole At-a-Glance top block): big amount bumped to `text-5xl sm:text-6xl`; "of $X this quarter" is now `text-base sm:text-lg text-primary-k/80 font-medium` (was tiny muted); the **"Spent $X · N% of budget used"** row is now `text-sm sm:text-base font-semibold text-primary-k` (was `text-xs text-muted-k`); care-management note `text-sm text-primary-k/70`. Progress bar thickened to `h-3`.
- Scope: WEB only. Web compiles clean. On-track state self-verified (pill gone, one status label, Spent/% row prominent). Over-budget RED branch to be verified by testing agent (needs an over-budget budget payload).


## Softer section colours (PageIntro system-wide) + Family Wall legibility & time format (Jun 2026, fork) ✅ verified (web, iter311)
- **System-wide: softened the `.sect-teal/.sect-clay/.sect-sage` section blocks** (`index.css`) used by `PageIntro` (What This Does / How To Use It / What You Get) and every screen that reuses them. Were solid bright brand fills (#0E4D52 / #A5512B / #425F47) with forced WHITE text — felt too bright/dominating. Now a gentle low-opacity wash (~8-10%) of each hue with a solid 5px accent bar and normal DARK text in light mode. The forced-white-text override is now scoped to `html.theme-dark` only (dark mode keeps its dark wash + light text). Applies everywhere automatically.
- **Family Wall composer** (`wall-composer`, `pages/extended/FamilyWall.jsx`): used `sect-teal`, so it inherits the new soft wash — the previously hard-to-read button row (Photo / Voice note / Share) now sits on a light background with dark, legible text.
- **Family Wall feed posts** (`wall-post-*`): were bright solid brand fills cycling teal/clay/sage/plum/red with `force-white`. Now a subtle tint of the same hue + a 5px coloured left accent bar + border + shadow; removed `force-white`; avatar chip `bg-primary-k/10`, author name `text-primary-k`, timestamp `text-primary-k/60`, delete button dark-on-hover. Body already dark.
- **Post time format** (`formatPostTime`): replaced `new Date(...).toLocaleString()` (seconds + lowercase am/pm) with `DD/MM/YYYY h:mm` + capitalised **AM/PM**, no seconds. Verified live: "15/08/2026 1:57 PM".
- Scope: WEB only. Web compiles clean. Verified via screenshots (PageIntro soft cards; Wall posts shaded + readable + correct time). Testing agent verification pending.


## ParticipantProfile agentic edits — remove header badge, subtle card tints, nicer timeline arrow, timeline toggle (Jun 2026, fork) ✅ verified (web)
- **Removed the header Classification badge** (`ParticipantProfile.jsx`): the `core1-classification-badge` pill in the profile header was redundant (classification is already listed in the sidebar "Personal Details"). Deleted the header usage + the now-unused `ClassificationBadge` component. Verified: badge absent from header, sidebar still shows "Classification · Level N".
- **Subtle per-card background tint** on each "Latest Activity" artefact card (`ArtefactCard`, empty + filled): each card now carries a faint wash of its own accent colour (`backgroundColor: ${accent}0F` ≈ 6%) — sage / clay / teal / navy — so the grid reads as distinct tiles without being loud. Kept the accent left-border on filled cards.
- **Nicer timeline open/close arrow** (`core1-timeline-section` summary): the bare ChevronDown became a circular affordance (`h-8 w-8 rounded-full bg-primary-k/10`, hover brighten, `group-open:rotate-180`) so it clearly reads as a toggle in light + dark.
- **Timeline milestones button now toggles both ways** (`TimelineList`, `core1-timeline-expand`): previously it only expanded (`setOpen(true)`) with no way to re-collapse. Refactored to a single toggle (`setOpen(v => !v)`, `aria-expanded`) that flips "Show N key milestones" ⇄ "Hide N key milestones" with a rotating chevron; list extracted to `listEl` and reused for the `showAll` path. Verified live: Show→Hide flip + list visibility confirmed.
- Scope: WEB only (matched the line-referenced edits). Web compiles clean.


## Dashboard agentic edits — budget status prominence + mini-stat routing/values + collapsible Wayly Chat (Jun 2026, fork) ✅ verified (web)
- **Budget status now prominent** (`CaregiverDashboard.jsx` `AtAGlance`): the "Left to spend / Over budget this quarter" eyebrow was tiny muted text — now a bold uppercase primary label PLUS a colour-coded status pill (`glance-budget-status`): On track (sage <75%), Getting close (gold 75-89%), Almost used up (terracotta ≥90%), Over budget (terracotta). Verified live: pill reads "ON TRACK" for Dorothy (70% used).
- **Mini-stat tiles corrected** (the 3-up grid):
  - **To review** now counts anomalies on the LATEST statement only (`(latest?.anomalies||[]).length`) with sub-label "On your latest statement", and links to that statement's detail page (`/app/statements/{latest.id}`). It naturally resets when a newer statement is decoded (statements[0] changes). Was: all-time anomaly count → `/app/budget-alerts`. (The separate "Things To Know" accordion still aggregates all statements.)
  - **Lifetime cap** now shows the DOLLAR figure (`formatAUD(lifetime_contributions)`, e.g. $6,887) with "5% of $137,917" underneath, and links to the participant's lifetime-cap profile (`/app/participants/{id}/contribution-position`, falls back to `/app/contribution-position`). Was: percentage only → `/app/reports`.
  - Statements tile unchanged (→ `/app/statements`).
- **Wayly Chat card** (`chat-preview-card`): renamed header "AI chat, last conversation" → "Wayly Chat, last conversation"; converted from an always-open card to a `<details>` collapsed BY DEFAULT with a clear rotating ChevronDown toggle (`chat-preview-toggle`, `group-open:rotate-180`), theme-aware `text-primary-k` so the arrow reads in light + dark. "Open chat" link `stopPropagation`s so it navigates without toggling.
- Scope: WEB only (matches the line-referenced edits). Verified via login as cathy@example.com: DOM probes confirmed pill="ON TRACK", cap tile="$6,887 / 5% of $137,917" → participant contribution-position href, review tile="3 / On your latest statement" → latest statement href, chat `open=false`. Web compiles clean.


## Dashboard/BC2 quarterly budget → official gross figure + summary cache flush (Jun 2026, fork) ✅ verified
- **Root of "wrong figure for Sam (Cl 6)":** two things. (1) The dashboard headline "of $X this quarter" was wired to `quarterly_usable` (annual×0.9/4 = $11,107.19 for Cl 6), not the official quarterly budget ($12,341.32). (2) The **Wayly Summary narrative** is an LLM result cached in `smart_ai_summaries`, so the old $10,825.65 (pre-figure-fix) stayed baked in until regeneration.
- **Fix (user chose option a — track all spend against the full budget):** CaregiverDashboard `AtAGlance` now uses `quarterly_gross` (official) for the headline, left-to-spend and % used, with a sub-line "$X of this is care management (10%)". Same for the mobile dashboard summary context (`app/(tabs)/index.tsx`). `routes/bc2.py` runway now uses the gross quarterly budget (`quarterly_budget(cls)/0.9`) so the Budget Runway matches. Service stream cards intentionally still sum to the usable amount (care management is the difference, shown in the sub-line).
- **Summary self-heal:** cleared all 230 cached `smart_ai_summaries` so narratives regenerate with live figures; the dashboard already passes the budget numbers in the summary context (`quarterly_budget_aud` + `quarterly_care_management_aud`), so future figure changes bust the cache automatically.
- Verified: `/budget/current` Cl 4 gross $7,617.13 / usable $6,855.41 / care mgmt $761.72; dashboard renders "of $7,617.13 this quarter" + "$761.72 of this is care management". Cl 6 → $12,341.32 + $1,234.13.
- **Backend transient hang recurred** during verification (`/api/health` timed out despite supervisor RUNNING); a `restart backend` cleared it. Still a P1 — needs RCA (possible event-loop saturation from lazy summary regeneration after the cache flush).
- **Outstanding (user-deferred / next):** (1) test accounts (sam@test.com) are reachable in production — preview/test and prod must NOT share a database; user asked to fix this next. (2) Short-Term Pathways tool (Restorative/EOL) + reviewer short-term budget checks. (3) Cross-platform testing-agent parity run.


## Support Plan Reviewer regression fixes + Budget Calculator + program-ref cache (Jun 2026, fork) ✅ verified
- **"Name" bug:** `_first_name_from` (care_plan_ingestion) captured the label word "Name" from a "Participant Name: Sam Burke" row. Rewritten with label-word guard + multi-pattern (Participant Name / Name: / Client / For). Verified against the real Sam Burke PDF → "Sam".
- **Budget coherence (the $33,540 vs $12,341.32 vs $12,028.50 problem):** (a) `build_facts` now uses the PLAN's stated quarterly (extraction) first so check #5 compares the plan's figure vs the official classification budget instead of official-vs-official (tautology); (b) `run_verification_panel` returns `classification_quarterly_budget` (official) and the overview chip now shows that official figure; (c) check #5 wording is plain-English with comma formatting; (d) the $12,028.50 was a stale in-memory INDEX-1 (monetary_constants) registry — a backend restart loads the regenerated $49,365.27. Verified: check #5 now flags "plan states $33,540.00 vs official $12,341.32".
- **False all-clear:** CarePlanReviewer.jsx no longer shows "Nothing needs your attention" when the verification panel has flags/cannot_runs; it shows "A few things worth a look" pointing to the safety checks.
- **Budget & Lifetime Cap Calculator:** classification buttons showed OLD figures for real browsers because `lib/programReference.js` cached the snapshot in `localStorage["wayly:program-reference:v1"]` for 1h; bumped the key to **v2** so every browser ignores the stale cache and refetches (verified: seeded a stale v1 cache, page still showed the new $49,365). Also: softened the unreadable `sect-clay` supplements sections to a subtle readable clay tint, and added a clarity caption ("Each figure is the annual … before the 10% care-management share").
- **"Add what we're missing" (ProfileInlinePrompts):** added a 7-day localStorage throttle (once-a-week) so it no longer duplicates the always-present inline supplements picker on the Budget Calculator.
- **Tests:** updated the CPR golden fixture (robert_henderson_q1_2027) to the new Cl 5 figures ($10,182.38 quarterly, $1,018.24 rollover) so it stays a valid all-pass case; golden 8/8 green; rollover/BUD-1 green in isolation.
- **Gap analysis (Support at Home short-term pathways):** Restorative Care and End-of-Life pathways have data in the reference engine but NO dedicated user tool; the Support Plan Reviewer verifies only the ongoing quarterly budget, not the short-term (AT-HM/Restorative/EOL) budgets or the outcome-letter goals/approved-services. Documented as next build candidates.


## Support at Home classification figures → July 2026 indexation + Care Plan Reviewer prefill fix (Jun 2026, fork) ✅ verified
- **Root issue (user report):** the Support/Care Plan Reviewer prefilled the "Quarterly budget" with the wrong amount (Cl 6 showed $10,826). Cause: (a) stale Nov-2025 annual figures, and (b) the prefill used the post-care-management "usable" amount (annual×0.9/4) instead of the official quarterly budget (annual/4, which INCLUDES the 10% care management).
- **Figures updated system-wide to the official 1 July 2026 indexation** (source: DoH "Support at Home program classifications and budgets" PDF + funding-classifications page). New annual/quarterly: Cl1 $11,010.01/$2,752.50 · Cl2 $16,451.35/$4,112.84 · Cl3 $22,536.81/$5,634.20 · Cl4 $30,468.51/$7,617.13 · Cl5 $40,729.53/$10,182.38 · Cl6 $49,365.27/$12,341.32 · Cl7 $59,660.00/$14,915.00 · Cl8 $80,137.12/$20,034.28. Transitioned HCP L1-4 annuals also updated ($11,272.15 / $19,821.76 / $43,148.74 / $65,415.91).
- **Where changed:** `seed_program_reference.py` (new rows effective 2026-07-01, old 2025-11-01 rows closed with effective_to); `program_reference.apply_reseed_for_authoritative_keys` skip-condition fixed so an unchanged value with a new effective_to still gets closed; `routes/csc.py` `/public/csc/bands` now returns the official quarterly budget (annual/4, to the cent); `lib/csc/scoring.py` `_quarterly_from_annual` → annual/4 (gross) + fallback figures; `budget.py` `_FALLBACK_ANNUAL`/`_FALLBACK_TRANSITIONAL_ANNUAL`; `batch2_routes.py` subsidy_by_classification; frontend `lib/programReference.js`, `pages/tools/BudgetCalculatorTool.jsx`, `data/supportAtHomeLevels.js`; mobile `src/data/budgetTool.ts`. Verified via curl: bands Cl6 quarterly = $12,341.32; `/budget/current` Dorothy (Cl4) annual $30,468.51.
- **Care Plan Reviewer (web + mobile):** changing the classification dropdown/pill now re-fills the Quarterly budget from that classification's official amount; the field shows a "$" prefix and AU-grouped 2-decimal formatting (e.g. $12,341.32); all submit paths strip the commas before sending. Web verified via automation (Cl6→$12,341.32, Cl1→$2,752.50). Mobile bundle compiles; not e2e-navigated.
- **Tests:** updated `test_rollover_cap.py` (level 6/7/8 now date-robust + new annual) and `test_bud1_v1.py` (T2/T3 ranges for new figures); `test_monetary_constants.py` back to green (regenerated `data/monetary_constants.yaml` from the new seed; key count unchanged at 237). PRE-EXISTING failures NOT caused by this change and left as-is: `test_csc_scoring::test_T04_no_em_dashes_in_frontend_copy` (em-dash in untouched `ClassificationCheck.jsx`), and the BC2/contribution-estimator HTTP-fixture setup ERRORs (stale post-fork base URL → LocationParseError in the `cathy` fixture).


## Agentic edits — SmartAISummary dark card + StatementRightsPanel polish (Jun 2026, fork) ✅ verified (screenshot)
- **SmartAISummary** ("Wayly Summary" card, used across data-heavy pages incl. CaregiverDashboard): user asked for the body text (line 111) + alert text (line 140) to be light/white in light mode. Making text white on the old cream/teal wash would be invisible, so the whole card was converted to a **dark-teal card** (`bg-[linear-gradient(135deg,#0E4D52,#0B3B3E)]`, gold left border). Title, eyebrow, body, refresh button and alert tiles all restyled for the dark surface (white text; alert icon accents brightened; alert tiles now white/7% translucent with white/15 borders). Verified via screenshot on the caregiver dashboard — fully legible.
- **StatementRightsPanel** (`components/statements/StatementRightsPanel.jsx`): "Your rights" → "Your Rights" (title case); subtitle bumped from `text-xs text-muted-k` → `text-sm font-medium text-primary-k/75` (more prominent, per "call me out more, too small"); annotation title now `truncate` with `min-w-0` parent so it stays on one line.
- **Still pending user decision** (asked but not answered): whether the same white-text-in-light-mode treatment should extend to DashboardActionBar, DualFlagshipHero, ToolClusterGrid, and the CaregiverDashboard bold/collapse-chat tweaks. Also outstanding: clickable graph data points, dark-mode marketing pass, mobile Recent Activity, shared mobile chip, full parity check.


## Responsive fix — Statements & Invoices tables on phone/tablet (Jun 2026, iter 306–307) ✅ verified

- **User report:** web app looked "broken / completely different" on phone (cited a "long orange Things to Know" block on the statement page).
- **Audit (iter 306) finding:** the app is actually well-responsive — `scrollWidth == clientWidth` on all 11 screens × 3 viewports (390/768/1440), zero page-level horizontal overflow. The statement-page orange section is fully contained (the "long" feel is vertical stacking of ~10 cards). The ONE real defect: Statements (`/app/statements`) and Invoices (`/app/invoices`) tables used a fixed `grid min-w-[720px]` → forced sideways scrolling inside the card on phones/tablets.
- **Fix:** both tables now render a **stacked card layout at `<md`** (period/provider/status + wrapped meta line for statements; provider/verdict/amount/date/findings for invoices) and keep the aligned **6-column grid at `md+`**. Also fixed a stray `,` shown for a null closing balance → now "—". Files: `StatementsList.jsx`, `InvoicesList.jsx`.
- **Verified (iter 307):** phone 390 + tablet 768 = no inner/page horizontal scroll, rows still clickable; desktop 1440 grid intact. PASS.
- Left as-is: CHSP comparison tables (legitimate side-by-side data; horizontal scroll is acceptable there and not flagged by the audit).


## CORS fix + dashboard redesign/softening + budget fixture + draft-preview chips (Jun 2026, fork iter 305) ✅ verified

### P0 — Expo web-preview CORS blocker (FIXED, backend)
- Root cause: the Emergent preview edge (Cloudflare worker) rewrites the inbound `Origin` header to the INTERNAL cluster host `https://<app>.cluster-N.preview.emergentcf.cloud` before it reaches the backend, so Starlette echoed that host in `Access-Control-Allow-Origin` and the browser rejected it (it validates ACAO against the REAL origin `...expo.preview.emergentagent.com`).
- Fix (`backend/server.py`): (1) widened the CORS regex to allow `emergentagent.com` + `emergentcf.cloud`; (2) added an OUTERMOST middleware `_fix_preview_cors_origin` that rewrites ACAO to the real browser origin — from `Referer` on actual requests, or reconstructed from `x-forwarded-host` (insert `.expo.`) on credential-less preflights. Only fires when the echoed origin is the internal `*.emergentcf.cloud` host, so production/same-origin are untouched. Mobile Expo web-preview login now works end-to-end.

### Capitalisation (web + mobile)
- Account menu / command palette / marketing header / Settings → "Plan & Billing" (was "Plan & billing"/"Plan and Billing"). AI Tools CTA → "Open Tool" (web + mobile). App back-links Title-Cased: "Back to Profile/Statement/Dashboard/Invoices/Articles/Switches". Backend email copy already used "Plan & Billing".

### Dashboard redesign + softening (web `CaregiverDashboard.jsx`)
- `BC2Projection.jsx` → "Budget Runway" card: plain-English, colour-blocked (this-quarter hero with teal accent, soft-clay lifetime cap, 3-quarter forecast tiles). All `bc2-*` testids preserved.
- New mobile `BudgetRunway.tsx` mounted in the dashboard detail section (parity; light header + soft-clay cap).
- `DashboardTimelinePanel.jsx` → unified `/core` timeline, plain-English connected timeline (coloured dots), no technical rows.
- `AtAGlance` hero softened to a light wash; 3 mini-stat boxes given distinct soft colours + left-accent stripes (gold / teal / clay), shadows, bigger numbers.
- "Budget Detail" bar: teal left-accent + prominent teal icon. "Things To Know" bar: warm gradient + terracotta accent + red count badge (`things-to-know-count`).
- Removed the Audit Log card from the dashboard (chat card now full width).
- `SmartAISummary` (shared): user asked for white text then to revert — FINAL state = soft light card (teal wash, dark text). DashboardActionBar FINAL state = original cream/gold light card (the teal white-text versions were reverted per user request).

### Budget fixture (DB)
- Seeded participant Dorothy Smith (cathy@example.com household): `qp1_schedules.quarterly_budget=6681.6` + latest statement `summary.total=4142.59` → participant money gauge now shows a live 62% instead of empty.

### Draft-preview chip (web + mobile)
- Tapping a cross-tool chip on a letter draft now shows an inline "Added: <label>" confirmation tag (e.g. "Added: price check $90"). Web `LetterGeneration.jsx` (CrossToolImportPanel), mobile `letters/[id].tsx` and `correspondence/[id].tsx`.

### Testing (iter 305)
- Backend + web + mobile all pass. One parity bug found (mobile correspondence tag not rendered) → fixed with the verified letters/[id] pattern.

### Notes / pending
- "Quarterly Pacing" tile is already removed from code (both dashboards); production (wayly.com.au) shows it only because it needs a re-publish.
- Not yet done: dark-mode marketing pass (Landing + Pricing).


## Dashboard / header / summary polish batch (Jun 2026) ✅ verified (iter 302)
- **Things To Know** (`CaregiverDashboard.jsx`): capped the visible list at 10 with a "View all N things to know" toggle (expands to full list / "Show fewer"). Redesigned each item — severity badge (Alert/Heads up), title, summary, "What to do" callout, "Why we flagged this" expander, and a "View statement" footer link — for clearer structure.
- **"Show Detail" / "Hide Detail"**: Budget Detail toggle relabelled (title case).
- **Global header** (`Layout.jsx`): grouped the utility icons (search/alerts/notifications/account-health) with a subtle divider before the participant/account cluster; logout now a clear "Sign out". Tidier, verified in light + dark, no page regressions.
- **SmartAISummary**: panel gradient lightened one step (`#0E4D52→#135A61`, `#10363A→#123F44`), same teal family.
...
- **New backend** `POST /api/lf1/correspondence/{id}/prefill` (`routes/lf1.py`) + `lib.lf1.build_starter_intake()` / `situation_auto_draftable()`. Picking a situation now seeds the intake fields with a first-person starter (per-situation templates, `{name}` substituted) **and** auto-generates a starter draft via the existing Claude generator. Intake is persisted up-front so fields fill even if the LLM is slow; idempotent (won't overwrite an existing draft) and never clobbers user-entered fields. Safeguarding (11) + response-draft (12) seed fields only, no auto letter.
- **Web** `CorrespondenceDetail.jsx`: auto-fires prefill on arrival for direct-situation entries (not tool-carried), passes the active participant name, shows a "Wayly has started this letter for you…" starter banner + a drafting busy note. Verified end-to-end (curl + screenshot): fields seeded, draft generated (~6s).
- **Still pending:** mobile letters prefill parity.

### ClassificationCheck (web) — agentic edits
- Confidence pill now has an Info icon + hover tooltip explaining High/Medium/Low. Result header redesigned: 1→8 band-scale graphic (gold = your band) + colour-accented Per year/quarter/week budget cells + who-pays supporting line. "You're likely here" badge → white text.

### PriceCheckerTool (web + mobile) — legibility + button colours
- **Web:** fixed light-mode white-on-white (the `sect-teal` "How this compares" panel forced white text over a `bg-white/70` box). Converted to a soft-teal surface with normal dark text; compare box → solid surface. Save/PDF/Email buttons now distinct solids (teal / clay / sage). **Mobile:** same three buttons given distinct solid colours.

### ParticipantProfile + ParticipantTimeline (web) redesign ✅ verified
- **Profile:** teal Financial data-viz hero (gauge + compare bars), clean clickable artefact/case/pattern cards (removed bright `panel-solid`/`item-*` fills), proper vertical timeline; then re-added subtle brand-tinted section backgrounds (~5–6% teal/clay/sage) so it isn't all white.
- **Timeline page** (`extended/ParticipantTimeline.jsx`): now a connected spine with coloured category dots + compact tinted cards; filters out technical `state` events; removed the technical "status transition" detail.
- **Settings:** free-trial row → gold banner with black text (kept gold as trial ends); Cancel button gold/black while trialing.

### Mobile parity ✅ verified where noted
- **SmartAISummary** → solid teal panel + white text + gold left border (✅ screenshot).
- **Statement + Invoice SVG donut graphics** (`src/components/InsightGraphics.tsx`, react-native-svg) — "Where the money went / your money stands" + "What we found" cards; wired into DecoderResultView + invoice detail (✅ present on statement).
- **Invoice WaylySummaryCard** → solid teal.
- **Timeline** → coloured dots + tinted cards + `cleanSummary()` to strip raw dict dumps (✅ screenshot).

### Still pending (mobile parity backlog)
- Mobile participant profile financial gauge/data-viz + collapsible timeline; mobile Settings trial gold row + days-left visibility; mobile letters prefill; coloured tables + whitespace audit on mobile statements list + tool screens. Full cross-platform testing-agent pass not yet run for this batch.



## Profile colour reduced (web) + Support Plan Reviewer mirrored to mobile (Jun 2026)

- **Web ParticipantProfile:** dialled back the colour — converted the Financial, Open Follow-ups, Household, Personal Details, Voice Check and Complaints panels from solid `panel-solid-*` back to calm light `sect-*` tints (removed `force-white`), keeping only the collapsed Timeline as the single solid accent. Much less overwhelming.
- **Mobile Support Plan Reviewer** (`/app/mobile/src/components/tools/CarePlanReviewer.tsx`): mirrored the web redesign — reordered to Overview → Findings → My Care Goals → Safety checks → Ask; plan overview stat chips; findings now URGENT-FIRST with a tally chip row and a collapsed accordion (tap to expand detail / what-to-ask / citation / draft-letter); "Add to My Care Goals" flow (POST `/cpr2/participants/{pid}/goals`, suggests from services when the plan has none); new "Ask about this plan" box (POST `/care-plans/ask`). Uses the shared backend goals extraction + ask endpoint. Mobile bundle compiles cleanly; full output not yet screenshot-verified end-to-end (needs in-app nav + LLM run).

### Mobile mirroring still queued (user-approved order)
2. Account Health Score (mobile). 3. Draft-letter prefill (mobile correspondence). 4. Contribution Estimator (mobile). 5. Provider Price Checker (mobile).


## Account Health Score moved to top-right header (Jun 2026)

- New `components/AccountHealthMenu.jsx`: compact **% progress ring** in the app header (top-right, next to the bells) with an outstanding-count badge. Click opens a dropdown listing each outstanding item with a direct link to its fix (`fix_route`); done items show struck-through. Uses the existing `/account/health` endpoint (email verification reflects the real `email_verified` flag; pension "unsure" surfaces a "Confirm pension status" item).
- Removed the large `AccountHealthCard` from the Caregiver Dashboard (import + render) so the score lives only in the header.
- Verified: ring shows %, badge shows count, dropdown lists items + links, dashboard no longer shows the card.


## Support Plan Reviewer — findings section cleaner/collapsed (Jun 2026)

- "What we found" redesigned into a **collapsed-by-default accordion**: each finding is a compact one-line card with a coloured left-accent + icon circle (red=needs attention, clay=worth checking, teal=good to know), severity label, title and chevron. Click expands to reveal the detail, a tinted "What to ask your provider" box, and the plan citation.
- Added a **visual tally chip row** ("N need attention · N worth checking · N good to know") for at-a-glance scanning instead of a wall of paragraphs.
- Verified collapsed + expanded states via screenshots.


## Support Plan Reviewer full redesign (Jun 2026)

Web `/ai-tools/care-plan-reviewer` results screen rebuilt + backend additions. Verified end-to-end (screenshots + curl).
- **Reordered & colourful, wider (max-w-6xl):** Plan overview → Findings → My Care Goals → Safety checks → Ask. (Verification checks now come AFTER findings, as requested.)
- **Plan overview:** solid teal panel, white text, stat chips (provider, level, quarterly budget, #services) + a slightly fuller plain-English summary. Trimmed the duplicate header out of the "what we read" preview.
- **Findings:** sorted URGENT-FIRST (compliance → warning → info), each a colour-coded solid card (terracotta / clay / teal) with white text, an icon bubble, a "What to ask your provider" sub-panel, and the plan citation. Empty state is a friendly sage panel. "Draft a letter about these" still wired (uses the fixed prefill bridge).
- **Safety checks:** redesigned as a 2-col grid of coloured tiles (sage pass / clay flag / neutral cannot-run).
- **My Care Goals flow:** backend now extracts plan goals deterministically (`_extract_goals` in services/care_plan_ingestion.py, new `goals` field on StructuredExtraction — depends on a recognisable GOALS heading). UI lists found goals with "Add to My Care Goals" (POST `/cpr2/participants/{pid}/goals`); if none found, suggests goals from the services list. Added goals show on the Participant Profile.
- **Ask a question:** new gated `POST /api/care-plans/ask` — plain-English, Year-6 reading level, answers ONLY from the review context; verified returns clear answers. Clay panel with input on the results screen.
- All text plain-English; findings/goals/verification carry no duplicated info.


## Draft-letter prefill fix, signup copy, Voice Check rename (Jun 2026)

- **Signup:** removed inaccurate "No card needed." line under the Google option.
- **Draft-a-letter prefill FIXED (the "just goes to correspondence page" bug):** tool→letter handoffs create an LF-1 entry carrying the issues in `source_import`, but `CorrespondenceDetail` never surfaced them and required a manual Generate click. Now: (1) a clay panel lists the exact issues carried over from the source tool (Invoice Checker, Support Plan Reviewer, etc.), and (2) the letter AUTO-DRAFTS on arrival from those issues via `/lf1/correspondence/{id}/generate` (safe now the LLM guard is thread-offloaded). Generic across single + bulk `source_import` shapes.
- **Rename "Voice Check" → "My Care Goals"** (user pick, option d) across web user-facing labels: VoiceCheck.jsx (header/eyebrow/buttons), ParticipantProfile ("Open My Care Goals", section title), Features.jsx card, CorrespondenceDetail prefill label. testIDs kept stable. No mobile strings existed.

### Still outstanding (confirmed order with user)
1. Support Plan Reviewer full redesign + goals→"My Care Goals" add flow + ask-a-question + visual plain-English results (backend goals API `/cpr2/participants/{pid}/goals` already exists).
2. Account Health Score % circle (top-right) with outstanding-items drill-down (email verification uses the real verified flag).
3. Easy-English pass across all tools (kill jargon like "Lifetime cap indicative check, cap value pending").
4. Landing + Features redesign with visual graphics (web + mobile).
5. Guided Journeys colour + graphics + light animation.
(Tool Output Charts: user said NO — skip.)


## Tool-output legibility fix + backend freeze diagnosis (Jun 2026)

- **Reverted the blended-panel regression:** my earlier blanket `.sect-*` → `.panel-solid-*` conversion on tool outputs made text blend/unreadable. Reverted PriceCheckerTool, BudgetCalculatorTool, CarePlanReviewer back to `.sect-*`; converted ContributionEstimator result panels to clean `.bg-surface` light cards (safety-net, October 2026, what-if, also-worth-knowing, how-calculated) and reverted their icons to tinted colours.
- **Verified (screenshots):** ContributionEstimator full result is now clean + legible — bold colour reserved for the hero + "who pays" donut; all detail sections are readable light cards. iter292 (testing agent) confirmed Provider Price Checker + Budget Calculator outputs legible too.
- **CRITICAL backend finding + FIX:** the backend runs a SINGLE uvicorn worker. `lib/llm_guard.py` wrapped every direct `LlmChat.send_message` with a `wait_for` timeout but kept it ON the request event loop; a blocking vendored call therefore froze the whole loop (the timer could never fire) and, because bcrypt/login share the loop's default executor, login + all requests hung — backend logged nothing for ~24 min and returned 502s. FIXED: `llm_guard` now offloads each `send_message` to a dedicated `ThreadPoolExecutor` (mirroring `lib/llm_wrapper`) so a hung/slow call can never freeze the loop and the 90s timeout always enforces. VERIFIED: with 3 concurrent care-plan LLM jobs running, login stayed ~0.9s and the loop kept serving requests (previously this exact scenario froze it). Restart also clears any prior zombie.
- **Still to visually verify (were blocked by the freeze):** Statement Decoder, Invoice Checker, Care Plan Reviewer, Family Coordinator, Classification Self-Check outputs.


## White-text full sweep + Settings edits (Jun 2026)

- **Global sweep:** added CSS so every `.panel-solid-teal / .panel-solid-clay / .panel-solid-sage` panel auto-whitens its descendant text (headings, body, links, buttons, labels, dt/dd, muted text → white / 85% white). Nested elements carrying their own `bg-*` utility (chips/badges) are excluded so white-on-colour pills keep their colours. This means any teal/clay/sage surface reads white app-wide without per-element `force-white`.
- **Settings edits:** trial-remaining pill changed from faint `bg-gold/15` (blended on teal) → solid white pill with teal text; "Cancel plan" button changed from terracotta → white text on the teal current-plan card. Verified on /settings/billing.


## Frontend accessibility + theme fixes (Jun 2026)

- **Accessibility widget was invisible** — root cause: `ScrollHideWidgets` applied a `transform` (translateY) which creates a CSS containing block, so the `position: fixed` a11y launcher + Help chat were pinned to the page bottom instead of the viewport. Removed the transform (kept the opacity fade). Both floating widgets now sit correctly bottom-left / bottom-right on every non-admin route. The widget already contains a Dark mode toggle, so users switch light↔dark entirely from the frontend.
- **Default light theme:** `uxf/theme.jsx` effective theme is now `override || "light"` (ignores OS dark preference) so any new/logged-out session starts light. Explicit user choice still persists.
- **Reset on logout:** `AuthContext.logout` now clears `wayly:app:appearance` / legacy keys, sets `wayly_a11y_v1.dark=false`, removes `.theme-dark` and sets `data-theme=light`.
- **AI Tools "Open tool" invisible in dark mode:** the global `html.theme-dark a:not(...)` rule forced links to teal. Added `force-white` to the AIToolsIndex tiles so all tile text/links stay white (`!important`) on the solid dark tiles. Verified in dark mode.
- **Audit:** `/app/memory/UI_UX_AUDIT.md` created — full Done/Partial/Outstanding matrix of the original overhaul request across web + mobile.


## About page visual rewrite + new mobile About screen (Jun 2026)

- **Web `/app/frontend/src/pages/About.jsx` fully redesigned:** long prose cut down; every section is now a bold brand-coloured panel (teal/clay/sage/plum/terracotta) with WHITE text, icons, stat tiles, numbered lesson cards, belief/cluster cards and pull-quotes. All original data-testids preserved (about-h1, about-read-on, about-section-*, about-cta-decode, about-cta-plans, about-cluster-card). Screenshot-verified light mode.
- **New mobile About screen `/app/mobile/app/about.tsx`:** parity with web — coloured Panel/Scene/StatTile/Lesson/Belief/Cluster/PullQuote primitives, white text throughout, Decode a Statement → /(tabs)/ai-tools and See Our Plans → /plan-select CTAs. Entry point added in mobile Settings → Privacy & legal ("About Wayly", testID settings-nav-about → router.push("/about")). Screenshot-verified full scroll on Expo preview.
- Self-tested via screenshots (web + Expo); testing agent not run for these visual changes.


## UI colour-pass agentic edits (Jun 2026) — Settings + ParticipantProfile solid brand panels

Applied and screenshot-verified (web, light mode):
- **Settings › Plan & Billing:** "What you are paying for" card switched clay → `panel-solid-teal force-white` (full teal, white text); participant rows now a distinct deeper teal (`#0A3B3F`); "Primary" chip is white-on-teal for contrast.
- **ParticipantProfile:** Financial Position, Open Follow-Ups (empty + populated), Household Members, sidebar Personal Details, Voice Check and Complaints sections converted from light `sect-*` tints → `panel-solid-clay/teal force-white` (solid brand bg + white text in light mode). Timeline "Show N milestones" button changed `bg-white/12 hover:bg-white/20` → `bg-black/20 hover:bg-black/30` so it stays readable and darkens on hover instead of blending to white.
- Web-only; no mobile or backend changes.
...
- **Backend (`server.py`)**: new `_compute_access_state(user, sub)` (admin bypass → then Stripe status: active→active, trialing→trial, past_due/unpaid/canceled/expired/incomplete→view_only, else legacy plan+trial fallback) with a `_trial_end_dt` guard that drops a STALE `trialing` status whose window has elapsed to view_only. The global `_enforce_read_only_for_unpaid` middleware and `_require_paid_plan` now consume it (admins never blocked; past_due/canceled — where `plan` still reads "family" — are now correctly blocked, which the old plan-only check missed). Exempt prefixes keep `/api/payments/`, `/api/billing/`, `/api/auth/`, `/api/support/` writable so view-only users can always reactivate.
- **Backend (`routes/payments.py`)**: `cancel_subscription` now branches — cancel DURING trial = immediate Stripe cancel → view_only now (nothing paid to honour); cancel a PAID sub = `cancel_at_period_end` (full access until period end, then the `customer.subscription.deleted` webhook flips it). Webhooks already mirror status (invoice.payment_failed→past_due, subscription.deleted→canceled, invoice.paid→active) so the DB updates immediately, no polling. Trial-end AUTO-CHARGE is native Stripe (Checkout subscription mode + trial + saved default card) — no extra code.
- **Web**: repointed the single chokepoints — `useExpiredTrial()` + `AuthContext._syncReadOnly()` now read `user.access_state === "view_only"` (previously matched a literal `"expired"` the backend never wrote, so cancel/payment-failure never enforced). This instantly drives the global `ReadOnlyBanner`, ~20 pages' `ReadOnlyLock` composers, and the `api.js` client-side write guard. Added `/payments/` to the read-only allow-list; refreshed banner/lock copy to "Your plan is inactive. Reactivate…".
- **Mobile (was NONE)**: added `access_state`/`viewOnly` to `AuthContext`, a `setReadOnlyMode` write-guard in `src/lib/api.ts` (blocks POST/PUT/PATCH/DELETE + SSE tool-runs with a friendly 402 unless billing/auth/support), a global `ViewOnlyBanner` (testID `view-only-banner` + `-reactivate` → `/plan-billing`) mounted above the Stack in `app/_layout.tsx`.
- **Verified iter256 (testing_agent, both surfaces + backend)**: /auth/me access_state correct for all 4 fixtures incl. stale-trial guard; view_only writes → 402 with the correct read_only payload; `/payments/checkout` NOT blocked for view_only; active+trial unaffected; web ReadOnlyBanner + billing reactivate; mobile ViewOnlyBanner + Reactivate → plan-billing; admin bypass static-verified. Live Stripe charge/webhook loop needs a real subscription via external hosted checkout — not automatable in preview; contracts + gating exercised instead. Fixtures in `test_credentials.md`.

DECISION (documented for review): paid-plan cancel keeps access until period end (honours "never view-only while a paid subscription is current"); only trial-cancel is immediate. Change if you want paid-cancel to be immediate too.



## Iteration 252-255 (Jun 2026) — Mobile Money & Statements parity + link/forms audit (Items 1-4 done & tested)

Completed the four-item parity sequence in strict order, each verified by the testing agent before proceeding.

- **Item 1 — Mobile Pacing rebuild (iter252 PASS):** `/app/mobile/app/pacing.tsx` rebuilt to full web parity with 4 tabs (Pacing, This week, Schedules, History) on the shared `/api/qp1/*` endpoints: pace status card + confidence + underspend flag + "How is this calculated?" breakdown; ledger Confirm/Missed/Changed actions; Log a one-off (ad_hoc); Reconcile from a decoded statement + paste-CSV reconcile; schedule create/list/end; four-quarter history. Exact web labels, single `$`/`%`, no dashes.
- **Item 2 — Mobile Budget Scenarios rebuild (iter253 PASS):** `/app/mobile/app/budget-scenarios.tsx` moved to the web `/api/bc2` scenario system: baseline projection, debounced projection-preview on adjustment, touch-friendly classification chip row + spending-pace/indexation steppers (values/labels match web sliders), save named scenarios, tick-two compare. Backend `/bc2` round-trip validated 5/5.
- **Item 3 — Mobile Report detail view (iter254 PASS):** new route `/app/mobile/app/report/[id].tsx` fetches `GET /api/reports/{id}/data` and renders per-type layouts (summary, stat grids, tables) for all 8 report types + Download PDF; `reports.tsx` gained a `View` button. Fixed a bogus-id loading edge case.
- **Item 4 — Broken-link crawl + forms audit (iter255 PASS):** fixed 6 dead web links (SwitchSettlement Open Cases, Carer Self-Check support directory, Provider Quality methodology, Participant Timeline Add Event, Ask Wayly v2 disclosure, Invoice Checker Open Vault) to real routes; full crawl found zero remaining dead targets across 145 web + 60+ mobile nav targets; key web+mobile forms have required-field validation and error states. Also removed 3 em-dashes from JSX code comments in InvoiceCheckerTool.jsx.

Note: backend event loop hung once during the iter255 report-generation flood (unrelated to code); recovered with `supervisorctl restart backend` (health 200).
...
- **A1 Rule Registry** (`/app/registry/cpr-rules-v1.yaml`, `/app/registry/cpr-categories-v1.yaml`): 31 seeded rules across all 6 categories (financial, category_placement, assessment, consumer_directed_care, clinical_best_practice, cross_tool_reconciliation), each carrying `rule_id`, `source_type`, verified `source_citation`/`source_url`, `confidence_tier` bound to source (L3), `severity`, `addressee_primary`/`addressee_secondary` (C1), and the L2 re-sign-off gates. All categories await category-level solicitor sign-off.
- **A2 Two-stage pipeline** (`backend/lib/cpr_rules.py` `enrich_findings`): findings are bound to the registry — citation/severity/confidence/addressees come from the registry, not the model; findings without a valid rule_id are demoted to freeform info/low with "Verification required"; banned claims (minimum RN hours, transport midpoints, minimum service hours by classification) are suppressed; title/body-incoherent findings dropped (A5/L6).
- **A3 Verification panel** (`run_verification_panel`, all Decimal): 5 flagship affirmative checks always render pass / flag / missing-info (never silence) — care-management 10% cap (AT-HM excluded), AT-HM ring-fence, personal-care Clinical-Care placement post 01/10/2026, rollover cap `max($1,000, 10% of quarterly budget)`, quarterly budget vs INDEX-1 classification value. A cannot_run names the missing field.
- **A6 confidence calibration** + **banned-rule enforcement** wired into `services/care_plan_analysis.analyse_care_plan`; `verification_panel` returned from public review endpoints (`routes/care_plans.py`).
- **A7 golden CI** (`backend/tests/test_cpr_err_a_golden.py`, `scripts/ci_cpr.sh`, fixture `tests/fixtures/CPR-ERR-A-golden-v1/robert_henderson_q1_2027.txt`): 8/8 green — asserts no fabricated RN-minimum, all 5 checks pass, goal-service + laundry findings emitted, header extraction (Provider/Effective), every finding has rule_id + addressee_primary, registry has no banned rule and ≥30 rules across ≥6 categories.
- **UI (web + mobile parity)**: `cp-verification-panel` (with `cp-check-<name>` rows) renders above the Findings list; interim M2 banner (`cp-safety-banner`) retained; A4 display fix — missing header fields show "Not stated in document" instead of a stray comma; preview Effective dates now DD/MM/YYYY on both surfaces. Verified iter238: web fully automated PASS; mobile at 1:1 source parity (Expo web-preview SecureStore auth injection blocked, same env limit as iter235–237).

**Still pending (Workstreams B/C/D of the single gate, not shipped this session):** UX overhaul (B1–B9: screen redesign, processing state, saved `care_plan_reviews`, filter/search, plan-summary panel, downloadable PDF), cross-tool draft-letter handoff to LF-1 (C1) + borrowed-data chips + delete-review, parity manifest, and category-level solicitor sign-off (gates flipping the mitigation flag off). Backlog: CHSP invoice-scan OCR (mobile), evidence-bundle PDF in-app, CE-2/LF-1 minor parity.
...
## Iter276 — Account Health Score (web) + decoder duplicate-summary removal
- NEW backend GET /api/account/health (participant_profile.py) — aggregates outstanding setup into a score + checklist: email verification (user.email_verified), each active participant's missing Tier-1 fields (reuses missing_required_fields/compute_profile_completeness) and pension_status=="unsure", each item carrying a fix_route + fix_label. Returns {score_pct, complete, done_count, total_count, outstanding_count, items[]}. Verified: cathy → 11%, 8 outstanding.
- NEW web component AccountHealthCard.jsx — teal card with an SVG circular progress ring (clay arc, % in centre), expandable checklist; each outstanding item deep-links (React Router Link) to its fix_route (/verify-email, /app/participants); done items shown struck-through. Placed on CaregiverDashboard right after the hero greeting. Verified on live dashboard.
- Statement Decoder: removed the duplicate "In plain English" summary box (DecoderResultView SECTION 0) — kept only the structured summary banner.
- STILL TODO (user-selected): (a) mobile Account Health Score parity (needs react-native-svg ring on the Expo dashboard, shared /account/health endpoint already done); (b) Draft Letter Prefill across all tools + single button; (c) Consistent plain-language coloured-card treatment for Price Checker, Care Plan, Contribution, Budget, CHSP; (d) System colour overhaul via design_agent (Phase 3) — remember custom color names (primary-k/gold/sage) are NOT valid Tailwind gradient stops, use arbitrary hex/rgba.

## Iter277 — Mobile Account Health Ring (parity with web)
- NEW mobile component src/components/AccountHealth.tsx: useAccountHealth() hook (GET /account/health, refetch on focus), HealthRing (react-native-svg circle, light/dark variants), AccountHealthCard (teal header + ring + expandable checklist; each outstanding item routes to the mobile fix screen — /participants for profile+pension, /(tabs)/settings for email; done items struck-through).
- Wired into app/(tabs)/index.tsx: compact HealthRing (light, 44px, "11%") in a NEW top row next to "Hi, {firstName}" top-right (taps → /participants); full AccountHealthCard under the greeting hero. Verified on Expo preview (cathy → 11%, "8 things left").
- STILL TODO (user-selected): weekly reminder if setup incomplete (DECISION NEEDED: true push notifications = Firebase google-services.json + only works after Publish/native build, vs in-app weekly nudge that works everywhere); Draft Letter Prefill (one button, prefilled from issues) across all tools; Consistent plain-language coloured-card treatment for Price Checker/Care Plan/Contribution/Budget/CHSP; System colour overhaul via design_agent (Phase 3).

## Iter278 — Weekly in-app nudge + System Colour Overhaul (foundation pass)
- Weekly in-app nudge (option b, no push): web AccountHealthCard fires a sonner toast at most once/7 days (localStorage wayly_health_nudge_ts) when setup incomplete; mobile AccountHealthCard shows a dismissible gold banner once/7 days (storage key health_nudge_ts) that opens the checklist. Verified rendering on mobile.
- design_agent run → wrote /app/design_guidelines.json (teal #0E4D52 / clay #A5512B / sage #6B8F71, colour-blocking, light+dark). NOTE the agent output assumes a Shadcn --card/--secondary token system; Wayly uses custom --kindred-* vars + utilities (bg-primary-k/bg-surface/bg-gold), so I adapted the INTENT into the existing system rather than swapping tokens.
- System colour FOUNDATION (affects every page): web index.css — base --kindred-bg #FBF8F3→#EFE8DB (warm sand, less white), --kindred-surface #FFFFFF→#FBF7F1 (soft cream cards); added a fixed page-wide radial gradient wash (teal+clay+sage tints) on body for light AND dark (html.theme-dark body); mode-aware .app-topbar/.app-bottomnav classes (color-mix on surface so the middle adapts to light/dark — fixed the cream streak in dark); new colour-block utilities .section-teal/.section-clay/.section-sage (+ *-soft). Verified dashboard in BOTH light and dark — strong WCAG contrast (dark text on cream, white text on teal/clay). Mobile parity: theme tokens lightColors bg→#EFE8DB, surface→#FBF7F1, surface2→#F1E9DC.
- STILL TODO: per-page section colour-blocking across every remaining screen (apply .section-* / coloured cards page-by-page: AI Tools hub, Statements, Billing, Participants, Family, Guided Journeys, etc.); Draft Letter Prefill (#1); Consistent Tool Colour for Price Checker/Care Plan/Contribution/Budget/CHSP (#2). Mobile full gradient wash (expo-linear-gradient) optional next.

## Iter279 — Dark de-grey, tile colour variety, per-item flag backgrounds, "viewing care for" indicator
- Dark mode de-greyed: web --kindred dark tokens bg #0B1416→#06201F, surface #152425→#0E3033, surface-2 #1C2F31→#14403F, border #2A3A3C→#1F5052 (all teal-toned, no grey). Mobile darkColors bg/surface/surface2 matched. Verified dark dashboard = teal throughout, strong WCAG contrast.
- Dashboard action tiles ("what would you like to do") rebalanced from 3 clay tones to a balanced teal/clay/sage rotation (#0E4D52,#A5512B,#3E6A4C,#0B3B3E,#B5623A,#2E5540 — 2 each, no adjacent repeats) on web (DashboardActionBar.jsx) + mobile (DashboardActionBar.tsx).
- Per-item flag/issue backgrounds: new plainText.FLAG_TINTS + flagTint(i) — rotating teal/clay/sage/plum soft fill + coloured left-accent border so each card visibly stands apart. Applied to DecoderResultView AnomalyCard, InvoiceResultView IssueCard, dashboard Things To Know items, StatementDetail anomalies.
- Web dashboard now shows a teal "Viewing care for {name}" pill under the greeting (participant context clarity — user confirmed switching works, just wanted it obvious). Mobile already shows the "VIEWING CARE FOR" switcher bar.
- STILL TODO (user list, deferred for budget): full per-page section colour pass (AI Tools, Profile, Statements, Billing, Participants, Family — top-to-bottom); Consistent Tool Colour for Price Checker/Care Plan/Contribution/Budget/CHSP; Mobile gradient wash (expo-linear-gradient); Draft Letter Prefill (one button, prefilled from issues) on every tool. Also add graphics/iconography per section.

## Iter280 — UX overhaul wave 1: dashboard bugs + Classification Self-Check redesign (web + mobile)
PHASE 1 bugs (testing_agent iter276 PASS):
- Invoice duplicate check: /api/invoices/upload now computes a SHA-256 and returns 409 DUPLICATE_EXACT (existing_invoice_id, provider, date, created_at) on re-upload of the same bytes — previously only statements had this. Web (inv1-duplicate card + "Open the existing check") + mobile parity. Stores file_sha256 on the invoice doc.
- Dashboard greeting em-dash removed: "Good evening, Cathy. What would you like to do?" (web + mobile).
- Trial banner copy fix: no longer says "Add a payment method to keep access" (trial users already added a card at signup) → "Trial: X days remaining. Your plan starts automatically when the trial ends." (web TrialCountdownBanner).
- Dashboard consolidation: removed the standalone ProfileCompletionBanner ("...accuracy guarantees...") + EmailVerificationBanner from the top of CaregiverDashboard; both are now surfaced inside the "Finish setting up your account" Account Health card.

PHASE 2a — Classification Self-Check redesign (testing_agent iter277 PASS, web + mobile, light + dark):
- NEW backend GET /api/public/csc/bands → 8 classifications with authoritative annual/quarterly budgets (INDEX-1) + plain-language headline/descriptor.
- Prefill: "current classification" now prefilled from the active participant (web activeParticipant.classification / mobile active.classification_level) with a "Prefilled from {name}" hint.
- Result redesigned + widened (web max-w-5xl): TEAL white-text header with band label, plain-language headline + one-line description (replaces jargon "Jean-style profile" — now sourced from band descriptor), two budget tiles (per year / per quarter). NEW comparison visual (csc-comparison) showing all 8 levels as labelled bars with the user's band highlighted. Coloured driver cards (clay/gold soft). Mobile at parity, dark-mode legible.

REMAINING from user's overhaul request (not yet done): Provider Price Checker redesign (clay/teal sections, charged-vs-median visual, fix Report-Issue hover visibility, coloured You-charged/Your-share/Indicative-range); Contribution Estimator redesign (graphics, fewer words, coloured sections); Support Plan Reviewer (goals feature, prefill classification+quarterly budget, block attach while running, richer review + summary/goals callout, ask-questions, draft-letter-from-findings, wider redesign); Invoice Checker wider (match Statement Decoder); wider AI/guided-workflow screens generally (e.g. carer/self-assessment); Statement screen colour pass (Private note + Ask Wayly bg, disclaimer to bottom, Stream Breakdown coloured + white text, Your Rights redesign); Money & Statements colour pass (Budget Scenario teal/clay, no white); Profile page more white text in light mode; global more-white-text/less-black + DD/MM/YYYY sweep; mobile expo-linear-gradient wash; Draft Letter Prefill one-button across all tools.

## Iter281 — UX overhaul wave 2: tool colour pass + CSC plain-English (web + mobile, tested iter277-280)
- Classification Self-Check v2: added plain-English "Wayly Summary" card (translates band → weekly $ figure + who pays, no jargon), teal white-text header, enriched 8-level comparison (per-level plain description + annual + weekly ≈ $/wk + "You're likely here" badge), prefill of current classification from active participant. Web + mobile, light + dark. New GET /api/public/csc/bands endpoint.
- Provider Price Checker: 3 result cards now distinct colours — charged=teal, your share=clay, indicative range=sage (web sect-* classes; mobile left-accent + soft tints incl. new primarySoft token). New charged-vs-median range-track visual (web) with charge pin + Above/Within-range badge. Shared ReportIssueButton fixed: "Something not right?" + "Report an Issue" now solid clay/white, readable on hover (was inverting to unreadable) — fixes across ALL tools.
- Budget & Lifetime Cap Calculator: "Applicable supplements (optional)" section now clay-toned; result cards coloured (annual=teal, supplements=clay, lifetime=sage). Web + mobile.
- Invoice Checker widened to max-w-[1720px] (matches Statement Decoder). ToolHero gained a `wide` prop → title/back-link/subheading left-align with the wide tool card; hero subheading widened to max-w-4xl (fewer line-wraps) for ALL tools.

STILL REMAINING from user's overhaul brief (large follow-on, NOT yet done):
- Contribution Estimator redesign (graphics, fewer words, coloured sections, white text) — web + mobile.
- Support Plan Reviewer: goals feature, prefill classification + quarterly budget, block attach while running, richer review (summary/overview + goals/no-goals callout), ask-questions, draft-letter-from-findings, visual aids, wider redesign — web + mobile.
- Statement detail screen: Private note + Ask Wayly bg colours, disclaimer moved to page bottom, Stream Breakdown coloured + white text, Your Rights redesign, more clay — web + mobile.
- Money & Statements colour pass across every screen (Budget Scenario teal/clay, no white) — web + mobile.
- Profile page: more white text in light mode; global "more white text / less black" + more clay/teal on EVERY page/section (ongoing standard).
- Guided Journeys screens widened (e.g. carer/self-assessment).
- Global DD/MM/YYYY sweep on all summaries; mobile expo-linear-gradient background wash; Draft Letter Prefill one-button across all tools.

---

## 2026-06 — Backend Stability Pass + Short-Term Pathways tool (web + mobile)

### P0 Backend Stability (root cause of recurring /api/health 502 hangs)
- Cause: `backend/document_extract.py:extract_document()` (async) ran heavy SYNC CPU work directly on the event loop — pdfplumber text parse, `pdf2image` 200-DPI rasterisation (up to 8 pages), OpenCV rotate, PIL/JPEG encode. Any statement/invoice/care-plan PDF or photo upload blocked the whole loop for seconds, so `/api/health` timed out → 502 while the process still showed RUNNING.
- Fix: offloaded every sync extraction call (`_extract_docx_text`, `_extract_pdf_selectable_text`, `_pdf_to_image_pages_b64`, `_image_to_base64_jpeg`) to worker threads via `asyncio.to_thread` (same pattern already used in `email_service.py`).
- Added `backend/event_loop_monitor.py` — a cheap background probe that logs a WARNING when the loop stalls beyond `EVENT_LOOP_LAG_WARN_SECONDS` (default 1s), so any future blocker is diagnosable (this hang had recurred 3× across forks precisely because it was invisible). Started in a FastAPI startup hook; stopped on shutdown. Disable via `EVENT_LOOP_MONITOR_ENABLED=0`.
- Verified (testing agent iter 309): `/api/health` <2s; decode offload safe.

### Short-Term Pathways tool (NEW) — Restorative Care + End-of-Life checker
- Placement: lives under **Guided Journeys** (web sidebar `Layout.jsx` guided_journeys group → `/app/short-term-pathways`; mobile drawer `navGroups.ts` journeys group → `/short-term-pathways`). Deliberately NOT an AI Tool — removed from `frontend/src/config/toolRegistry.js`, `backend/data/tool_registry.yaml`, and mobile `(tabs)/ai-tools.tsx` (both registries back to 9 tools, in sync).
- Interactive form→result tool (matches other tools' standard): user picks a situation (recovering / end-of-life / exploring) + 1-2 Yes/No follow-ups → tailored result.
- Backend (deterministic, reference-only, public, no LLM):
  - `GET /api/public/short-term-pathways` — full detail for both pathways from the versioned `program_reference` register.
  - `POST /api/public/short-term-pathways/check` — screening answers → headline, recommended pathway(s), per-pathway confidence (likely/possible/explore) + plain-English eligibility note.
  - Figures (July 2026): Restorative $6,000/episode, $12,000 max, up to 16 wks (112 days), up to 2 episodes/yr 3 months apart. End-of-Life $25,000, ~12 wks up to 16, prognosis ~3 months.
- Files: `frontend/src/pages/tools/ShortTermPathways.jsx`, `mobile/src/components/tools/ShortTermPathways.tsx`, `mobile/app/short-term-pathways.tsx`, route in `frontend/src/App.js`.
- Visual system (per user feedback: "no dark text on dark backgrounds, reduce colour strength, less slop, lighter shades, collapse Who-it-is-for + Questions"): LIGHT-FIRST — white surfaces, thin accent strip, accent icon chip, soft teal/clay/sage tint callouts, dark readable text, colour as ACCENTS only. Removed the previous dark saturated header blocks + dot textures. Compact "Who it is for" callout; numbered-badge Questions panel. Responsive (phone / iPad / desktop verified). NO em-dashes or dash-as-separator anywhere.

### STILL REMAINING from the approved plan (next up)
- P0: Support Plan Reviewer checks 6-8 — AT-HM, Restorative, End-of-Life budget checks in `backend/lib/cpr_rules.py`.
- P1: Contribution Estimator broad audit vs MyAgedCare fee estimator (keep its "full picture" feel).
- P1: Prepare/verify `scripts/clean_prod_users.py` for the user to run against prod (test accounts like Sam@test.com; this preview uses LOCAL mongo `test_database`, so prod cannot be purged from here).

---

## 2026-06 — Support Plan Reviewer checks 6-8 + Contribution Estimator budget breakdown

### Support Plan Reviewer — new short-term budget checks (backend/lib/cpr_rules.py)
- Added verification checks 6-8, appended only when the plan states the relevant amount (conditional, so plans without them stay clean):
  - `at_hm_budget` — AT-HM allocation vs tier cap (low $500 / medium $2,000 / high $15,000). Over the high cap = flag prompting the user to confirm the exceedance was approved with clinical evidence (Aged Care Rules 2025 s211), not a hard fail.
  - `restorative_budget` — vs $6,000 episode / $12,000 max across episodes.
  - `end_of_life_budget` — vs $25,000 pathway budget.
- `build_facts` now extracts `at_hm_tier`, `restorative_budget`, `end_of_life_budget` from plan text; broadened the AT-HM regex to allocation|budget|funding|cap.
- Caps sourced from the INDEX-1 register via new `_index1()` helper (never model-derived). Golden suite updated (now 6 checks, all pass) and still green.

### Contribution Estimator — "budget summary by service category" (parity with My Aged Care fee estimator)
- Backend CE2 engine now returns `category_breakdown`: per-category (Clinical / Independence / Everyday) budget $, government $, you-pay $ and rate %, quarterly + annual. Totals reconcile to the classification quarterly budget and the quarterly contribution.
- New "Where your budget goes" panel on web (`ContributionEstimator.jsx`) and mobile (`ContributionEstimator.tsx`): per-category cards with a who-pays bar, sage/teal/clay tints, a teal totals block, and a Quarterly/Monthly toggle. Light aesthetic consistent with the Short-Term Pathways redesign.
- Verified (testing agent iter 310) on both surfaces + 67 CE2 unit tests pass. Class 5 example: budget $10,182.38/qtr, you $674.58, govt $9,507.80; clinical always $0.

### Testing
- iter 310: both features passed end-to-end on web + mobile. No critical bugs. Minor copy-parity nit (mobile "in budget" → fixed to "in your budget"). Pre-existing dev-only `<span> in <option>` hydration warning on CarePlanReviewer noted, not user-visible.
